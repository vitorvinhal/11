...
class QueuedGraphExitRuntime extends OrcaRuntimeService {
  ...
  mobile(worktreeId: string) {
    return this.getMobileSessionTabsForWorktree(worktreeId).tabs.flatMap(
      (tab) =>
        tab.type === "terminal"
          ? [
              {
                ptyId: tab.ptyId,
                handlePtyId: tab.terminal
                  ? this.handles.get(tab.terminal)?.ptyId
                  : null,
              },
            ]
          : [],
...
export async function runQueuedGraphExitScenario(
  ...
  try {
    ...
    if (replacement) {
      const next = await h.adapter.spawn({
        cols: 80,
        rows: 24,
    ...
    try {
      resolution = resolveStablePaneOwner(
        runtime,
        store,
        `${TAB}:${LEAF}`,
        WT,
