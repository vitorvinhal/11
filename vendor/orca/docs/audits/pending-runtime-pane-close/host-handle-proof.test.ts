...
it.each([false, true])(
  ...
  async (replacement) => {
    ...
    runtime.setPtyController({ kill } as Parameters<
    ...
    p.runtimeCall.mockImplementation(async (request) => {
      ...
      return {
        ok: true,
        result: { close: await runtime.closeTerminal(params.terminal) },
      };
    });
    ...
    try {
      ...
      if (replacement) {
        ...
        expect(
          runtime.resolveTerminalPane(paneKey, "workspace").handle,
        ).not.toBe(original.handle);
      }
      ...
      expect(p.runtimeCall).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          params: { terminal: original.handle },
        }),
      );
      if (replacement) {
        expect(kill).not.toHaveBeenCalled();
        expect(
          runtime.resolveTerminalPane(paneKey, "workspace").connected,
        ).toBe(true);
      } else {
