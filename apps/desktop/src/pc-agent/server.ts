...
app.put('/api/sessions/:id/approve', authMiddleware, async (req: Request, res: Response) => {
  const userId = (req as any).user.userId;
  ...
  broadcastToUser(userId, {
    type: 'session_status',
    payload: { sessionId: session.id, status: session.status, approved: isApproved }
  });
...
app.patch('/api/sessions/:id/cancel', authMiddleware, async (req: Request, res: Response) => {
  ...
  if (session.status !== 'running' && session.status !== 'pending') {
    return res.status(400).json({ error: 'Cannot cancel session in current state' });
  }
      ...
      switch (message.type) {
        case 'auth': {
          ...
          if (decoded) {
            ...
            ws.send(JSON.stringify({ type: 'auth_error', payload: { error: 'Invalid token' } }));
          }
          break;
        }
        ...
        case 'session_create': {
          ...
          sessionStore.set(session.id, session);
          ws.send(JSON.stringify({ type: 'session_created', payload: session }));
          break;
        }
        ...
        case 'session_approve': {
          ...
          if (sessionToApprove && sessionToApprove.userId === userId && sessionToApprove.status === 'pending') {
            ...
            sessionToApprove.status = message.payload.approved ? 'running' : 'rejected';
            sessionToApprove.updatedAt = new Date().toISOString();
            sessionStore.set(sessionToApprove.id, sessionToApprove);

            ws.send(JSON.stringify({ type: 'session_approved', payload: sessionToApprove }));
        ...
        case 'session_cancel': {
          ...
          if (sessionToCancel && sessionToCancel.userId === userId &&
            ...
            ws.send(JSON.stringify({ type: 'session_cancelled', payload: sessionToCancel }));
          }
          break;
        }
      }
    } catch (error) {
      ws.send(JSON.stringify({ type: 'error', payload: { error: (error as Error).message } }));
    }
  });
...
httpServer.listen(PORT, () => {
  console.log(`PC Agent HTTP server running on port ${PORT}`);
  console.log(`PC Agent WebSocket server running on port ${PORT}`);
});

// Export for testing
export { app, httpServer, wss, sessionStore, wsClients };
