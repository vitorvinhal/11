...
import { _ as utils_default, m as parseFontSize, t as ZERO_WIDTH_SPACE } from "./chunk-75Z2AOVW-DlWr2fif.js";
import { a as drawRect, c as getTextObj, i as drawImage, n as drawBackgroundRect, r as drawEmbeddedImage, s as getNoteRect } from "./chunk-F27PBJKO-DSkpyj5-.js";
...
var import_dist = require_dist(), parser = (function() {
	var t = /* @__PURE__ */ __name(function(e, t, n, r) {
		...
		64,
		73
	], L = [1, 74], R = [1, 80], z = [1, 81], B = [1, 82], V = [1, 83], H = [1, 84], Be = [1, 85], Ve = [1, 86], He = [1, 87], Ue = [1, 88], We = [1, 89], Ge = [1, 90], Ke = [1, 91], qe = [1, 92], Je = [1, 93], Ye = [1, 94], U = [1, 95], W = [1, 96], Xe = [1, 97], Ze = [1, 98], Qe = [1, 99], $e = [1, 100], et = [1, 101], G = [1, 102], tt = [1, 103], nt = [1, 104], rt = [1, 105], it = [2, 78], K = [
		4,
		5,
		17,
		51,
...
`;
	...
	let r = e.append("foreignObject"), a = await renderKatexSanitized(t.text, getConfig()), o = r.append("xhtml:div").attr("style", "width: fit-content;").attr("xmlns", "http://www.w3.org/1999/xhtml").html(a).node().getBoundingClientRect();
	...
	let r = 0, i = 0, a = n.text.split(common_default.lineBreakRegex), [o, s] = parseFontSize(n.fontSize), c = [], l = 0, u = /* @__PURE__ */ __name(() => n.y, "yfunc");
	if (n.valign !== void 0 && n.textMargin !== void 0 && n.textMargin > 0) switch (n.valign) {
		case "top":
		case "start":
			u = /* @__PURE__ */ __name(() => Math.round(n.y + n.textMargin), "yfunc");
			break;
		case "middle":
		case "center":
			u = /* @__PURE__ */ __name(() => Math.round(n.y + (r + i + n.textMargin) / 2), "yfunc");
			break;
		case "bottom":
		case "end":
			u = /* @__PURE__ */ __name(() => Math.round(n.y + (r + i + 2 * n.textMargin) - n.textMargin), "yfunc");
			break;
	}
...
}, "fixLifeLineHeights"), drawActorTypeParticipant = /* @__PURE__ */ __name(function(e, t, n, i, a) {
	...
	var h = m;
	i || (actorCnt++, Object.keys(t.links || {}).length && !n.forceMenus && h.attr("onclick", popupMenuToggle(`actor${actorCnt}_popup`)).attr("cursor", "pointer"), h.append("line").attr("id", "actor" + actorCnt).attr("x1", s).attr("y1", c).attr("x2", s).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", t.name).attr("data-et", "life-line").attr("data-id", t.name), h = m.append("g"), t.actorCnt = actorCnt, t.links != null && h.attr("id", "root-" + actorCnt), l === "neo" && h.attr("data-look", "neo"));
	let g = getNoteRect();
	var _ = "actor";
	t.properties?.class ? _ = t.properties.class : g.fill = "#eaeaea", i ? _ += ` ${BOTTOM_ACTOR_CLASS}` : _ += ` ${TOP_ACTOR_CLASS}`, g.x = t.x, g.y = o, g.width = t.width, g.height = t.height, g.class = _, g.rx = 3, g.ry = 3, g.name = t.name, l === "neo" && (g.rx = 6, g.ry = 6);
	let v = drawRect2(h, g), y = a.get(t.name) ?? 0;
	if (COLOR_THEMES.has(u) && (v.style("stroke", p[y % p.length]), v.style("fill", f[y % p.length])), l === "neo" && v.attr("filter", "url(#drop-shadow)"), t.rectData = g, t.properties?.icon) {
	...
	i || (h.attr("data-et", "participant"), h.attr("data-type", "participant"), h.attr("data-id", t.name)), _drawTextCandidateFunc(n, hasKatex(t.description))(t.description, h, g.x, g.y, g.width, g.height, { class: `actor ${ACTOR_BOX_CLASS}` }, n);
	let b = t.height;
	if (v.node) {
	...
	return b;
}, "drawActorTypeParticipant"), drawActorTypeCollections = /* @__PURE__ */ __name(function(e, t, n, i, a) {
	let o = i ? t.stopy : t.starty, s = t.x + t.width / 2, c = o + t.height, { look: l, theme: u, themeVariables: d } = n, { bkgColorArray: f, borderColorArray: p } = d, m = e.append("g").lower();
	var h = m;
	i || (actorCnt++, Object.keys(t.links || {}).length && !n.forceMenus && h.attr("onclick", popupMenuToggle(`actor${actorCnt}_popup`)).attr("cursor", "pointer"), h.append("line").attr("id", "actor" + actorCnt).attr("x1", s).attr("y1", c).attr("x2", s).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", t.name).attr("data-et", "life-line").attr("data-id", t.name), h = m.append("g"), t.actorCnt = actorCnt, t.links != null && h.attr("id", "root-" + actorCnt), l === "neo" && h.attr("data-look", "neo"));
	let g = getNoteRect();
	var _ = "actor";
	t.properties?.class ? _ = t.properties.class : g.fill = "#eaeaea", i ? _ += ` ${BOTTOM_ACTOR_CLASS}` : _ += ` ${TOP_ACTOR_CLASS}`, g.x = t.x, g.y = o, g.width = t.width, g.height = t.height, g.class = _, g.name = t.name;
	let v = {
		...g,
		x: g.x + -6,
		y: g.y + 6,
	...
	if (COLOR_THEMES.has(u) && (y.style("stroke", p[x % p.length]), y.style("fill", f[x % p.length]), b.style("stroke", p[x % p.length]), b.style("fill", f[x % p.length])), t.properties?.icon) {
		let e = t.properties.icon.trim();
		e.charAt(0) === "@" ? drawEmbeddedImage(h, g.x + g.width - 20, g.y + 10, e.substr(1)) : drawImage(h, g.x + g.width - 20, g.y + 10, e);
	}
...
}, "drawActorTypeCollections"), drawActorTypeQueue = /* @__PURE__ */ __name(function(e, t, n, i, a) {
	...
	t.properties?.class ? _ = t.properties.class : g.fill = "#eaeaea", i ? _ += ` ${BOTTOM_ACTOR_CLASS}` : _ += ` ${TOP_ACTOR_CLASS}`, h.attr("class", _), g.x = t.x, g.y = o, g.width = t.width, g.height = t.height, g.name = t.name;
  ...
  `;
	...
	if (COLOR_THEMES.has(u) && (b.style("stroke", p[C % p.length]), b.style("fill", f[C % p.length]), x.style("stroke", p[C % p.length]), x.style("fill", f[C % p.length])), t.properties?.icon) {
...
}, "drawActorTypeQueue"), drawActorTypeControl = /* @__PURE__ */ __name(function(e, t, n, i, a, o) {
	...
	i || (actorCnt++, _.append("line").attr("id", "actor" + actorCnt).attr("x1", c).attr("y1", l).attr("x2", c).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", t.name).attr("data-et", "life-line").attr("data-id", t.name), t.actorCnt = actorCnt);
	let v = e.append("g"), y = ACTOR_MAN_FIGURE_CLASS;
	i ? y += ` ${BOTTOM_ACTOR_CLASS}` : y += ` ${TOP_ACTOR_CLASS}`, v.attr("class", y), v.attr("name", t.name);
	let b = getNoteRect();
	...
	i ? g += ` ${BOTTOM_ACTOR_CLASS}` : g += ` ${TOP_ACTOR_CLASS}`, h.attr("class", g), h.attr("name", t.name);
	let _ = getNoteRect();
	...
	return COLOR_THEMES.has(u) && (h.style("stroke", p[b % p.length]), h.style("fill", f[b % p.length])), t.height = h.node().getBBox().height + (n?.sequence?.labelBoxHeight ?? 0), i || (actorCnt++, m.append("line").attr("id", "actor" + actorCnt).attr("x1", s).attr("y1", c).attr("x2", s).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", t.name).attr("data-et", "life-line").attr("data-id", t.name), t.actorCnt = actorCnt), _drawTextCandidateFunc(n, hasKatex(t.description))(t.description, h, _.x, _.y + (i ? 15 : 30), _.width, _.height, { class: `actor ${ACTOR_MAN_FIGURE_CLASS}` }, n), i ? h.attr("transform", "translate(0, 22)") : (h.attr("transform", `translate(0, ${22 / 2 - 5})`), h.attr("data-et", "participant"), h.attr("data-type", "entity"), h.attr("data-id", t.name)), t.height;
}, "drawActorTypeEntity"), drawActorTypeDatabase = /* @__PURE__ */ __name(function(e, t, n, i, a) {
	...
	i || (actorCnt++, Object.keys(t.links || {}).length && !n.forceMenus && g.attr("onclick", popupMenuToggle(`actor${actorCnt}_popup`)).attr("cursor", "pointer"), g.append("line").attr("id", "actor" + actorCnt).attr("x1", s).attr("y1", c).attr("x2", s).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", t.name).attr("data-et", "life-line").attr("data-id", t.name), g = h.append("g"), t.actorCnt = actorCnt, t.links != null && g.attr("id", "root-" + actorCnt), d === "neo" && g.attr("data-look", "neo"));
	let _ = getNoteRect(), v = "actor";
	t.properties?.class ? v = t.properties.class : _.fill = "#eaeaea", i ? v += ` ${BOTTOM_ACTOR_CLASS}` : v += ` ${TOP_ACTOR_CLASS}`, _.x = t.x, _.y = o, _.width = t.width, _.height = t.height, _.class = v, _.name = t.name, _.x = t.x, _.y = o;
...
`;
	...
	COLOR_THEMES.has(l) ? (C.style("stroke", p[T % p.length]), C.style("fill", f[T % p.length])) : C.style("stroke", m), C.attr("transform", `translate(${y}, ${S})`), t.rectData = _, _drawTextCandidateFunc(n, hasKatex(t.description))(t.description, g, _.x, _.y + 35, _.width, _.height, { class: `actor ${ACTOR_BOX_CLASS}` }, n);
...
}, "drawActorTypeDatabase"), drawActorTypeBoundary = /* @__PURE__ */ __name(function(e, t, n, i, a) {
	...
	i || (actorCnt++, l.append("line").attr("id", "actor" + actorCnt).attr("x1", s).attr("y1", c).attr("x2", s).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", t.name).attr("data-et", "life-line").attr("data-id", t.name), t.actorCnt = actorCnt);
	let g = e.append("g"), _ = ACTOR_MAN_FIGURE_CLASS;
	i ? _ += ` ${BOTTOM_ACTOR_CLASS}` : _ += ` ${TOP_ACTOR_CLASS}`, g.attr("class", _), g.attr("name", t.name);
	let v = getNoteRect();
	v.x = t.x, v.y = o, v.fill = "#eaeaea", v.width = t.width, v.height = t.height, v.class = "actor", g.append("line").attr("id", "actor-man-torso" + actorCnt).attr("x1", t.x + t.width / 2 - 22 * 2.5).attr("y1", o + 12).attr("x2", t.x + t.width / 2 - 15).attr("y2", o + 12), g.append("line").attr("id", "actor-man-arms" + actorCnt).attr("x1", t.x + t.width / 2 - 22 * 2.5).attr("y1", o + 2).attr("x2", t.x + t.width / 2 - 22 * 2.5).attr("y2", o + 22), g.append("circle").attr("cx", t.x + t.width / 2).attr("cy", o + 12).attr("r", 22), u === "neo" && g.attr("filter", "url(#drop-shadow)");
	let y = a.get(t.name) ?? 0;
	return COLOR_THEMES.has(d) ? (g.style("stroke", m[y % m.length]), g.style("fill", p[y % m.length])) : g.style("stroke", h), t.height = g.node().getBBox().height + (n.sequence.labelBoxHeight ?? 0), _drawTextCandidateFunc(n, hasKatex(t.description))(t.description, g, v.x, v.y + 15, v.width, v.height, { class: `actor ${ACTOR_MAN_FIGURE_CLASS}` }, n), g.attr("transform", "translate(0,21)"), i || (g.attr("data-et", "participant"), g.attr("data-type", "boundary"), g.attr("data-id", t.name)), t.height;
}, "drawActorTypeBoundary"), drawActorTypeActor = /* @__PURE__ */ __name(function(e, t, n, i, a) {
	...
	i || (actorCnt++, h.append("line").attr("id", "actor" + actorCnt).attr("x1", s).attr("y1", c).attr("x2", s).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", t.name).attr("data-et", "life-line").attr("data-id", t.name), t.actorCnt = actorCnt);
	let g = e.append("g"), _ = ACTOR_MAN_FIGURE_CLASS;
	i ? _ += ` ${BOTTOM_ACTOR_CLASS}` : _ += ` ${TOP_ACTOR_CLASS}`, g.attr("class", _), g.attr("name", t.name), i || g.attr("data-et", "participant").attr("data-type", "actor").attr("data-id", t.name);
	let v = l === "neo" ? .5 : 1, y = l === "neo" ? o + (1 - v) * 30 : o;
	g.append("line").attr("id", "actor-man-torso" + actorCnt).attr("x1", s).attr("y1", y + 25 * v).attr("x2", s).attr("y2", y + 45 * v), g.append("line").attr("id", "actor-man-arms" + actorCnt).attr("x1", s - ACTOR_TYPE_WIDTH / 2 * v).attr("y1", y + 33 * v).attr("x2", s + ACTOR_TYPE_WIDTH / 2 * v).attr("y2", y + 33 * v), g.append("line").attr("x1", s - ACTOR_TYPE_WIDTH / 2 * v).attr("y1", y + 60 * v).attr("x2", s).attr("y2", y + 45 * v), g.append("line").attr("x1", s).attr("y1", y + 45 * v).attr("x2", s + (ACTOR_TYPE_WIDTH / 2 - 2) * v).attr("y2", y + 60 * v);
	...
	return COLOR_THEMES.has(u) ? (g.style("stroke", p[S % p.length]), g.style("fill", f[S % p.length])) : g.style("stroke", m), _drawTextCandidateFunc(n, hasKatex(t.description))(t.description, g, x.x, y + 35 * v - (l === "neo" ? 10 : 0), x.width, x.height, { class: `actor ${ACTOR_MAN_FIGURE_CLASS}` }, n), t.height;
...
}, "drawActivation"), drawLoop = /* @__PURE__ */ __name(async function(t, n, i, a, o) {
	let { boxMargin: s, boxTextMargin: c, labelBoxHeight: l, labelBoxWidth: u, messageFontFamily: d, messageFontSize: f, messageFontWeight: p } = a, m = t.append("g").attr("data-et", "control-structure").attr("data-id", "i" + o.id), h = /* @__PURE__ */ __name(function(e, t, n, r) {
	...
	let g = getTextObj();
	g.text = i, g.x = n.startx, g.y = n.starty, g.fontFamily = d, g.fontSize = f, g.fontWeight = p, g.anchor = "middle", g.valign = "middle", g.tspan = !1, g.width = Math.max(u ?? 0, 50), g.height = l + (a.look === "neo" ? 15 : 0) || 20, g.textMargin = c, g.class = "labelText", drawLabel(m, g), g = getTextObj2(), g.text = n.title, g.x = n.startx + u / 2 + (n.stopx - n.startx) / 2, g.y = n.starty + s + c, g.anchor = "middle", g.valign = "middle", g.textMargin = c, g.class = "loopText", g.fontFamily = d, g.fontSize = f, g.fontWeight = p, g.wrap = !0;
	...
	if (n.sectionTitles !== void 0) {
		for (let [e, t] of Object.entries(n.sectionTitles)) if (t.message) {
			g.text = t.message, g.x = n.startx + (n.stopx - n.startx) / 2, g.y = n.sections[e].y + s + c, g.class = "sectionTitle", g.anchor = "middle", g.valign = "middle", g.tspan = !1, g.fontFamily = d, g.fontSize = f, g.fontWeight = p, g.wrap = n.wrap, hasKatex(g.text) ? (n.starty = n.sections[e].y, await drawKatex(m, g, n)) : drawText(m, g);
	...
	function n(e, t, n, r, i, a, s, c) {
		let { actorFontSize: l, actorFontFamily: u, actorFontWeight: d } = c, [f, p] = parseFontSize(l), m = e.split(common_default.lineBreakRegex);
	...
	async function a(e, t, r, a, s, c, l, u) {
		let f = await calculateMathMLDimensions(e, getConfig()), m = t.append("switch"), h = m.append("foreignObject").attr("x", r + s / 2 - f.width / 2).attr("y", a + c / 2 - f.height / 2).attr("width", f.width).attr("height", f.height).append("xhtml:div").style("height", "100%").style("width", "100%");
		h.append("div").style("text-align", "center").style("vertical-align", "middle").html(await renderKatexSanitized(e, getConfig())), n(e, m, r, a, s, c, l, u), o(h, l);
	}
	__name(a, "byKatex");
	function o(e, t) {
		for (let n in t) t.hasOwnProperty(n) && e.attr(n, t[n]);
	}
	return __name(o, "_setTextAttrs"), function(e, i = !1) {
	...
	function n(e, t, n, r, a, o, s, c) {
		let { actorFontSize: l, actorFontFamily: u, actorFontWeight: d } = c, f = e.split(common_default.lineBreakRegex);
	...
	insert: /* @__PURE__ */ __name(function(e, t, n, r) {
		let i = common_default.getMin(e, n), a = common_default.getMax(e, n), o = common_default.getMin(t, r), s = common_default.getMax(t, r);
		this.updateVal(bounds.data, "startx", i, Math.min), this.updateVal(bounds.data, "starty", o, Math.min), this.updateVal(bounds.data, "stopx", a, Math.max), this.updateVal(bounds.data, "stopy", s, Math.max), this.updateBounds(i, o, a, s);
	}, "insert"),
	...
	let i = getNoteRect();
	i.x = t.startx, i.y = t.starty, i.width = t.width || conf.width, i.class = "note";
	let a = e.append("g");
	a.attr("data-et", "note"), a.attr("data-id", "i" + n);
	let o = svgDraw_default.drawRect(a, i), s = getTextObj();
	s.x = t.startx, s.y = t.starty, s.width = i.width, s.dy = "1em", s.text = t.message, s.class = "noteText", s.fontFamily = conf.noteFontFamily, s.fontSize = conf.noteFontSize, s.fontWeight = conf.noteFontWeight, s.anchor = conf.noteAlign, s.textMargin = conf.noteMargin, s.valign = "center";
	let c = hasKatex(s.text) ? await drawKatex(a, s) : drawText(a, s), l = Math.round(c.map((e) => (e._groups || e)[0][0].getBBox().height).reduce((e, t) => e + t));
	o.attr("height", l + 2 * conf.noteMargin), t.height += l + 2 * conf.noteMargin, bounds.bumpVerticalPos(l + 2 * conf.noteMargin), t.stopy = t.starty + l + 2 * conf.noteMargin, t.stopx = t.startx + i.width, bounds.insert(t.startx, t.starty, t.stopx, t.stopy), bounds.models.addNote(t);
...
async function boundMessage(e, t) {
	...
	let { startx: n, stopx: i, message: a } = t, o = common_default.splitBreaks(a).length, s = hasKatex(a), c = s ? await calculateMathMLDimensions(a, getConfig2()) : utils_default.calculateTextDimensions(a, messageFont(conf));
	if (!s) {
		let e = c.height / o;
	...
	if (n === i) {
		l = bounds.getVerticalPos() + u, conf.rightAngles || (u += conf.boxMargin, l = bounds.getVerticalPos() + u), u += 30;
		let e = common_default.getMax(d / 2, conf.width / 2);
		bounds.insert(n - e, bounds.getVerticalPos() - 10 + u, i + e, bounds.getVerticalPos() + 30 + u);
	} else u += conf.boxMargin, l = bounds.getVerticalPos() + u, bounds.insert(n, l - 10, i, l);
	return bounds.bumpVerticalPos(u), t.height += u, t.stopy = t.starty + t.height, bounds.insert(t.fromBounds, t.starty, t.toBounds, t.stopy), l;
}
__name(boundMessage, "boundMessage");
var drawMessage = /* @__PURE__ */ __name(async function(e, t, n, i, a, o) {
	let { startx: s, stopx: l, starty: u, message: d, type: f, sequenceIndex: p, sequenceVisible: m } = t, g = utils_default.calculateTextDimensions(d, messageFont(conf)), _ = getTextObj();
	_.x = Math.min(s, l), _.y = u + 10, _.width = Math.abs(l - s), _.class = "messageText", _.dy = "1em", _.text = d, _.fontFamily = conf.messageFontFamily, _.fontSize = conf.messageFontSize, _.fontWeight = conf.messageFontWeight, _.anchor = conf.messageAlign, _.valign = "center", _.textMargin = conf.wrapPadding, _.tspan = !1, hasKatex(_.text) ? await drawKatex(e, _, {
		startx: s,
		stopx: l,
		starty: n
	}) : drawText(e, _);
	let v = g.width, y;
	if (s === l) {
		let r = m || conf.showSequenceNumbers, o = isReverseArrowType(a, i), c = isBidirectionalArrowType(a, i), u = s + (r && (o || c) ? 10 : 0);
		y = conf.rightAngles ? e.append("path").attr("d", `M  ${u},${n} H ${s + common_default.getMax(conf.width / 2, v / 2)} V ${n + 25} H ${s}`) : e.append("path").attr("d", "M " + u + "," + n + " C " + (u + 60) + "," + (n - 10) + " " + (s + 60) + "," + (n + 30) + " " + s + "," + (n + 20)), hasCentralConnection(a, i) && drawCentralConnection(e, a, t, i, s, l, n);
	} else y = e.append("line"), y.attr("x1", s), y.attr("y1", n), y.attr("x2", l), y.attr("y2", n), hasCentralConnection(a, i) && drawCentralConnection(e, a, t, i, s, l, n);
...
}, "drawMessage"), addActorRenderingData = /* @__PURE__ */ __name(function(e, t, n, r, i, a, o) {
	let s = 0, c = 0, l, u = 0;
	for (let e of r) {
		let r = t.get(e), a = r.box;
		l && l != a && (o || bounds.models.addBox(l), c += conf.boxMargin + l.margin), a && a != l && (o || (a.x = s + c, a.y = i), c += a.margin), r.width = common_default.getMax(r.width || conf.width, conf.width), r.height = common_default.getMax(r.height || conf.height, conf.height), r.margin = r.margin || conf.actorMargin, u = common_default.getMax(u, r.height), n.get(r.name) && (c += r.width / 2), r.x = s + c, r.starty = bounds.getVerticalPos(), bounds.insert(r.x, i, r.x + r.width, r.height), s += r.width + c, r.box && (r.box.width = s + a.margin - r.box.x), c = r.margin, l = r.box, bounds.models.addActor(r);
	}
	l && !o && bounds.models.addBox(l), bounds.bumpVerticalPos(u);
}, "addActorRenderingData"), drawActors = /* @__PURE__ */ __name(async function(e, t, n, r, i, a, o) {
	if (r) {
		let r = 0;
		...
		for (let s of n) {
			...
			let c = await svgDraw_default.drawActor(e, n, conf, !0, i, a, o);
			r = common_default.getMax(r, c);
		}
		bounds.bumpVerticalPos(r + conf.boxMargin);
	} else for (let r of n) {
		let n = t.get(r);
		await svgDraw_default.drawActor(e, n, conf, !1, i, a, o);
	}
...
function adjustLoopHeightForWrap(e, n, r, i, a) {
	bounds.bumpVerticalPos(r);
	let o = i;
	if (n.id && n.message && e[n.id]) {
		let r = e[n.id].width, a = messageFont(conf);
		n.message = utils_default.wrapLabel(`[${n.message}]`, r - 2 * conf.wrapPadding, a), n.width = r, n.wrap = !0;
		let s = utils_default.calculateTextDimensions(n.message, a), c = common_default.getMax(s.height, conf.labelBoxHeight);
		o = i + c, log.debug(`${c} - ${n.message}`);
	}
	a(n), bounds.bumpVerticalPos(o);
}
...
var draw = /* @__PURE__ */ __name(async function(r, i, a, o) {
	let { securityLevel: s, sequence: c, look: l, themeVariables: u } = getConfig2();
	conf = c;
	let d;
	s === "sandbox" && (d = select_default("#i" + i));
	let p = select_default(s === "sandbox" ? d.nodes()[0].contentDocument.body : "body"), m = s === "sandbox" ? d.nodes()[0].contentDocument : document;
	bounds.init(), log.debug(o.db);
	let h = s === "sandbox" ? p.select(`[id="${i}"]`) : select_default(`[id="${i}"]`), g = o.db.getActors(), _ = o.db.getCreatedActors(), y = o.db.getDestroyedActors(), b = o.db.getBoxes(), x = o.db.getActorKeys(), S = o.db.getMessages(), C = o.db.getDiagramTitle(), w = o.db.hasAtLeastOneBox(), T = o.db.hasAtLeastOneBoxWithTitle(), E = await getMaxMessageWidthPerActor(g, S, o);
	if (conf.height = await calculateActorMargins(g, E, b), svgDraw_default.insertComputerIcon(h, i), svgDraw_default.insertDatabaseIcon(h, i), svgDraw_default.insertClockIcon(h, i), w && (bounds.bumpVerticalPos(conf.boxMargin), T && bounds.bumpVerticalPos(b[0].textMaxHeight)), conf.hideUnusedParticipants === !0) {
	...
	addActorRenderingData(h, g, _, x, 0, S, !1);
	let O = await calculateLoopBounds(S, g, E, o);
	svgDraw_default.insertArrowHead(h, i), svgDraw_default.insertArrowCrossHead(h, i), svgDraw_default.insertArrowFilledHead(h, i), svgDraw_default.insertSequenceNumber(h, i), svgDraw_default.insertSolidTopArrowHead(h, i), svgDraw_default.insertSolidBottomArrowHead(h, i), svgDraw_default.insertStickTopArrowHead(h, i), svgDraw_default.insertStickBottomArrowHead(h, i), l === "neo" && svgDraw_default.insertDropShadow(h, conf);
	function k(e, t) {
		let n = bounds.endActivation(e);
		n.starty + 18 > t && (n.starty = t - 6, t += 12), svgDraw_default.drawActivation(h, n, t, conf, actorActivations(e.from).length, o, D), bounds.insert(n.startx, t - 10, n.stopx, t);
	}
	__name(k, "activeEnd");
	let A = 1, j = 1, M = [], N = [], P = 0;
	for (let e of S) {
		...
		switch (e.type) {
			...
			case o.db.LINETYPE.ACTIVE_END:
				k(e, bounds.getVerticalPos());
				break;
			case o.db.LINETYPE.LOOP_START:
				adjustLoopHeightForWrap(O, e, conf.boxMargin, conf.boxMargin + conf.boxTextMargin, (e) => bounds.newLoop(e));
				break;
			case o.db.LINETYPE.LOOP_END:
				n = bounds.endLoop(), await svgDraw_default.drawLoop(h, n, "loop", conf, e), bounds.bumpVerticalPos(n.stopy - bounds.getVerticalPos()), bounds.models.addLoop(n);
				break;
			case o.db.LINETYPE.RECT_START:
				adjustLoopHeightForWrap(O, e, conf.boxMargin, conf.boxMargin, (e) => {
					let t = e.message;
					t ||= u?.rectBkgColor || u?.actorBkg || "rgba(128, 128, 128, 0.5)", bounds.newLoop(void 0, t);
				});
				break;
			case o.db.LINETYPE.RECT_END:
				n = bounds.endLoop(), N.push(n), bounds.models.addLoop(n), bounds.bumpVerticalPos(n.stopy - bounds.getVerticalPos());
				break;
			case o.db.LINETYPE.OPT_START:
				adjustLoopHeightForWrap(O, e, conf.boxMargin, conf.boxMargin + conf.boxTextMargin, (e) => bounds.newLoop(e));
				break;
			case o.db.LINETYPE.OPT_END:
				n = bounds.endLoop(), await svgDraw_default.drawLoop(h, n, "opt", conf, e), bounds.bumpVerticalPos(n.stopy - bounds.getVerticalPos()), bounds.models.addLoop(n);
				break;
			case o.db.LINETYPE.ALT_START:
				adjustLoopHeightForWrap(O, e, conf.boxMargin, conf.boxMargin + conf.boxTextMargin, (e) => bounds.newLoop(e));
				break;
			case o.db.LINETYPE.ALT_ELSE:
				adjustLoopHeightForWrap(O, e, conf.boxMargin + conf.boxTextMargin, conf.boxMargin, (e) => bounds.addSectionToLoop(e));
				break;
			case o.db.LINETYPE.ALT_END:
				n = bounds.endLoop(), await svgDraw_default.drawLoop(h, n, "alt", conf, e), bounds.bumpVerticalPos(n.stopy - bounds.getVerticalPos()), bounds.models.addLoop(n);
				break;
			case o.db.LINETYPE.PAR_START:
			case o.db.LINETYPE.PAR_OVER_START:
				adjustLoopHeightForWrap(O, e, conf.boxMargin, conf.boxMargin + conf.boxTextMargin, (e) => bounds.newLoop(e)), bounds.saveVerticalPos();
				break;
			case o.db.LINETYPE.PAR_AND:
				adjustLoopHeightForWrap(O, e, conf.boxMargin + conf.boxTextMargin, conf.boxMargin, (e) => bounds.addSectionToLoop(e));
				break;
			case o.db.LINETYPE.PAR_END:
				n = bounds.endLoop(), await svgDraw_default.drawLoop(h, n, "par", conf, e), bounds.bumpVerticalPos(n.stopy - bounds.getVerticalPos()), bounds.models.addLoop(n);
				break;
			...
			case o.db.LINETYPE.CRITICAL_END:
				n = bounds.endLoop(), await svgDraw_default.drawLoop(h, n, "critical", conf, e), bounds.bumpVerticalPos(n.stopy - bounds.getVerticalPos()), bounds.models.addLoop(n);
				break;
			case o.db.LINETYPE.BREAK_START:
				adjustLoopHeightForWrap(O, e, conf.boxMargin, conf.boxMargin + conf.boxTextMargin, (e) => bounds.newLoop(e));
				break;
			case o.db.LINETYPE.BREAK_END:
				n = bounds.endLoop(), await svgDraw_default.drawLoop(h, n, "break", conf, e), bounds.bumpVerticalPos(n.stopy - bounds.getVerticalPos()), bounds.models.addLoop(n);
				break;
			default: try {
				i = e.msgModel, i.starty = bounds.getVerticalPos(), i.sequenceIndex = A, i.sequenceVisible = o.db.showSequenceNumbers(), i.id = e.id, i.from = e.from, i.to = e.to;
				let t = await boundMessage(h, i);
				adjustCreatedDestroyedData(e, i, t, P, g, _, y), M.push({
					messageModel: i,
					lineStartY: t,
					msg: e
	...
	log.debug("createdActors", _), log.debug("destroyedActors", y), await drawActors(h, g, x, !1, i, o, D);
	for (let e of M) await drawMessage(h, e.messageModel, e.lineStartY, o, e.msg, i);
	conf.mirrorActors && await drawActors(h, g, x, !0, i, o, D), N.forEach((e) => svgDraw_default.drawBackgroundRect(h, e)), fixLifeLineHeights(h, g, x, conf);
	...
	w && bounds.bumpVerticalPos(conf.boxMargin);
	let F = drawActorsPopup(h, g, x, m), { bounds: I } = bounds.getBounds();
	I.startx === void 0 && (I.startx = 0), I.starty === void 0 && (I.starty = 0), I.stopx === void 0 && (I.stopx = 0), I.stopy === void 0 && (I.stopy = 0);
	let L = I.stopy - I.starty;
	L < F.maxHeight && (L = F.maxHeight);
	let R = L + 2 * conf.diagramMarginY;
	conf.mirrorActors && (R = R - conf.boxMargin + conf.bottomMarginAdj);
	let z = I.stopx - I.startx;
	z < F.maxWidth && (z = F.maxWidth);
	let B = z + 2 * conf.diagramMarginX;
	C && h.append("text").text(C).attr("x", (I.stopx - I.startx) / 2 - 2 * conf.diagramMarginX).attr("y", -25), configureSvgSize(h, R, B, conf.useMaxWidth);
	let V = C ? 40 : 0, H = g.size && l === "neo" ? 30 : 0;
	h.attr("viewBox", I.startx - conf.diagramMarginX + " -" + (conf.diagramMarginY + V) + " " + B + " " + (R + V + H)), log.debug("models:", bounds.models);
}, "draw");
async function getMaxMessageWidthPerActor(e, n, i) {
	...
	for (let t of n) if (e.get(t.to) && e.get(t.from)) {
		...
		let o = t.placement !== void 0, s = !o, c = o ? noteFont(conf) : messageFont(conf), l = t.wrap ? utils_default.wrapLabel(t.message, conf.width - 2 * conf.wrapPadding, c) : t.message, u = (hasKatex(l) ? await calculateMathMLDimensions(t.message, getConfig2()) : utils_default.calculateTextDimensions(l, c)).width + 2 * conf.wrapPadding;
		s && t.from === n.nextActor ? a[t.to] = common_default.getMax(a[t.to] || 0, u) : s && t.from === n.prevActor ? a[t.from] = common_default.getMax(a[t.from] || 0, u) : s && t.from === t.to ? (a[t.from] = common_default.getMax(a[t.from] || 0, u / 2), a[t.to] = common_default.getMax(a[t.to] || 0, u / 2)) : t.placement === i.db.PLACEMENT.RIGHTOF ? a[t.from] = common_default.getMax(a[t.from] || 0, u) : t.placement === i.db.PLACEMENT.LEFTOF ? a[n.prevActor] = common_default.getMax(a[n.prevActor] || 0, u) : t.placement === i.db.PLACEMENT.OVER && (n.prevActor && (a[n.prevActor] = common_default.getMax(a[n.prevActor] || 0, u / 2)), n.nextActor && (a[t.from] = common_default.getMax(a[t.from] || 0, u / 2)));
	}
...
async function calculateActorMargins(e, t, n) {
	...
	for (let t of e.keys()) {
		...
		n.width = n.wrap ? conf.width : common_default.getMax(conf.width, a.width + 2 * conf.wrapPadding), n.height = n.wrap ? common_default.getMax(a.height, conf.height) : conf.height, i = common_default.getMax(i, n.height);
	}
	for (let n in t) {
		let r = e.get(n);
		if (!r) continue;
	...
	return n.forEach((t) => {
		...
		let o = utils_default.calculateTextDimensions(t.name, n);
		a = common_default.getMax(o.height, a);
		let s = common_default.getMax(r, o.width + 2 * conf.wrapPadding);
		if (t.margin = conf.boxTextMargin, r < s) {
			let e = (s - r) / 2;
			t.margin += e;
		}
	}), n.forEach((e) => e.textMaxHeight = a), common_default.getMax(i, conf.height);
}
__name(calculateActorMargins, "calculateActorMargins");
var buildNoteModel = /* @__PURE__ */ __name(async function(e, n, i) {
	let a = n.get(e.from), o = n.get(e.to), s = a.x, c = o.x, l = e.wrap && e.message, u = hasKatex(e.message) ? await calculateMathMLDimensions(e.message, getConfig2()) : utils_default.calculateTextDimensions(l ? utils_default.wrapLabel(e.message, conf.width, noteFont(conf)) : e.message, noteFont(conf)), d = {
		width: l ? conf.width : common_default.getMax(conf.width, u.width + 2 * conf.noteMargin),
		height: 0,
		startx: a.x,
		stopx: 0,
		starty: 0,
	...
	return e.placement === i.db.PLACEMENT.RIGHTOF ? (d.width = l ? common_default.getMax(conf.width, u.width) : common_default.getMax(a.width / 2 + o.width / 2, u.width + 2 * conf.noteMargin), d.startx = s + (a.width + conf.actorMargin) / 2) : e.placement === i.db.PLACEMENT.LEFTOF ? (d.width = l ? common_default.getMax(conf.width, u.width + 2 * conf.noteMargin) : common_default.getMax(a.width / 2 + o.width / 2, u.width + 2 * conf.noteMargin), d.startx = s - d.width + (a.width - conf.actorMargin) / 2) : e.to === e.from ? (u = utils_default.calculateTextDimensions(l ? utils_default.wrapLabel(e.message, common_default.getMax(conf.width, a.width), noteFont(conf)) : e.message, noteFont(conf)), d.width = l ? common_default.getMax(conf.width, a.width) : common_default.getMax(a.width, conf.width, u.width + 2 * conf.noteMargin), d.startx = s + (a.width - d.width) / 2) : (d.width = Math.abs(s + a.width / 2 - (c + o.width / 2)) + conf.actorMargin, d.startx = s < c ? s + a.width / 2 - conf.actorMargin / 2 : c + o.width / 2 - conf.actorMargin / 2), l && (d.message = utils_default.wrapLabel(e.message, d.width - 2 * conf.wrapPadding, noteFont(conf))), log.debug(`NM:[${d.startx},${d.stopx},${d.starty},${d.stopy}:${d.width},${d.height}=${e.message}]`), d;
	...
	let [a, o] = activationBounds(t.from, n), [s, c] = activationBounds(t.to, n), l = a <= s, u = l ? o : a, d = l ? s : c;
	i === "neo" && (t.type !== r.db.LINETYPE.SOLID_OPEN && (d += l ? -3 : 3), (t.type === r.db.LINETYPE.BIDIRECTIONAL_SOLID || t.type === r.db.LINETYPE.BIDIRECTIONAL_DOTTED) && (u += l ? 3 : -3)), u += calculateCentralConnectionOffset(t, r, l);
	let f = Math.abs(s - c) > 2, p = /* @__PURE__ */ __name((e) => l ? -e : e, "adjustValue");
	t.from === t.to ? d = u : (t.activate && !f && (d += p(conf.activationWidth / 2 - 1)), [
		r.db.LINETYPE.SOLID_OPEN,
		r.db.LINETYPE.DOTTED_OPEN,
		r.db.LINETYPE.STICK_TOP,
		r.db.LINETYPE.STICK_BOTTOM,
	...
	t.wrap && t.message && (t.message = utils_default.wrapLabel(t.message, common_default.getMax(g + 2 * conf.wrapPadding, conf.width), messageFont(conf)));
	let _ = utils_default.calculateTextDimensions(t.message, messageFont(conf));
	return {
		width: common_default.getMax(t.wrap ? 0 : _.width + 2 * conf.wrapPadding, g + 2 * conf.wrapPadding, conf.width),
		height: 0,
		startx: u,
		stopx: d,
		starty: 0,
	...
	for (let t of e) {
		...
		t.placement === void 0 ? (l = buildMessageModel(t, n, i), t.msgModel = l, l.startx && l.stopx && o.length > 0 && o.forEach((e) => {
			if (s = e, l.startx === l.stopx) {
				let e = n.get(t.from), r = n.get(t.to);
				s.from = common_default.getMin(e.x - l.width / 2, e.x - e.width / 2, s.from), s.to = common_default.getMax(r.x + l.width / 2, r.x + e.width / 2, s.to), s.width = common_default.getMax(s.width, Math.abs(s.to - s.from)) - conf.labelBoxWidth;
			} else s.from = common_default.getMin(l.startx, s.from), s.to = common_default.getMax(l.stopx, s.to), s.width = common_default.getMax(s.width, l.width) - conf.labelBoxWidth;
		})) : (c = await buildNoteModel(t, n, i), t.noteModel = c, o.forEach((e) => {
			s = e, s.from = common_default.getMin(s.from, c.startx), s.to = common_default.getMax(s.to, c.startx + c.width), s.width = common_default.getMax(s.width, Math.abs(s.from - s.to)) - conf.labelBoxWidth;
		}));
	}
