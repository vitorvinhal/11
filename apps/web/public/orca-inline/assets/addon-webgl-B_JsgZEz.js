...
[
	...
	"Win32",
	"WinCE"
].includes(ce), ce.indexOf("Linux"), /\bCrOS\b/.test(O0);
...
((e) => {
	function t(e, t, n, r) {
		return r === void 0 ? `#${E0(e)}${E0(t)}${E0(n)}` : `#${E0(e)}${E0(t)}${E0(n)}${E0(r)}`;
	}
	e.toCss = t;
...
((e) => {
	function t(e, t) {
		...
		return P = a + Math.round((n - a) * x), B = o + Math.round((r - o) * x), w = s + Math.round((i - s) * x), {
			css: Z.toCss(P, B, w),
			rgba: Z.toRgba(P, B, w)
		};
	}
	...
	e.isOpaque = n;
	function r(e, t, n) {
		let r = d0.ensureContrastRatio(e.rgba, t.rgba, n);
		if (r) return Z.toColor(r >> 24 & 255, r >> 16 & 255, r >> 8 & 255);
	}
	...
	e.opaque = i;
	function a(e, t) {
		return x = Math.round(t * 255), [P, B, w] = d0.toChannels(e.rgba), {
			css: Z.toCss(P, B, w, x),
	...
	e.opacity = a;
	function o(e, t) {
		return x = e.rgba & 255, a(e, x * t / 255);
	}
	e.multiplyOpacity = o;
	function s(e) {
		return [
...
((e) => {
	...
	function r(e) {
		if (e.match(/#[\da-f]{3,8}/i)) switch (e.length) {
			case 4: return P = parseInt(e.slice(1, 2).repeat(2), 16), B = parseInt(e.slice(2, 3).repeat(2), 16), w = parseInt(e.slice(3, 4).repeat(2), 16), Z.toColor(P, B, w);
			case 5: return P = parseInt(e.slice(1, 2).repeat(2), 16), B = parseInt(e.slice(2, 3).repeat(2), 16), w = parseInt(e.slice(3, 4).repeat(2), 16), x = parseInt(e.slice(4, 5).repeat(2), 16), Z.toColor(P, B, w, x);
			case 7: return {
				css: e,
		...
		if (r) return P = parseInt(r[1], 10), B = parseInt(r[2], 10), w = parseInt(r[3], 10), x = Math.round((r[5] === void 0 ? 1 : parseFloat(r[5])) * 255), Z.toColor(P, B, w, x);
		if (e === "transparent") return {
			css: "transparent",
			rgba: 0
		};
		if (!t || !n || (t.fillStyle = n, t.fillStyle = e, typeof t.fillStyle != "string") || (t.fillRect(0, 0, 1, 1), [P, B, w, x] = t.getImageData(0, 0, 1, 1).data, x !== 255)) throw Error("css.toColor: Unsupported css format");
		return {
			rgba: Z.toRgba(P, B, w, x),
			css: e
		};
...
((e) => {
	function t(e, t) {
		if (x = (t & 255) / 255, x === 1) return t;
		let n = t >> 24 & 255, r = t >> 16 & 255, i = t >> 8 & 255, a = e >> 24 & 255, o = e >> 16 & 255, s = e >> 8 & 255;
		return P = a + Math.round((n - a) * x), B = o + Math.round((r - o) * x), w = s + Math.round((i - s) * x), Z.toRgba(P, B, w);
	}
	e.blend = t;
	function n(e, t, n) {
		let a = V.relativeLuminance(e >> 8), o = V.relativeLuminance(t >> 8);
		if (h0(a, o) < n) {
			if (o < a) {
				let o = r(e, t, n), s = h0(a, V.relativeLuminance(o >> 8));
				if (s < n) {
					let r = i(e, t, n);
					return s > h0(a, V.relativeLuminance(r >> 8)) ? o : r;
				}
				return o;
			}
			let s = i(e, t, n), c = h0(a, V.relativeLuminance(s >> 8));
			if (c < n) {
				let i = r(e, t, n);
				return c > h0(a, V.relativeLuminance(i >> 8)) ? s : i;
			}
			return s;
		}
	}
	...
	function r(e, t, n) {
		let r = e >> 24 & 255, i = e >> 16 & 255, a = e >> 8 & 255, o = t >> 24 & 255, s = t >> 16 & 255, c = t >> 8 & 255, u = h0(V.relativeLuminance2(o, s, c), V.relativeLuminance2(r, i, a));
		for (; u < n && (o > 0 || s > 0 || c > 0);) o -= Math.max(0, Math.ceil(o * .1)), s -= Math.max(0, Math.ceil(s * .1)), c -= Math.max(0, Math.ceil(c * .1)), u = h0(V.relativeLuminance2(o, s, c), V.relativeLuminance2(r, i, a));
		return (o << 24 | s << 16 | c << 8 | 255) >>> 0;
	}
	e.reduceLuminance = r;
	function i(e, t, n) {
		let r = e >> 24 & 255, i = e >> 16 & 255, a = e >> 8 & 255, o = t >> 24 & 255, s = t >> 16 & 255, c = t >> 8 & 255, u = h0(V.relativeLuminance2(o, s, c), V.relativeLuminance2(r, i, a));
		for (; u < n && (o < 255 || s < 255 || c < 255);) o = Math.min(255, o + Math.ceil((255 - o) * .1)), s = Math.min(255, s + Math.ceil((255 - s) * .1)), c = Math.min(255, c + Math.ceil((255 - c) * .1)), u = h0(V.relativeLuminance2(o, s, c), V.relativeLuminance2(r, i, a));
		return (o << 24 | s << 16 | c << 8 | 255) >>> 0;
	}
...
function Ze(e, t, n, r) {
	return t === 1 && n > Math.ceil(r * 1.5) && e !== void 0 && e > 255 && !yt(e) && !W0(e) && !Mt(e);
}
function V0(e) {
	return W0(e) || At(e);
}
...
var l;
((e) => (e.LINE_H = Object.freeze({
	type: 2,
	data: "M0,.5 L1,.5",
	strokeWidth: 1
}), e.LINE_V = Object.freeze({
	type: 2,
	data: "M.5,0 L.5,1",
	strokeWidth: 1
}), e.FADE_RIGHT = Object.freeze({
	type: 2,
	data: "M0,.5 L.28,.5 M.32,.5 L.52,.5 M.60,.5 L.72,.5 M.84,.5 L.90,.5",
	strokeWidth: 1
}), e.FADE_LEFT = Object.freeze({
	type: 2,
	data: "M.10,.5 L.16,.5 M.28,.5 L.40,.5 M.48,.5 L.68,.5 M.72,.5 L1,.5",
	strokeWidth: 1
}), e.FADE_DOWN = Object.freeze({
	type: 2,
	data: "M.5,0 L.5,.28 M.5,.32 L.5,.52 M.5,.60 L.5,.72 M.5,.84 L.5,.90",
	strokeWidth: 1
}), e.FADE_UP = Object.freeze({
	type: 2,
	data: "M.5,.10 L.5,.16 M.5,.28 L.5,.40 M.5,.48 L.5,.68 M.5,.72 L.5,1",
	strokeWidth: 1
}), e.CURVE_DOWN_RIGHT = Object.freeze({
	type: 2,
	data: (e, t) => `M.5,1 L.5,${.5 + t / .15 * .5} C.5,${.5 + t / .15 * .5},.5,.5,1,.5`,
	strokeWidth: 1
}), e.CURVE_DOWN_LEFT = Object.freeze({
	type: 2,
	data: (e, t) => `M.5,1 L.5,${.5 + t / .15 * .5} C.5,${.5 + t / .15 * .5},.5,.5,0,.5`,
	strokeWidth: 1
}), e.CURVE_UP_RIGHT = Object.freeze({
	type: 2,
	data: (e, t) => `M.5,0 L.5,${.5 - t / .15 * .5} C.5,${.5 - t / .15 * .5},.5,.5,1,.5`,
	strokeWidth: 1
}), e.CURVE_UP_LEFT = Object.freeze({
	type: 2,
	data: (e, t) => `M.5,0 L.5,${.5 - t / .15 * .5} C.5,${.5 - t / .15 * .5},.5,.5,0,.5`,
	strokeWidth: 1
}), e.NODE_FILL = Object.freeze({
	type: 3,
	data: "M.85,.5 A.35,.175,0,1,1,.15,.5 A.35,.175,0,1,1,.85,.5"
}), e.NODE_STROKE = Object.freeze({
	type: 3,
	data: "M.85,.5 A.35,.175,0,1,1,.15,.5 A.35,.175,0,1,1,.85,.5",
	strokeWidth: 1
}), e.NODE_LINE_RIGHT = Object.freeze({
	type: 3,
	data: "M1,.5 L.85,.5",
	strokeWidth: 1
}), e.NODE_LINE_LEFT = Object.freeze({
	type: 3,
	data: "M0,.5 L.15,.5",
	strokeWidth: 1
}), e.NODE_LINE_DOWN = Object.freeze({
	type: 3,
	data: "M.5,1 L.5,.7",
	strokeWidth: 1
}), e.NODE_LINE_UP = Object.freeze({
	type: 3,
	data: "M.5,0 L.5,.3",
	strokeWidth: 1
})))(l ||= {});
var Ve = {
	...
	"═": {
		type: 2,
		data: (e, t) => `M0,${.5 - t} L1,${.5 - t} M0,${.5 + t} L1,${.5 + t}`,
		strokeWidth: 1
	},
	"║": {
		type: 2,
		data: (e, t) => `M${.5 - e},0 L${.5 - e},1 M${.5 + e},0 L${.5 + e},1`,
		strokeWidth: 1
	},
	"╒": {
		type: 2,
		data: (e, t) => `M.5,1 L.5,${.5 - t} L1,${.5 - t} M.5,${.5 + t} L1,${.5 + t}`,
		strokeWidth: 1
	},
	"╓": {
		type: 2,
		data: (e, t) => `M${.5 - e},1 L${.5 - e},.5 L1,.5 M${.5 + e},.5 L${.5 + e},1`,
		strokeWidth: 1
	},
	"╔": {
		type: 2,
		data: (e, t) => `M1,${.5 - t} L${.5 - e},${.5 - t} L${.5 - e},1 M1,${.5 + t} L${.5 + e},${.5 + t} L${.5 + e},1`,
		strokeWidth: 1
	},
	"╕": {
		type: 2,
		data: (e, t) => `M0,${.5 - t} L.5,${.5 - t} L.5,1 M0,${.5 + t} L.5,${.5 + t}`,
		strokeWidth: 1
	},
	"╖": {
		type: 2,
		data: (e, t) => `M${.5 + e},1 L${.5 + e},.5 L0,.5 M${.5 - e},.5 L${.5 - e},1`,
		strokeWidth: 1
	},
	"╗": {
		type: 2,
		data: (e, t) => `M0,${.5 + t} L${.5 - e},${.5 + t} L${.5 - e},1 M0,${.5 - t} L${.5 + e},${.5 - t} L${.5 + e},1`,
		strokeWidth: 1
	},
	"╘": {
		type: 2,
		data: (e, t) => `M.5,0 L.5,${.5 + t} L1,${.5 + t} M.5,${.5 - t} L1,${.5 - t}`,
		strokeWidth: 1
	},
	"╙": {
		type: 2,
		data: (e, t) => `M1,.5 L${.5 - e},.5 L${.5 - e},0 M${.5 + e},.5 L${.5 + e},0`,
		strokeWidth: 1
	},
	"╚": {
		type: 2,
		data: (e, t) => `M1,${.5 - t} L${.5 + e},${.5 - t} L${.5 + e},0 M1,${.5 + t} L${.5 - e},${.5 + t} L${.5 - e},0`,
		strokeWidth: 1
	},
	"╛": {
		type: 2,
		data: (e, t) => `M0,${.5 + t} L.5,${.5 + t} L.5,0 M0,${.5 - t} L.5,${.5 - t}`,
		strokeWidth: 1
	},
	"╜": {
		type: 2,
		data: (e, t) => `M0,.5 L${.5 + e},.5 L${.5 + e},0 M${.5 - e},.5 L${.5 - e},0`,
		strokeWidth: 1
	},
	"╝": {
		type: 2,
		data: (e, t) => `M0,${.5 - t} L${.5 - e},${.5 - t} L${.5 - e},0 M0,${.5 + t} L${.5 + e},${.5 + t} L${.5 + e},0`,
		strokeWidth: 1
	},
	"╞": {
		type: 2,
		data: (e, t) => `M.5,0 L.5,1 M.5,${.5 - t} L1,${.5 - t} M.5,${.5 + t} L1,${.5 + t}`,
		strokeWidth: 1
	},
	"╟": {
		type: 2,
		data: (e, t) => `M${.5 - e},0 L${.5 - e},1 M${.5 + e},0 L${.5 + e},1 M${.5 + e},.5 L1,.5`,
		strokeWidth: 1
	},
	"╠": {
		type: 2,
		data: (e, t) => `M${.5 - e},0 L${.5 - e},1 M1,${.5 + t} L${.5 + e},${.5 + t} L${.5 + e},1 M1,${.5 - t} L${.5 + e},${.5 - t} L${.5 + e},0`,
		strokeWidth: 1
	},
	"╡": {
		type: 2,
		data: (e, t) => `M.5,0 L.5,1 M0,${.5 - t} L.5,${.5 - t} M0,${.5 + t} L.5,${.5 + t}`,
		strokeWidth: 1
	},
	"╢": {
		type: 2,
		data: (e, t) => `M0,.5 L${.5 - e},.5 M${.5 - e},0 L${.5 - e},1 M${.5 + e},0 L${.5 + e},1`,
		strokeWidth: 1
	},
	"╣": {
		type: 2,
		data: (e, t) => `M${.5 + e},0 L${.5 + e},1 M0,${.5 + t} L${.5 - e},${.5 + t} L${.5 - e},1 M0,${.5 - t} L${.5 - e},${.5 - t} L${.5 - e},0`,
		strokeWidth: 1
	},
	"╤": {
		type: 2,
		data: (e, t) => `M0,${.5 - t} L1,${.5 - t} M0,${.5 + t} L1,${.5 + t} M.5,${.5 + t} L.5,1`,
		strokeWidth: 1
	},
	"╥": {
		type: 2,
		data: (e, t) => `M0,.5 L1,.5 M${.5 - e},.5 L${.5 - e},1 M${.5 + e},.5 L${.5 + e},1`,
		strokeWidth: 1
	},
	"╦": {
		type: 2,
		data: (e, t) => `M0,${.5 - t} L1,${.5 - t} M0,${.5 + t} L${.5 - e},${.5 + t} L${.5 - e},1 M1,${.5 + t} L${.5 + e},${.5 + t} L${.5 + e},1`,
		strokeWidth: 1
	},
	"╧": {
		type: 2,
		data: (e, t) => `M.5,0 L.5,${.5 - t} M0,${.5 - t} L1,${.5 - t} M0,${.5 + t} L1,${.5 + t}`,
		strokeWidth: 1
	},
	"╨": {
		type: 2,
		data: (e, t) => `M0,.5 L1,.5 M${.5 - e},.5 L${.5 - e},0 M${.5 + e},.5 L${.5 + e},0`,
		strokeWidth: 1
	},
	"╩": {
		type: 2,
		data: (e, t) => `M0,${.5 + t} L1,${.5 + t} M0,${.5 - t} L${.5 - e},${.5 - t} L${.5 - e},0 M1,${.5 - t} L${.5 + e},${.5 - t} L${.5 + e},0`,
		strokeWidth: 1
	},
	"╪": {
		type: 2,
		data: (e, t) => `M.5,0 L.5,1 M0,${.5 - t} L1,${.5 - t} M0,${.5 + t} L1,${.5 + t}`,
		strokeWidth: 1
	},
	"╫": {
		type: 2,
		data: (e, t) => `M0,.5 L1,.5 M${.5 - e},0 L${.5 - e},1 M${.5 + e},0 L${.5 + e},1`,
		strokeWidth: 1
	},
	"╬": {
		type: 2,
		data: (e, t) => `M0,${.5 + t} L${.5 - e},${.5 + t} L${.5 - e},1 M1,${.5 + t} L${.5 + e},${.5 + t} L${.5 + e},1 M0,${.5 - t} L${.5 - e},${.5 - t} L${.5 - e},0 M1,${.5 - t} L${.5 + e},${.5 - t} L${.5 + e},0`,
		strokeWidth: 1
	},
	"╭": {
		type: 2,
	...
	...Object.fromEntries(Array.from({ length: 256 }, (e, t) => [String.fromCodePoint(10240 + t), {
		type: 6,
		data: t
	}]))
}, $e = new Set([
...
function f(e) {
	return {
		...
		data: () => {
			...
			for (let i = 0; i < 3; i++) {
				let a = e >> i * 2 & 1, o = e >> i * 2 + 1 & 1;
				a && o ? t.push(`M0,${r[i]} L1,${r[i]} L1,${r[i] + n[i]} L0,${r[i] + n[i]} Z`) : a ? t.push(`M0,${r[i]} L0.5,${r[i]} L0.5,${r[i] + n[i]} L0,${r[i] + n[i]} Z`) : o && t.push(`M0.5,${r[i]} L1,${r[i]} L1,${r[i] + n[i]} L0.5,${r[i] + n[i]} Z`);
			}
			return t.join(" ");
		}
	};
...
function o0(e) {
	let t = [];
	if (e & 64) {
		let e = .21500000000000002, n = .7849999999999999;
		t.push(`M${e},0.2125 L${e + .075},0.175 L${n - .075},0.175 L${n},0.2125 L${n - .075},0.25 L${e + .075},0.25 Z`);
	}
	if (e & 32) {
		let e = .7999999999999999, n = .95 - .15 / 2, r = .95, i = .265, a = .485;
		t.push(`M${n},${i} L${r},${i + .0375} L${r},${a - .0375} L${n},${a} L${e},${a - .0375} L${e},${i + .0375} Z`);
	}
	if (e & 16) {
		let e = .7999999999999999, n = .95 - .15 / 2, r = .95, i = .515, a = .735;
		t.push(`M${n},${i} L${r},${i + .0375} L${r},${a - .0375} L${n},${a} L${e},${a - .0375} L${e},${i + .0375} Z`);
	}
	if (e & 8) {
		let e = .21500000000000002, n = .7849999999999999;
		t.push(`M${e},0.7875 L${e + .075},0.75 L${n - .075},0.75 L${n},0.7875 L${n - .075},0.825 L${e + .075},0.825 Z`);
	}
	if (e & 4 && t.push("M0.125,0.515 L0.2,0.5525 L0.2,0.6975 L0.125,0.735 L0.05,0.6975 L0.05,0.5525 Z"), e & 2 && t.push("M0.125,0.265 L0.2,0.3025 L0.2,0.4475 L0.125,0.485 L0.05,0.4475 L0.05,0.3025 Z"), e & 1) {
		let e = .21500000000000002, n = .7849999999999999;
		t.push(`M${e},0.5 L${e + .075},0.4625 L${n - .075},0.4625 L${n},0.5 L${n - .075},0.5375 L${e + .075},0.5375 Z`);
	}
	return t.join(" ");
}
var k = 0, H = 0, T0 = !1, _0 = !1, $0 = !1, z, x0 = 0, G0 = class {
	constructor(e, t, n, r, i, a) {
		this._terminal = e, this._optionService = t, this._selectionRenderModel = n, this._decorationService = r, this._coreBrowserService = i, this._themeService = a, this.result = {
			fg: 0,
			bg: 0,
			ext: 0
		};
	...
	resolve(e, t, n, r, i) {
		this.result.bg = e.bg, this.result.fg = e.fg, this.result.ext = e.bg & 268435456 ? e.extended.ext : 0, H = 0, k = 0, _0 = !1, T0 = !1, $0 = !1, z = this._themeService.colors, x0 = 0;
		let a = e.getCode();
		if (a !== 0 && e.extended.underlineStyle === 4) {
			let e = Math.max(1, Math.floor(this._optionService.rawOptions.fontSize * this._coreBrowserService.dpr / 15));
			x0 = t * r % (Math.round(e) * 2);
		}
		if (x0 === 0 && $e.has(a) && (x0 = t * r % 2 * 2 + n * i % 2), this._decorationService.forEachDecorationAtCell(t, n, "bottom", (e) => {
			e.backgroundColorRGB && (H = e.backgroundColorRGB.rgba >> 8 & 16777215, _0 = !0), e.foregroundColorRGB && (k = e.foregroundColorRGB.rgba >> 8 & 16777215, T0 = !0);
		}), $0 = this._selectionRenderModel.isCellSelected(this._terminal, t, n), $0) {
			...
			if (_0 = !0, z.selectionForeground && (k = z.selectionForeground.rgba >> 8 & 16777215, T0 = !0), V0(e.getCode())) {
				if (this.result.fg & 67108864 && !(this.result.bg & 50331648)) k = (this._coreBrowserService.isFocused ? z.selectionBackgroundOpaque : z.selectionInactiveBackgroundOpaque).rgba >> 8 & 16777215;
				else {
		...
		this._decorationService.forEachDecorationAtCell(t, n, "top", (e) => {
			e.backgroundColorRGB && (H = e.backgroundColorRGB.rgba >> 8 & 16777215, _0 = !0), e.foregroundColorRGB && (k = e.foregroundColorRGB.rgba >> 8 & 16777215, T0 = !0);
		}), _0 && (H = $0 ? e.bg & -150994944 | H | 50331648 : e.bg & -16777216 | H | 50331648), T0 && (k = e.fg & -83886080 | k | 50331648), this.result.fg & 67108864 && (_0 && !T0 && (k = this.result.bg & 50331648 ? this.result.fg & -134217728 | this.result.bg & 67108863 : this.result.fg & -134217728 | z.background.rgba >> 8 & 16777215 | 50331648, T0 = !0), !_0 && T0 && (H = this.result.fg & 50331648 ? this.result.bg & -67108864 | this.result.fg & 67108863 : this.result.bg & -67108864 | z.foreground.rgba >> 8 & 16777215 | 50331648, _0 = !0)), z = void 0, this.result.bg = _0 ? H : this.result.bg, this.result.fg = T0 ? k : this.result.fg, this.result.ext &= 536870911, this.result.ext |= x0 << 29 & 3758096384;
	}
}, Ke = .5, K0 = Be || we ? "bottom" : "ideographic", Pt = typeof OffscreenCanvas > "u" ? void 0 : (e, t) => new OffscreenCanvas(e, t), Bt = (e, t) => {
	let n = document.createElement("canvas");
	return n.width = e, n.height = t, n;
};
function wt(e, t, n = Pt, r = Bt) {
	return n?.(e, t) ?? r(e, t);
}
function qe(e, t, n, r, i, a, o, s, c, u, d, p, m = 0) {
	let h = Ve[t];
	if (h) {
		let t = Array.isArray(h) ? h : [h];
		for (let h of t) Ht(e, h, n, r, i, a, o, s, c, u, d, p, m);
		return !0;
	}
	return !1;
}
function Ht(e, t, n, r, i, a, o, s, c, u, d, p, m = 0) {
	let h = i, _ = a, v = n, y = r;
	switch (t.scaleType === 1 && (h = o, _ = s, v = n + (i - o) / 2, y = r + (a - s) / 2), t.clipPath && (e.save(), Kt(e, t.clipPath, v, y, h, _)), t.type) {
		case 0:
			kt(e, t.data, v, y, h, _);
			break;
		case 1:
			$t(e, t.data, v, y, h, _, m);
			break;
		case 2:
			Gt(e, t.data, v, y, h, _, u, d, t.strokeWidth);
			break;
		case 3:
			Ut(e, t.data, v, y, h, _, u, t.strokeWidth);
			break;
		case 4:
			Vt(e, t.data, v, y, h, _, u, p);
			break;
		case 5:
			zt(e, t.data, v, y, h, _, c, u, d);
			break;
		case 6:
			Zt(e, t.data, v, y, h, _);
			break;
	}
	t.clipPath && e.restore();
}
function kt(e, t, n, r, i, a) {
	for (let o = 0; o < t.length; o++) {
		let s = t[o], c = i / 8, u = a / 8;
		e.fillRect(n + s.x * c, r + s.y * u, s.w * c, s.h * u);
	}
}
...
function Zt(e, t, n, r, i, a) {
	let o = i / 8, s = a * .1, c = a * .8 / 8, u = Math.min(o, c);
	for (let i = 0; i < 8; i++) if (t & 1 << i) {
		let t = ze[i * 2], a = ze[i * 2 + 1], d = n + (t + 1) * o, p = r + s + (a + 1) * c;
		e.beginPath(), e.arc(d, p, u, 0, Math.PI * 2), e.fill();
	}
}
function Ut(e, t, n, r, i, a, o, s) {
	let c = typeof t == "string" ? t : t(0, 0);
	e.beginPath();
	let u = 0, d = 0, p = 0, m = 0, h = "";
	for (let t of c.split(" ")) {
		let o = t[0], s = t.substring(1).split(",");
		if (o === "Z") {
			e.closePath(), h = o;
			continue;
		}
		if (o === "V") {
			let t = r + parseFloat(s[0]) * a;
			e.lineTo(u, t), d = t, p = u, m = d, h = o;
			continue;
		}
		if (o === "H") {
			let t = n + parseFloat(s[0]) * i;
			e.lineTo(t, d), u = t, p = u, m = d, h = o;
			continue;
		}
		if (!s[0] || !s[1]) continue;
		if (o === "A") {
			let t = parseFloat(s[0]) * i, o = parseFloat(s[1]) * a, c = parseFloat(s[2]) * Math.PI / 180, p = parseInt(s[3], 10), m = parseInt(s[4], 10), h = n + parseFloat(s[5]) * i, _ = r + parseFloat(s[6]) * a;
			Wt(e, u, d, t, o, c, p, m, h, _), u = h, d = _;
			continue;
		}
		let c = s.map((e, t) => {
			let o = parseFloat(e);
			return t % 2 == 0 ? n + o * i : r + o * a;
		});
		if (o === "M") e.moveTo(c[0], c[1]), u = c[0], d = c[1], p = u, m = d;
		else if (o === "L") e.lineTo(c[0], c[1]), u = c[0], d = c[1], p = u, m = d;
		else if (o === "Q") e.quadraticCurveTo(c[0], c[1], c[2], c[3]), p = c[0], m = c[1], u = c[2], d = c[3];
		else if (o === "T") {
			let t, n;
			h === "Q" || h === "T" ? (t = 2 * u - p, n = 2 * d - m) : (t = u, n = d), e.quadraticCurveTo(t, n, c[0], c[1]), p = t, m = n, u = c[0], d = c[1];
		} else o === "C" && (e.bezierCurveTo(c[0], c[1], c[2], c[3], c[4], c[5]), p = c[2], m = c[3], u = c[4], d = c[5]);
		h = o;
	}
	s === void 0 ? e.fill() : (e.strokeStyle = e.fillStyle, e.lineWidth = o * s, e.stroke());
}
function Wt(e, t, n, r, i, a, o, s, c, u) {
	if (r === 0 || i === 0) {
		e.lineTo(c, u);
		return;
	}
	r = Math.abs(r), i = Math.abs(i);
	let d = Math.cos(a), p = Math.sin(a), m = (t - c) / 2, h = (n - u) / 2, _ = d * m + p * h, v = -p * m + d * h, y = r * r, b = i * i, C = _ * _, T = v * v, E = C / y + T / b;
	if (E > 1) {
		let e = Math.sqrt(E);
		r *= e, i *= e, y = r * r, b = i * i;
	}
	let D = (y * b - y * T - b * C) / (y * T + b * C);
	D < 0 && (D = 0);
	let O = (o === s ? -1 : 1) * Math.sqrt(D), j = O * (r * v / i), M = O * -(i * _ / r), F = d * j - p * M + (t + c) / 2, yr = p * j + d * M + (n + u) / 2, br = (_ - j) / r, I = (v - M) / i, L = (-_ - j) / r, R = (-v - M) / i, G = Math.atan2(I, br), q = Math.atan2(R, L) - G;
	s === 0 && q > 0 ? q -= 2 * Math.PI : s === 1 && q < 0 && (q += 2 * Math.PI);
	let Y = G + q;
	e.ellipse(F, yr, r, i, a, G, Y, s === 0);
}
function Vt(e, t, n, r, i, a, o, s) {
	e.save(), e.fillRect(n, r, i, a), s && (e.fillStyle = s, e.strokeStyle = s), e.lineWidth = o, e.lineCap = "square", e.beginPath();
	for (let o of t.d.split(" ")) {
		let t = o[0], s = o.substring(1).split(",");
		if (!s[0] || !s[1]) {
			t === "Z" && e.closePath();
			continue;
		}
		...
		t === "M" ? e.moveTo(c[0], c[1]) : t === "L" && e.lineTo(c[0], c[1]);
	}
	t.type === 1 ? e.stroke() : e.fill(), e.restore();
}
var Xe = /* @__PURE__ */ new Map();
function $t(e, t, n, r, i, a, o = 0) {
	let s = Xe.get(t);
	s || (s = /* @__PURE__ */ new Map(), Xe.set(t, s));
	let c = e.fillStyle;
	if (typeof c != "string") throw Error(`Unexpected fillStyle type "${c}"`);
	let u = s.get(c);
	if (!u) {
		let n = t[0].length, r = t.length, i = wt(n, r), a = A(i.getContext("2d")), o = new ImageData(n, r), d, p, m, h;
		if (c.startsWith("#")) d = parseInt(c.slice(1, 3), 16), p = parseInt(c.slice(3, 5), 16), m = parseInt(c.slice(5, 7), 16), h = c.length > 7 && parseInt(c.slice(7, 9), 16) || 1;
		else if (c.startsWith("rgba")) [d, p, m, h] = c.substring(5, c.length - 1).split(",").map((e) => parseFloat(e));
		else throw Error(`Unexpected fillStyle color format "${c}" when drawing pattern glyph`);
		for (let e = 0; e < r; e++) for (let r = 0; r < n; r++) o.data[(e * n + r) * 4] = d, o.data[(e * n + r) * 4 + 1] = p, o.data[(e * n + r) * 4 + 2] = m, o.data[(e * n + r) * 4 + 3] = t[e][r] * (h * 255);
		a.putImageData(o, 0, 0), u = A(e.createPattern(i, null)), s.set(c, u);
	}
	let d = o >> 1 & 1, p = o & 1;
	d !== 0 || p !== 0 ? u.setTransform(new DOMMatrix().translateSelf(-d, -p)) : u.setTransform(new DOMMatrix()), e.fillStyle = u, e.fillRect(n, r, i, a);
}
function Gt(e, t, n, r, i, a, o, s, c) {
	e.save(), e.beginPath(), e.rect(n, r, i, a), e.clip(), e.beginPath();
	let u;
	u = typeof t == "function" ? t(.15, .15 / a * i) : t;
	let d = {
		currentX: 0,
		currentY: 0,
		lastControlX: 0,
	...
	for (let t of u.split(" ")) {
		let c = t[0];
		if (c === "Z") {
			e.closePath(), d.lastCommand = c;
			continue;
		}
		let u = je[c];
		if (!u) {
			s.error(`Could not find drawing instructions for "${c}"`);
			continue;
		}
		let p = t.substring(1).split(",");
		!p[0] || !p[1] || (u(e, Je(p, i, a, n, r, !0, o, 0, 0, !1), d), d.lastCommand = c);
	}
	c === void 0 ? e.fill() : (e.strokeStyle = e.fillStyle, e.lineWidth = o * c, e.stroke()), e.closePath(), e.restore();
}
function Kt(e, t, n, r, i, a) {
	e.beginPath();
	for (let o of t.split(" ")) {
		...
		let s = o.substring(1).split(",");
		if (!s[0] || !s[1]) continue;
		let c = n + parseFloat(s[0]) * i, u = r + parseFloat(s[1]) * a;
		t === "M" ? e.moveTo(c, u) : t === "L" && e.lineTo(c, u);
	}
	e.clip();
}
function zt(e, t, n, r, i, a, o, s, c) {
	let u = new Path2D();
	u.rect(n, r, i, a), e.clip(u), e.beginPath();
	let d = o / 12;
	e.lineWidth = s * d;
	let p = {
		currentX: 0,
	...
	for (let o of t.d.split(" ")) {
		let u = o[0];
		if (u === "Z") {
			e.closePath(), p.lastCommand = u;
			continue;
		}
		let m = je[u];
		if (!m) {
			c.error(`Could not find drawing instructions for "${u}"`);
			continue;
		}
		let h = o.substring(1).split(",");
		!h[0] || !h[1] || (m(e, Je(h, i, a, n, r, !1, s, (t.leftPadding ?? 0) * (d / 2), (t.rightPadding ?? 0) * (d / 2)), p), p.lastCommand = u);
	}
...
var je = {
	C: (e, t, n) => {
		e.bezierCurveTo(t[0], t[1], t[2], t[3], t[4], t[5]), n.lastControlX = t[2], n.lastControlY = t[3], n.currentX = t[4], n.currentY = t[5];
	},
	L: (e, t, n) => {
		e.lineTo(t[0], t[1]), n.lastControlX = n.currentX = t[0], n.lastControlY = n.currentY = t[1];
	},
	M: (e, t, n) => {
		e.moveTo(t[0], t[1]), n.lastControlX = n.currentX = t[0], n.lastControlY = n.currentY = t[1];
	},
	Q: (e, t, n) => {
		e.quadraticCurveTo(t[0], t[1], t[2], t[3]), n.lastControlX = t[0], n.lastControlY = t[1], n.currentX = t[2], n.currentY = t[3];
	},
	T: (e, t, n) => {
		let r, i;
		n.lastCommand === "Q" || n.lastCommand === "T" ? (r = 2 * n.currentX - n.lastControlX, i = 2 * n.currentY - n.lastControlY) : (r = n.currentX, i = n.currentY), e.quadraticCurveTo(r, i, t[0], t[1]), n.lastControlX = r, n.lastControlY = i, n.currentX = t[0], n.currentY = t[1];
	}
};
function Je(e, t, n, r, i, a, o, s = 0, c = 0, u = !0) {
	let d = e.map((e) => parseFloat(e) || parseInt(e));
	if (d.length < 2) throw Error("Too few arguments for instruction");
	for (let e = 0; e < d.length; e += 2) {
		if (d[e] *= t - s * o - c * o, a && d[e] !== 0) {
			let n = Math.round(d[e] + .5) - .5;
			d[e] = u ? Ye(n, t, 0) : n;
		}
		d[e] += r + s * o;
	}
...
var z0 = class {
	...
	set(e, t, n, r, i) {
		this._data.get(e, t) || this._data.set(e, t, new z0()), this._data.get(e, t).set(n, r, i);
	}
	get(e, t, n, r) {
...
}, et = "requestIdleCallback" in globalThis ? Qe : Ce, s0 = class e {
	constructor() {
		this.fg = 0, this.bg = 0, this.extended = new P0();
	}
	static toColorRGB(e) {
		return [
			e >>> 16 & 255,
	...
	get event() {
		return this._event ||= (t, n, r) => {
			if (this._disposed) return g(() => {});
			let i = {
				fn: t,
				thisArgs: n
			};
			this._listeners = this._listeners.slice(), this._listeners.push(i);
			let a = g(() => {
				let e = this._listeners.indexOf(i);
...
((e) => {
	...
	function i(...e) {
		return (t, n, i) => {
			let a = new R0();
			for (let r of e) a.add(r((e) => t.call(n, e)));
			return i && (Array.isArray(i) ? i.push(a) : i.add(a)), a;
		};
	}
	e.any = i;
	function a(e, t, n) {
...
var tt = {
	...
	constructor(e, t, n, r) {
		this._document = e, this._config = t, this._unicodeService = n, this._logService = r, this._didWarmUp = !1, this._cacheMap = new D0(), this._cacheMapCombined = new D0(), this._pages = [], this._activePages = [], this._workBoundingBox = {
			top: 0,
			left: 0,
			bottom: 0,
			right: 0
		}, this._workAttributeData = new s0(), this._textureSize = 512, this._onAddTextureAtlasCanvas = new U(), this.onAddTextureAtlasCanvas = this._onAddTextureAtlasCanvas.event, this._onRemoveTextureAtlasCanvas = new U(), this.onRemoveTextureAtlasCanvas = this._onRemoveTextureAtlasCanvas.event, this._pageLayoutVersion = 0, this.fontProbeMismatchCount = 0, this._createNewPage(), this._tmpCanvas = it(e, this._config.deviceCellWidth * 4 + B0 * 2, this._config.deviceCellHeight + B0 * 2), this._tmpCtx = A(this._tmpCanvas.getContext("2d", {
	...
	get pageLayoutVersion() {
		return this._pageLayoutVersion;
	}
	clearTexture() {
		this._pages.every((e) => e.glyphs.length === 0 && e.currentRow.x === 0 && e.currentRow.y === 0) || (this._evictAllPages(), this._createNewPage());
	}
	_createNewPage() {
		if (e.maxAtlasPages && this._pages.length >= Math.max(4, e.maxAtlasPages)) {
			let t = this._pages.filter((t) => t.canvas.width * 2 <= (e.maxTextureSize || 4096)).sort((e, t) => t.canvas.width === e.canvas.width ? t.percentageUsed - e.percentageUsed : t.canvas.width - e.canvas.width), n = -1, r = 0;
			for (let e = 0; e < t.length; e++) if (t[e].canvas.width !== r) n = e, r = t[e].canvas.width;
			else if (e - n === 3) break;
			let i = t.slice(n, n + 4);
			if (i.length < 4 || i.some((e) => e.canvas.width !== i[0].canvas.width)) {
				this._evictAllPages();
			...
			let a = i.map((e) => e.glyphs[0].texturePage).sort((e, t) => e - t), o = this.pages.length - i.length, s = this._mergePages(i, o);
			s.version = ++u0.nextVersion;
			for (let e = a.length - 1; e >= 0; e--) this._deletePage(a[e]);
			this.pages.push(s), this._pageLayoutVersion++, this._onAddTextureAtlasCanvas.fire(s.canvas);
		}
		let t = new u0(this._document, this._textureSize);
		return this._pages.push(t), this._activePages.push(t), this._onAddTextureAtlasCanvas.fire(t.canvas), t;
	}
	_mergePages(e, t) {
		let n = e[0].canvas.width * 2, r = new u0(this._document, n, e);
		for (let [i, a] of e.entries()) {
			let e = i * a.canvas.width % n, o = Math.floor(i / 2) * a.canvas.height;
			r.ctx.drawImage(a.canvas, e, o);
			for (let r of a.glyphs) r.texturePage = t, r.sizeClipSpace.x = r.size.x / n, r.sizeClipSpace.y = r.size.y / n, r.texturePosition.x += e, r.texturePosition.y += o, r.texturePositionClipSpace.x = r.texturePosition.x / n, r.texturePositionClipSpace.y = r.texturePosition.y / n;
	...
	_deletePage(e) {
		...
		for (let t = e; t < this._pages.length; t++) {
			...
			e.version = ++u0.nextVersion;
		}
	}
	_evictAllPages() {
		let e = performance.now(), t = this._pages.length;
		for (let e of this._pages) this._onRemoveTextureAtlasCanvas.fire(e.canvas), e.canvas.remove();
		this._pages.length = 0, this._activePages.length = 0, this._overflowSizePage = void 0, this._cacheMap.clear(), this._cacheMapCombined.clear(), this._didWarmUp = !1, this._pageLayoutVersion++, this._logService.debug(`Evicted ${t} WebGL atlas pages in ${(performance.now() - e).toFixed(2)}ms`);
	}
	...
	_getFromCacheMap(e, t, n, r, i, a, o) {
		return w0 = e.get(t, n, r, i), w0 || (w0 = this._drawToCache(t, n, r, i, a, o), e.set(t, n, r, i, w0)), w0;
	}
	...
	_getBackgroundColor(e, t, n, r) {
		...
		switch (e) {
			...
			case 33554432:
				...
				break;
			case 50331648:
				let e = s0.toColorRGB(t);
				i = Z.toColor(e[0], e[1], e[2]);
				break;
			case 0:
			default:
				i = n ? I0.opaque(this._config.colors.foreground) : this._config.colors.background;
				break;
		}
		return this._config.allowTransparency || (i = I0.opaque(i)), i;
	}
	_getForegroundColor(e, t, n, r, i, a, o, s, c, u) {
		let d = this._getMinimumContrastColor(e, t, n, r, i, a, o, c, s, u);
		if (d) return d;
		let p;
		switch (i) {
			case 16777216:
			case 33554432:
				this._config.drawBoldTextInBrightColors && c && a < 8 && (a += 8), p = this._getColorFromAnsiIndex(a);
				break;
			case 50331648:
				let e = s0.toColorRGB(a);
				p = Z.toColor(e[0], e[1], e[2]);
				break;
			case 0:
			default: p = o ? this._config.colors.background : this._config.colors.foreground;
		}
		return this._config.allowTransparency && (p = I0.opaque(p)), s && (p = I0.multiplyOpacity(p, Ke)), p;
	}
	...
	_resolveForegroundRgba(e, t, n, r) {
		switch (e) {
			case 16777216:
			case 33554432: return this._config.drawBoldTextInBrightColors && r && t < 8 && (t += 8), this._getColorFromAnsiIndex(t).rgba;
			case 50331648: return t << 8;
			case 0:
			default: return n ? this._config.colors.background.rgba : this._config.colors.foreground.rgba;
		}
	}
	_getMinimumContrastColor(e, t, n, r, i, a, o, s, c, u) {
		if (this._config.minimumContrastRatio === 1 || u) return;
		let d = this._getContrastCache(c), p = d.getColor(e, r);
		if (p !== void 0) return p ?? void 0;
		let m = this._resolveBackgroundRgba(t, n, o), h = this._resolveForegroundRgba(i, a, o, s), _ = d0.ensureContrastRatio(m, h, this._config.minimumContrastRatio / (c ? 2 : 1));
		if (!_) {
			d.setColor(e, r, null);
			return;
		}
		let v = Z.toColor(_ >> 24 & 255, _ >> 16 & 255, _ >> 8 & 255);
		return d.setColor(e, r, v), v;
	}
	_getContrastCache(e) {
		return e ? this._config.colors.halfContrastCache : this._config.colors.contrastCache;
	}
	_probeRasterizationFontWeight(e) {
		let t = String(e);
		if (!/^(?:[1-8]\d{2}|900)$/.test(t)) return;
		let n = this._tmpCtx.font.match(/(?:^|\s)(normal|bold|[1-9]\d{0,3})(?=\s+\d+(?:\.\d+)?px(?:\s|$))/)?.[1] ?? "400";
		if ((n === "normal" ? "400" : n === "bold" ? "700" : n) !== t) {
			this.fontProbeMismatchCount++, this.fontProbeLastMismatch = {
				desired: t,
				actual: this._tmpCtx.font
			};
			try {
				globalThis.__orcaAtlasFontProbe?.(this.fontProbeLastMismatch);
			} catch {}
		}
	}
	_drawToCache(t, n, r, i, a, o) {
		...
		let c = Math.min(this._config.deviceCellWidth * Math.max(s.length, 2) + B0 * 2, this._config.deviceMaxTextureSize);
		this._tmpCanvas.width < c && (this._tmpCanvas.width = c);
		let u = Math.min(this._config.deviceCellHeight + B0 * 4, this._textureSize);
		if (this._tmpCanvas.height < u && (this._tmpCanvas.height = u), this._tmpCtx.save(), this._workAttributeData.fg = r, this._workAttributeData.bg = n, this._workAttributeData.extended.ext = i, this._workAttributeData.isInvisible()) return tt;
		let d = !!this._workAttributeData.isBold(), p = !!this._workAttributeData.isInverse(), m = !!this._workAttributeData.isDim(), h = !!this._workAttributeData.isItalic(), _ = !!this._workAttributeData.isUnderline(), v = !!this._workAttributeData.isStrikethrough(), y = !!this._workAttributeData.isOverline(), b = this._workAttributeData.getFgColor(), C = this._workAttributeData.getFgColorMode(), T = this._workAttributeData.getBgColor(), E = this._workAttributeData.getBgColorMode();
		if (p) {
			let e = b;
			b = T, T = e;
			let t = C;
		...
		let D = this._getBackgroundColor(E, T, p, m);
		this._tmpCtx.globalCompositeOperation = "copy", this._tmpCtx.fillStyle = D.css, this._tmpCtx.fillRect(0, 0, this._tmpCanvas.width, this._tmpCanvas.height), this._tmpCtx.globalCompositeOperation = "source-over";
		let O = d ? this._config.fontWeightBold : this._config.fontWeight, j = h ? "italic" : "";
		this._tmpCtx.font = `${j} ${O} ${this._config.fontSize * this._config.devicePixelRatio}px ${this._config.fontFamily}`, this._probeRasterizationFontWeight(O), this._tmpCtx.textBaseline = K0;
		let M = s.length === 1 && W0(s.charCodeAt(0)), F = s.length === 1 && ke(s.charCodeAt(0)), I = this._getForegroundColor(n, E, T, r, C, b, p, m, d, V0(s.charCodeAt(0)));
		this._tmpCtx.fillStyle = I.css;
		let L = F ? 0 : B0 * 2, R = !1;
		if (this._config.customGlyphs !== !1) {
			let e = this._workAttributeData.getUnderlineVariantOffset();
			R = qe(this._tmpCtx, s, L, L, this._config.deviceCellWidth, this._config.deviceCellHeight, this._config.deviceCharWidth, this._config.deviceCharHeight, this._config.fontSize, this._config.devicePixelRatio, this._logService, D.css, e);
		}
		let G = !M, Y;
		if (Y = typeof t == "number" ? this._unicodeService.wcwidth(t) : this._unicodeService.getStringCellWidth(t), _) {
			this._tmpCtx.save();
			let e = Math.max(1, Math.floor(this._config.fontSize * this._config.devicePixelRatio / 15)), t = e % 2 == 1 ? .5 : 0;
			...
			else if (this._workAttributeData.isUnderlineColorRGB()) G = !1, this._tmpCtx.strokeStyle = `rgb(${s0.toColorRGB(this._workAttributeData.getUnderlineColor()).join(",")})`;
			else {
				G = !1;
			...
			let n = L, r = Math.ceil(L + this._config.deviceCharHeight) - t - (a ? e * 2 : 0), i = r + e * 2, o = this._workAttributeData.getUnderlineVariantOffset(), c = 0, u = 0;
			for (let t = 0; t < Y; t++) {
				let a = !1;
				this._tmpCtx.save(), c = r, u = i;
				let s = n + t * this._config.deviceCellWidth, d = n + (t + 1) * this._config.deviceCellWidth;
				switch (this._workAttributeData.extended.underlineStyle) {
					case 2:
						this._tmpCtx.moveTo(s, r), this._tmpCtx.lineTo(d, r), this._tmpCtx.moveTo(s, i), this._tmpCtx.lineTo(d, i);
						break;
					case 3:
						c = this._config.deviceCharHeight + 1, u = c + 3 * this._config.devicePixelRatio;
						let t = new Path2D();
						t.rect(s, c, this._config.deviceCellWidth, u - c), this._tmpCtx.clip(t);
						let n = this._config.deviceCellWidth, p = u - c, m = n / 6, h = p / 3;
						for (let e of [
							[
								0,
								2,
							...
							for (let t = 0; t < e.length; t += 2) {
								let n = s + e[t] * m, r = u - e[t + 1] * h;
								t === 0 ? this._tmpCtx.moveTo(n, r) : this._tmpCtx.lineTo(n, r);
							}
						...
						break;
					case 4:
						let _ = o === 0 ? 0 : o >= e ? e * 2 - o : e - o;
						o >= e || _ === 0 ? (this._tmpCtx.setLineDash([Math.round(e), Math.round(e)]), this._tmpCtx.moveTo(s + _, r), this._tmpCtx.lineTo(d, r)) : (this._tmpCtx.setLineDash([Math.round(e), Math.round(e)]), this._tmpCtx.moveTo(s, r), this._tmpCtx.lineTo(s + _, r), this._tmpCtx.moveTo(s + _ + e, r), this._tmpCtx.lineTo(d, r)), o = We(d - s, e, o);
						break;
					case 5:
						let v = d - s, y = Math.floor(.6 * v), b = Math.floor(.3 * v), C = v - y - b;
						this._tmpCtx.setLineDash([
							y,
							b,
							C
						]), this._tmpCtx.moveTo(s, r), this._tmpCtx.lineTo(d, r);
						break;
					case 1:
					default:
						this._tmpCtx.moveTo(s, r), this._tmpCtx.lineTo(d, r);
				...
				a || this._tmpCtx.stroke(), this._tmpCtx.restore();
			}
			if (this._tmpCtx.restore(), !R && this._config.fontSize >= 12 && !this._config.allowTransparency && s !== " ") {
				...
				if (this._tmpCtx.restore(), "actualBoundingBoxDescent" in t && t.actualBoundingBoxDescent > 0) {
					this._tmpCtx.save();
					let t = new Path2D();
					t.rect(n, c - Math.ceil(e / 2), this._config.deviceCellWidth * Y, u - c + Math.ceil(e / 2)), this._tmpCtx.clip(t), this._tmpCtx.lineWidth = this._config.devicePixelRatio * 3, this._tmpCtx.strokeStyle = D.css, this._tmpCtx.strokeText(s, L, L + this._config.deviceCharHeight), this._tmpCtx.restore();
				}
			}
		}
		if (y) {
			let e = Math.max(1, Math.floor(this._config.fontSize * this._config.devicePixelRatio / 15)), t = e % 2 == 1 ? .5 : 0;
			this._tmpCtx.lineWidth = e, this._tmpCtx.strokeStyle = this._tmpCtx.fillStyle, this._tmpCtx.beginPath(), this._tmpCtx.moveTo(L, L + t), this._tmpCtx.lineTo(L + this._config.deviceCharWidth * Y, L + t), this._tmpCtx.stroke();
		}
		if (R || this._tmpCtx.fillText(s, L, L + this._config.deviceCharHeight), s === "_" && !this._config.allowTransparency) {
			let e = fe(this._tmpCtx.getImageData(L, L, this._config.deviceCellWidth, this._config.deviceCellHeight), D, I, G);
			if (e) for (let t = 1; t <= 5 && (this._tmpCtx.save(), this._tmpCtx.fillStyle = D.css, this._tmpCtx.fillRect(0, 0, this._tmpCanvas.width, this._tmpCanvas.height), this._tmpCtx.restore(), this._tmpCtx.fillText(s, L, L + this._config.deviceCharHeight - t), e = fe(this._tmpCtx.getImageData(L, L, this._config.deviceCellWidth, this._config.deviceCellHeight), D, I, G), e); t++);
		}
		if (v) {
			let e = Math.max(1, Math.floor(this._config.fontSize * this._config.devicePixelRatio / 10)), t = this._tmpCtx.lineWidth % 2 == 1 ? .5 : 0;
			this._tmpCtx.lineWidth = e, this._tmpCtx.strokeStyle = this._tmpCtx.fillStyle, this._tmpCtx.beginPath(), this._tmpCtx.moveTo(L, L + Math.floor(this._config.deviceCharHeight / 2) - t), this._tmpCtx.lineTo(L + this._config.deviceCharWidth * Y, L + Math.floor(this._config.deviceCharHeight / 2) - t), this._tmpCtx.stroke();
		}
		this._tmpCtx.restore();
		let xr = this._tmpCtx.getImageData(0, 0, this._tmpCanvas.width, this._tmpCanvas.height), Sr;
		if (Sr = this._config.allowTransparency ? Xt(xr) : fe(xr, D, I, G), Sr) return tt;
		let X = this._findGlyphBoundingBox(xr, this._workBoundingBox, c, F, R, L), Q, $;
		for (;;) {
			if (this._activePages.length === 0) {
				let e = this._createNewPage();
				Q = e, $ = e.currentRow, $.height = X.size.y;
				break;
			}
			Q = this._activePages[this._activePages.length - 1], $ = Q.currentRow;
			for (let e of this._activePages) X.size.y <= e.currentRow.height && (Q = e, $ = e.currentRow);
			for (let e = this._activePages.length - 1; e >= 0; e--) for (let t of this._activePages[e].fixedRows) t.height <= $.height && X.size.y <= t.height && (Q = this._activePages[e], $ = t);
			if (X.size.x > this._textureSize) {
				this._overflowSizePage || (e.maxAtlasPages && this._pages.length >= e.maxAtlasPages && this._evictAllPages(), this._overflowSizePage = new u0(this._document, this._config.deviceMaxTextureSize), this.pages.push(this._overflowSizePage), this._pageLayoutVersion++, this._onAddTextureAtlasCanvas.fire(this._overflowSizePage.canvas)), Q = this._overflowSizePage, $ = this._overflowSizePage.currentRow, $.x + X.size.x >= Q.canvas.width && ($.x = 0, $.y += $.height, $.height = 0);
				break;
			}
			if ($.y + X.size.y >= Q.canvas.height || $.height > X.size.y + 2) {
				let t = !1;
				if (Q.currentRow.y + Q.currentRow.height + X.size.y >= Q.canvas.height) {
					let n;
					for (let e of this._activePages) if (e.currentRow.y + e.currentRow.height + X.size.y < e.canvas.height) {
						n = e;
						break;
					}
					if (n) Q = n;
					else if (e.maxAtlasPages && this._pages.length >= e.maxAtlasPages && $.y + X.size.y <= Q.canvas.height && $.height >= X.size.y && $.x + X.size.x <= Q.canvas.width) t = !0;
					else {
						let e = this._createNewPage();
						Q = e, $ = e.currentRow, $.height = X.size.y, t = !0;
					}
				}
				t || (Q.currentRow.height > 0 && Q.fixedRows.push(Q.currentRow), $ = {
					x: 0,
					y: Q.currentRow.y + Q.currentRow.height,
					height: X.size.y
				}, Q.fixedRows.push($), Q.currentRow = {
					x: 0,
					y: $.y + $.height,
					height: 0
				});
			}
			if ($.x + X.size.x <= Q.canvas.width) break;
			$ === Q.currentRow ? ($.x = 0, $.y += $.height, $.height = 0) : Q.fixedRows.splice(Q.fixedRows.indexOf($), 1);
		}
		return X.texturePage = this._pages.indexOf(Q), X.texturePosition.x = $.x, X.texturePosition.y = $.y, X.texturePositionClipSpace.x = $.x / Q.canvas.width, X.texturePositionClipSpace.y = $.y / Q.canvas.height, X.sizeClipSpace.x /= Q.canvas.width, X.sizeClipSpace.y /= Q.canvas.height, $.height = Math.max($.height, X.size.y), $.x += X.size.x, Q.ctx.putImageData(xr, X.texturePosition.x - this._workBoundingBox.left, X.texturePosition.y - this._workBoundingBox.top, this._workBoundingBox.left, this._workBoundingBox.top, X.size.x, X.size.y), Q.addGlyph(X), Q.version = ++u0.nextVersion, X;
	}
	_findGlyphBoundingBox(e, t, n, r, i, a) {
		t.top = 0;
		let o = r ? this._config.deviceCellHeight : this._tmpCanvas.height, s = r ? this._config.deviceCellWidth : n, c = !1;
		for (let n = 0; n < o; n++) {
			for (let r = 0; r < s; r++) {
		...
		return {
			...
			offset: {
				x: -t.left + a + (r || i ? Math.floor((this._config.deviceCellWidth - this._config.deviceCharWidth) / 2) : 0),
				y: -t.top + a + (r || i ? this._config.lineHeight === 1 ? 0 : Math.round((this._config.deviceCellHeight - this._config.deviceCharHeight) / 2) : 0)
			}
		};
	}
}, H0 = class e {
	constructor(t, n, r) {
		if (this._usedPixels = 0, this._glyphs = [], this.version = ++e.nextVersion, this.currentRow = {
			x: 0,
			y: 0,
			height: 0
		}, this.fixedRows = [], this.canvas = it(t, n, n), this.ctx = A(this.canvas.getContext("2d", { alpha: !0 })), r) if (r.length === 4) this._glyphs = this._glyphs.concat(r[0].glyphs, r[1].glyphs, r[2].glyphs, r[3].glyphs), this._usedPixels = r[0]._usedPixels + r[1]._usedPixels + r[2]._usedPixels + r[3]._usedPixels;
		else for (let e = 0; e < r.length; ++e) this._glyphs = this._glyphs.concat(r[e].glyphs), this._usedPixels += r[e]._usedPixels;
	}
	get percentageUsed() {
	...
	clear() {
		this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height), this.currentRow.x = 0, this.currentRow.y = 0, this.currentRow.height = 0, this.fixedRows.length = 0, this.version = ++e.nextVersion;
	}
};
H0.nextVersion = 0;
var u0 = H0;
function fe(e, t, n, r) {
	let i = t.rgba >>> 24, a = t.rgba >>> 16 & 255, o = t.rgba >>> 8 & 255, s = n.rgba >>> 24, c = n.rgba >>> 16 & 255, u = n.rgba >>> 8 & 255, d = Math.floor((Math.abs(i - s) + Math.abs(a - c) + Math.abs(o - u)) / 12), p = !0;
	for (let t = 0; t < e.data.length; t += 4) e.data[t] === i && e.data[t + 1] === a && e.data[t + 2] === o || r && Math.abs(e.data[t] - i) + Math.abs(e.data[t + 1] - a) + Math.abs(e.data[t + 2] - o) < d ? e.data[t + 3] = 0 : p = !1;
	return p;
}
function Xt(e) {
...
function rt(e, t, n, r, i, a, o, s, c = !0) {
	let u = {
...
function Ee(e, t) {
	for (let n = 0; n < e.colors.ansi.length; n++) if (e.colors.ansi[n].rgba !== t.colors.ansi[n].rgba) return !1;
	return e.devicePixelRatio === t.devicePixelRatio && e.deviceMaxTextureSize === t.deviceMaxTextureSize && e.customGlyphs === t.customGlyphs && e.lineHeight === t.lineHeight && e.letterSpacing === t.letterSpacing && e.fontFamily === t.fontFamily && e.fontSize === t.fontSize && e.fontWeight === t.fontWeight && e.fontWeightBold === t.fontWeightBold && e.allowTransparency === t.allowTransparency && e.deviceCellWidth === t.deviceCellWidth && e.deviceCellHeight === t.deviceCellHeight && e.deviceCharWidth === t.deviceCharWidth && e.deviceCharHeight === t.deviceCharHeight && e.drawBoldTextInBrightColors === t.drawBoldTextInBrightColors && e.minimumContrastRatio === t.minimumContrastRatio && e.colors.foreground.rgba === t.colors.foreground.rgba && e.colors.background.rgba === t.colors.background.rgba;
}
function st(e) {
	return (e & 50331648) == 16777216 || (e & 50331648) == 33554432;
}
var e0 = [];
function Y0(e, t, n, r, i, a, o, s, c, u = !0) {
	let d = rt(r, i, a, o, t, n, s, c, u);
	for (let t = 0; t < e0.length; t++) {
		let n = e0[t], r = n.ownedBy.indexOf(e);
		if (r >= 0) {
			if (Ee(n.config, d)) return n.atlas;
			n.ownedBy.length === 1 ? (n.atlas.dispose(), e0.splice(t, 1)) : n.ownedBy.splice(r, 1);
			break;
		}
	}
	for (let t = 0; t < e0.length; t++) {
		let n = e0[t];
		if (Ee(n.config, d)) return n.ownedBy.push(e), n.atlas;
	}
	let p = e._core, m = p._logService, h = {
		atlas: new J(document, d, p.unicodeService, m),
		config: d,
		ownedBy: [e]
	};
	return e0.push(h), h.atlas;
}
function be(e) {
	for (let t = 0; t < e0.length; t++) {
		let n = e0[t].ownedBy.indexOf(e);
		if (n !== -1) {
			e0[t].ownedBy.length === 1 ? (e0[t].atlas.dispose(), e0.splice(t, 1)) : e0[t].ownedBy.splice(n, 1);
			break;
		}
	}
}
var q0 = class {
	constructor(e, t) {
		this._renderCallback = e, this._coreBrowserService = t, this._isIdlePaused = !1, this.isCursorVisible = !0, this._coreBrowserService.isFocused && (this._restartInterval(), this._resetIdleTimer());
	}
	get isPaused() {
		return !(this._blinkStartTimeout || this._blinkInterval);
	}
	dispose() {
		this._blinkInterval &&= (this._coreBrowserService.window.clearInterval(this._blinkInterval), void 0), this._blinkStartTimeout &&= (this._coreBrowserService.window.clearTimeout(this._blinkStartTimeout), void 0), this._animationFrame &&= (this._coreBrowserService.window.cancelAnimationFrame(this._animationFrame), void 0), this._idleTimeout &&= (this._coreBrowserService.window.clearTimeout(this._idleTimeout), void 0);
	}
	restartBlinkAnimation() {
		this._isIdlePaused && this._resetIdleTimer(), !this.isPaused && (this._animationTimeRestarted = Date.now(), this.isCursorVisible = !0, this._animationFrame ||= this._coreBrowserService.window.requestAnimationFrame(() => {
			this._renderCallback(), this._animationFrame = void 0;
		}));
	}
	_restartInterval(e = 600) {
		this._blinkInterval &&= (this._coreBrowserService.window.clearInterval(this._blinkInterval), void 0), this._blinkStartTimeout = this._coreBrowserService.window.setTimeout(() => {
			if (this._animationTimeRestarted) {
				let e = 600 - (Date.now() - this._animationTimeRestarted);
	...
	pause() {
		this.isCursorVisible = !0, this._isIdlePaused = !1, this._blinkInterval &&= (this._coreBrowserService.window.clearInterval(this._blinkInterval), void 0), this._blinkStartTimeout &&= (this._coreBrowserService.window.clearTimeout(this._blinkStartTimeout), void 0), this._animationFrame &&= (this._coreBrowserService.window.cancelAnimationFrame(this._animationFrame), void 0), this._idleTimeout &&= (this._coreBrowserService.window.clearTimeout(this._idleTimeout), void 0);
	}
	resume() {
		this.pause(), this._animationTimeRestarted = void 0, this._restartInterval(), this._resetIdleTimer(), this.restartBlinkAnimation();
	}
	_resetIdleTimer() {
		this._isIdlePaused = !1, this._idleTimeout && this._coreBrowserService.window.clearTimeout(this._idleTimeout), this._idleTimeout = this._coreBrowserService.window.setTimeout(() => {
			this._stopBlinkingDueToIdle();
		}, 3e5);
	}
	_stopBlinkingDueToIdle() {
		this.isCursorVisible = !0, this._isIdlePaused = !0, this._blinkInterval &&= (this._coreBrowserService.window.clearInterval(this._blinkInterval), void 0), this._blinkStartTimeout &&= (this._coreBrowserService.window.clearTimeout(this._blinkStartTimeout), void 0), this._animationFrame &&= (this._coreBrowserService.window.cancelAnimationFrame(this._animationFrame), void 0), this._coreBrowserService.window.clearTimeout(this._idleTimeout), this._idleTimeout = void 0, this._renderCallback();
	}
};
function ve(t, n, r) {
	let i = new n.ResizeObserver((e) => {
		let n = e.find((e) => e.target === t);
		if (!n) return;
		if (!("devicePixelContentBoxSize" in n)) {
			i?.disconnect(), i = void 0;
			return;
		}
		let a = n.devicePixelContentBoxSize[0].inlineSize, o = n.devicePixelContentBoxSize[0].blockSize;
		a > 0 && o > 0 && r(a, o);
	});
	try {
		i.observe(t, { box: ["device-pixel-content-box"] });
	} catch {
		i.disconnect(), i = void 0;
	}
	return g(() => i?.disconnect());
}
function nt(e) {
	return e > 65535 ? (e -= 65536, String.fromCharCode((e >> 10) + 55296) + String.fromCharCode(e % 1024 + 56320)) : String.fromCharCode(e);
}
var j0 = class e extends s0 {
	constructor() {
		super(...arguments), this.content = 0, this.fg = 0, this.bg = 0, this.extended = new P0(), this.combinedData = "";
	}
	...
	constructor(t, n, r) {
		super(), this._renderCallback = t, this._coreBrowserService = n, this._optionsService = r, this._intervalDuration = 0, this._blinkOn = !0, this._needsBlinkInViewport = !1, this._isViewportVisible = !0, this._register(this._optionsService.onSpecificOptionChange("blinkIntervalDuration", (e) => {
			this.setIntervalDuration(e);
		})), this.setIntervalDuration(this._optionsService.rawOptions.blinkIntervalDuration), this._register(g(() => this._clearInterval()));
	}
	get isBlinkOn() {
...
function ie(e, t, n, r) {
	let i = A(e.createProgram());
	if (e.attachShader(i, A(at(e, e.VERTEX_SHADER, t, r))), e.attachShader(i, A(at(e, e.FRAGMENT_SHADER, n, r))), e.linkProgram(i), e.getProgramParameter(i, e.LINK_STATUS)) return i;
	r.error(e.getProgramInfoLog(i)), e.deleteProgram(i);
}
function at(e, t, n, r) {
	let i = A(e.createShader(t));
	if (e.shaderSource(i, n), e.compileShader(i), e.getShaderParameter(i, e.COMPILE_STATUS)) return i;
	r.error(e.getShaderInfoLog(i)), e.deleteShader(i);
}
function ot(e, t) {
	let n = Math.min(e.length * 2, t), r = new Float32Array(n);
...
function Jt(e) {
	let t = "";
	for (let n = 1; n < e; n++) t += ` else if (v_texpage == ${n}) { outColor = texture(u_texture[${n}], v_texcoord); }`;
...
void main() {
  ...
  } ${t} else { outColor = vec4(0.0, 0.0, 0.0, 0.0); }
}`;
}
var N = 0, S, Ie = 0, k0 = 0, re = class extends W {
	constructor(t, n, r, i, a) {
		super(), this._terminal = t, this._gl = n, this._dimensions = r, this._optionsService = i, this._logService = a, this._lastSeenPageLayoutVersion = -1, this._pageOverflowWarned = !1, this._activeBuffer = 0, this._vertices = {
			count: 0,
			attributes: new Float32Array(),
			attributesBuffers: [new Float32Array(), new Float32Array()]
		};
		let o = this._gl;
		J.maxAtlasPages === void 0 && (J.maxAtlasPages = Math.min(32, A(o.getParameter(o.MAX_TEXTURE_IMAGE_UNITS))), J.maxTextureSize = A(o.getParameter(o.MAX_TEXTURE_SIZE))), this._program = A(ie(o, jt, Jt(J.maxAtlasPages), this._logService)), this._register(g(() => o.deleteProgram(this._program))), this._projectionLocation = A(o.getUniformLocation(this._program, "u_projection")), this._resolutionLocation = A(o.getUniformLocation(this._program, "u_resolution")), this._textureLocation = A(o.getUniformLocation(this._program, "u_texture"));
		let s = this._vertexArrayObject = o.createVertexArray();
		this._register(g(() => o.deleteVertexArray(s))), o.bindVertexArray(s);
		...
		this._register(g(() => o.deleteBuffer(u))), o.bindBuffer(o.ARRAY_BUFFER, u), o.bufferData(o.ARRAY_BUFFER, c, o.STATIC_DRAW), o.enableVertexAttribArray(0), o.vertexAttribPointer(0, 2, this._gl.FLOAT, !1, 0, 0);
		...
		this._register(g(() => o.deleteBuffer(p))), o.bindBuffer(o.ELEMENT_ARRAY_BUFFER, p), o.bufferData(o.ELEMENT_ARRAY_BUFFER, d, o.STATIC_DRAW), this._attributesBuffer = A(o.createBuffer()), this._register(g(() => o.deleteBuffer(this._attributesBuffer))), o.bindBuffer(o.ARRAY_BUFFER, this._attributesBuffer), o.enableVertexAttribArray(2), o.vertexAttribPointer(2, 2, o.FLOAT, !1, 44, 0), o.vertexAttribDivisor(2, 1), o.enableVertexAttribArray(3), o.vertexAttribPointer(3, 2, o.FLOAT, !1, 44, 2 * Float32Array.BYTES_PER_ELEMENT), o.vertexAttribDivisor(3, 1), o.enableVertexAttribArray(4), o.vertexAttribPointer(4, 1, o.FLOAT, !1, 44, 4 * Float32Array.BYTES_PER_ELEMENT), o.vertexAttribDivisor(4, 1), o.enableVertexAttribArray(5), o.vertexAttribPointer(5, 2, o.FLOAT, !1, 44, 5 * Float32Array.BYTES_PER_ELEMENT), o.vertexAttribDivisor(5, 1), o.enableVertexAttribArray(6), o.vertexAttribPointer(6, 2, o.FLOAT, !1, 44, 7 * Float32Array.BYTES_PER_ELEMENT), o.vertexAttribDivisor(6, 1), o.enableVertexAttribArray(1), o.vertexAttribPointer(1, 2, o.FLOAT, !1, 44, 9 * Float32Array.BYTES_PER_ELEMENT), o.vertexAttribDivisor(1, 1), o.useProgram(this._program);
		let m = new Int32Array(J.maxAtlasPages);
		for (let e = 0; e < J.maxAtlasPages; e++) m[e] = e;
		o.uniform1iv(this._textureLocation, m), o.uniformMatrix4fv(this._projectionLocation, !1, te), this._atlasTextures = [];
		for (let t = 0; t < J.maxAtlasPages; t++) {
			let n = new ee(A(o.createTexture()));
			this._register(g(() => o.deleteTexture(n.texture))), o.activeTexture(o.TEXTURE0 + t), o.bindTexture(o.TEXTURE_2D, n.texture), o.texParameteri(o.TEXTURE_2D, o.TEXTURE_WRAP_S, o.CLAMP_TO_EDGE), o.texParameteri(o.TEXTURE_2D, o.TEXTURE_WRAP_T, o.CLAMP_TO_EDGE), o.texImage2D(o.TEXTURE_2D, 0, o.RGBA, 1, 1, 0, o.RGBA, o.UNSIGNED_BYTE, new Uint8Array([
				255,
				0,
				0,
				255
			])), this._atlasTextures[t] = n;
		}
		o.enable(o.BLEND), o.blendFunc(o.SRC_ALPHA, o.ONE_MINUS_SRC_ALPHA), this.handleResize();
	}
	beginFrame() {
		return this._atlas ? this._atlas.pageLayoutVersion === this._lastSeenPageLayoutVersion ? !1 : (this._lastSeenPageLayoutVersion = this._atlas.pageLayoutVersion, !0) : !0;
	}
	updateCell(e, t, n, r, i, a, o, s, c) {
		this._updateCell(this._vertices.attributes, e, t, n, r, i, a, o, s, c);
	}
	_updateCell(e, t, n, r, i, a, o, s, c, u) {
		if (N = (n * this._terminal.cols + t) * 11, r === 0 || r === void 0) {
			e.fill(0, N, N + 11 - 1 - 2);
			return;
		}
		this._atlas && (S = s && s.length > 1 ? this._atlas.getRasterizedGlyphCombinedChar(s, i, a, o, !1, this._terminal.element) : this._atlas.getRasterizedGlyph(r, i, a, o, !1, this._terminal.element), Ie = Math.floor((this._dimensions.device.cell.width - this._dimensions.device.char.width) / 2), i !== u && S.offset.x > Ie ? (k0 = S.offset.x - Ie, e[N] = -(S.offset.x - k0) + this._dimensions.device.char.left, e[N + 1] = -S.offset.y + this._dimensions.device.char.top, e[N + 2] = (S.size.x - k0) / this._dimensions.device.canvas.width, e[N + 3] = S.size.y / this._dimensions.device.canvas.height, e[N + 4] = S.texturePage, e[N + 5] = S.texturePositionClipSpace.x + k0 / this._atlas.pages[S.texturePage].canvas.width, e[N + 6] = S.texturePositionClipSpace.y, e[N + 7] = S.sizeClipSpace.x - k0 / this._atlas.pages[S.texturePage].canvas.width, e[N + 8] = S.sizeClipSpace.y) : (e[N] = -S.offset.x + this._dimensions.device.char.left, e[N + 1] = -S.offset.y + this._dimensions.device.char.top, e[N + 2] = S.size.x / this._dimensions.device.canvas.width, e[N + 3] = S.size.y / this._dimensions.device.canvas.height, e[N + 4] = S.texturePage, e[N + 5] = S.texturePositionClipSpace.x, e[N + 6] = S.texturePositionClipSpace.y, e[N + 7] = S.sizeClipSpace.x, e[N + 8] = S.sizeClipSpace.y), this._optionsService.rawOptions.rescaleOverlappingGlyphs && Ze(r, c, S.size.x, this._dimensions.device.cell.width) && (e[N + 2] = (this._dimensions.device.cell.width - 1) / this._dimensions.device.canvas.width));
	}
	clear() {
		let e = this._terminal, t = e.cols * e.rows * 11;
		this._vertices.count === t ? this._vertices.attributes.fill(0) : this._vertices.attributes = new Float32Array(t);
		let n = 0;
		for (; n < this._vertices.attributesBuffers.length; n++) this._vertices.count === t ? this._vertices.attributesBuffers[n].fill(0) : this._vertices.attributesBuffers[n] = new Float32Array(t);
		this._vertices.count = t, n = 0;
		for (let t = 0; t < e.rows; t++) for (let r = 0; r < e.cols; r++) this._vertices.attributes[n + 9] = r / e.cols, this._vertices.attributes[n + 10] = t / e.rows, n += 11;
	}
	handleResize() {
		let e = this._gl;
		e.useProgram(this._program), e.viewport(0, 0, this._dimensions.device.canvas.width, this._dimensions.device.canvas.height), e.uniform2f(this._resolutionLocation, this._dimensions.device.canvas.width, this._dimensions.device.canvas.height), this.clear();
	}
	render(e) {
		...
		let n = this._vertices.attributesBuffers[this._activeBuffer], r = 0;
		for (let t = 0; t < e.lineLengths.length; t++) {
			let i = t * this._terminal.cols * 11, a = this._vertices.attributes.subarray(i, i + e.lineLengths[t] * 11);
			n.set(a, r), r += a.length;
		}
		t.bindBuffer(t.ARRAY_BUFFER, this._attributesBuffer), t.bufferData(t.ARRAY_BUFFER, n.subarray(0, r), t.STREAM_DRAW);
		let i = Math.min(this._atlas.pages.length, this._atlasTextures.length);
		this._atlas.pages.length > this._atlasTextures.length && !this._pageOverflowWarned && (this._pageOverflowWarned = !0, this._logService.warn(`Atlas page count (${this._atlas.pages.length}) exceeds the renderer's texture capacity (${this._atlasTextures.length}), some glyphs will not render correctly`));
		for (let e = 0; e < i; e++) this._atlas.pages[e].version !== this._atlasTextures[e].version && this._bindAtlasPageTexture(t, this._atlas, e);
		t.drawElementsInstanced(t.TRIANGLE_STRIP, 4, t.UNSIGNED_BYTE, 0, r / 11);
	}
	setAtlas(e) {
		this._atlas = e, this._lastSeenPageLayoutVersion = -1, this.invalidateAtlasTextures();
	}
	invalidateAtlasTextures() {
		for (let e of this._atlasTextures) e.version = -1;
	}
	_bindAtlasPageTexture(e, t, n) {
		e.activeTexture(e.TEXTURE0 + n), e.bindTexture(e.TEXTURE_2D, this._atlasTextures[n].texture), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_S, e.CLAMP_TO_EDGE), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_T, e.CLAMP_TO_EDGE), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MIN_FILTER, e.LINEAR), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MAG_FILTER, e.LINEAR), e.texImage2D(e.TEXTURE_2D, 0, e.RGBA, e.RGBA, e.UNSIGNED_BYTE, t.pages[n].canvas), this._atlasTextures[n].version = t.pages[n].version;
	}
	setDimensions(e) {
		this._dimensions = e;
	}
...
var lt = 2147483648, se = class {
	constructor() {
		this.cells = new Uint32Array(), this.lineLengths = new Uint32Array(), this.selection = Tt();
	}
	resize(e, t) {
		let n = e * t * 4;
...
}, t1 = "#version 300 es\nlayout (location = 0) in vec2 a_position;\nlayout (location = 1) in vec2 a_size;\nlayout (location = 2) in vec4 a_color;\nlayout (location = 3) in vec2 a_unitquad;\n\nuniform mat4 u_projection;\n\nout vec4 v_color;\n\nvoid main() {\n  vec2 zeroToOne = a_position + (a_unitquad * a_size);\n  gl_Position = u_projection * vec4(zeroToOne, 0.0, 1.0);\n  v_color = a_color;\n}", i1 = "#version 300 es\nprecision lowp float;\n\nin vec4 v_color;\n\nout vec4 outColor;\n\nvoid main() {\n  outColor = v_color;\n}", l0 = 8, ge = l0 * Float32Array.BYTES_PER_ELEMENT, r1 = 20 * l0, ne = class {
	constructor() {
		this.attributes = new Float32Array(r1), this.count = 0;
	}
}, p0 = 0, Lt = 0, ht = 0, dt = 0, _t = 0, ct = 0, ut = 0, ae = class extends W {
	constructor(t, n, r, i, a) {
		super(), this._terminal = t, this._gl = n, this._dimensions = r, this._themeService = i, this._logService = a, this._vertices = new ne(), this._verticesCursor = new ne();
		let o = this._gl;
		this._program = A(ie(o, t1, i1, this._logService)), this._register(g(() => o.deleteProgram(this._program))), this._projectionLocation = A(o.getUniformLocation(this._program, "u_projection"));
		let s = this._vertexArrayObject = o.createVertexArray();
		this._register(g(() => o.deleteVertexArray(s))), o.bindVertexArray(s);
		let c = new Float32Array([
			0,
		...
		this._register(g(() => o.deleteBuffer(u))), o.bindBuffer(o.ARRAY_BUFFER, u), o.bufferData(o.ARRAY_BUFFER, c, o.STATIC_DRAW), o.enableVertexAttribArray(3), o.vertexAttribPointer(3, 2, this._gl.FLOAT, !1, 0, 0);
		let d = new Uint8Array([
			0,
			1,
			2,
		...
		this._register(g(() => o.deleteBuffer(p))), o.bindBuffer(o.ELEMENT_ARRAY_BUFFER, p), o.bufferData(o.ELEMENT_ARRAY_BUFFER, d, o.STATIC_DRAW), this._attributesBuffer = A(o.createBuffer()), this._register(g(() => o.deleteBuffer(this._attributesBuffer))), o.bindBuffer(o.ARRAY_BUFFER, this._attributesBuffer), o.enableVertexAttribArray(0), o.vertexAttribPointer(0, 2, o.FLOAT, !1, ge, 0), o.vertexAttribDivisor(0, 1), o.enableVertexAttribArray(1), o.vertexAttribPointer(1, 2, o.FLOAT, !1, ge, 2 * Float32Array.BYTES_PER_ELEMENT), o.vertexAttribDivisor(1, 1), o.enableVertexAttribArray(2), o.vertexAttribPointer(2, 4, o.FLOAT, !1, ge, 4 * Float32Array.BYTES_PER_ELEMENT), o.vertexAttribDivisor(2, 1), this._updateCachedColors(i.colors), this._register(this._themeService.onChangeColors((e) => {
	...
	_renderVertices(e) {
		let t = this._gl;
		t.useProgram(this._program), t.bindVertexArray(this._vertexArrayObject), t.uniformMatrix4fv(this._projectionLocation, !1, te), t.bindBuffer(t.ARRAY_BUFFER, this._attributesBuffer), t.bufferData(t.ARRAY_BUFFER, e.attributes, t.DYNAMIC_DRAW), t.drawElementsInstanced(this._gl.TRIANGLE_STRIP, 4, t.UNSIGNED_BYTE, 0, e.count);
	}
	handleResize() {
	...
	updateBackgrounds(e) {
		let t = this._terminal, n = this._vertices, r = 1, i, a, o, s, c, u, d, p, m, h, _;
		for (i = 0; i < t.rows; i++) {
			for (o = -1, s = 0, c = 0, u = !1, a = 0; a < t.cols; a++) d = (i * t.cols + a) * 4, p = e.cells[d + 1], m = e.cells[d + 2], h = !!(m & 67108864), (p !== s || m !== c && (u || h)) && ((s !== 0 || u && c !== 0) && (_ = r++ * l0, this._updateRectangle(n, _, c, s, o, a, i)), o = a, s = p, c = m, u = h);
			(s !== 0 || u && c !== 0) && (_ = r++ * l0, this._updateRectangle(n, _, c, s, o, t.cols, i));
		}
		n.count = r;
	}
	updateCursor(e) {
		let t = this._verticesCursor, n = e.cursor;
		if (!n || n.style === "block") {
			t.count = 0;
			return;
		}
		let r, i = 0;
		(n.style === "bar" || n.style === "outline") && (r = i++ * l0, this._addRectangleFloat(t.attributes, r, n.x * this._dimensions.device.cell.width, n.y * this._dimensions.device.cell.height, n.style === "bar" ? n.dpr * n.cursorWidth : n.dpr, this._dimensions.device.cell.height, this._cursorFloat)), (n.style === "underline" || n.style === "outline") && (r = i++ * l0, this._addRectangleFloat(t.attributes, r, n.x * this._dimensions.device.cell.width, (n.y + 1) * this._dimensions.device.cell.height - n.dpr, n.width * this._dimensions.device.cell.width, n.dpr, this._cursorFloat)), n.style === "outline" && (r = i++ * l0, this._addRectangleFloat(t.attributes, r, n.x * this._dimensions.device.cell.width, n.y * this._dimensions.device.cell.height, n.width * this._dimensions.device.cell.width, n.dpr, this._cursorFloat), r = i++ * l0, this._addRectangleFloat(t.attributes, r, (n.x + n.width) * this._dimensions.device.cell.width - n.dpr, n.y * this._dimensions.device.cell.height, n.dpr, this._dimensions.device.cell.height, this._cursorFloat)), t.count = i;
	}
	_updateRectangle(e, t, n, r, i, a, o) {
		if (n & 67108864) switch (n & 50331648) {
			case 16777216:
			case 33554432:
				p0 = this._themeService.colors.ansi[n & 255].rgba;
				break;
			case 50331648:
				p0 = (n & 16777215) << 8;
				break;
			...
			case 16777216:
			case 33554432:
				p0 = this._themeService.colors.ansi[r & 255].rgba;
				break;
			case 50331648:
				p0 = (r & 16777215) << 8;
				break;
		...
		e.attributes.length < t + l0 && (e.attributes = ot(e.attributes, (this._terminal.rows * this._terminal.cols + 1) * l0)), Lt = i * this._dimensions.device.cell.width, ht = o * this._dimensions.device.cell.height, dt = (p0 >> 24 & 255) / 255, _t = (p0 >> 16 & 255) / 255, ct = (p0 >> 8 & 255) / 255, ut = 1, this._addRectangle(e.attributes, t, Lt, ht, (a - i) * this._dimensions.device.cell.width, this._dimensions.device.cell.height, dt, _t, ct, ut);
	}
	_addRectangle(e, t, n, r, i, a, o, s, c, u) {
		e[t] = n / this._dimensions.device.canvas.width, e[t + 1] = r / this._dimensions.device.canvas.height, e[t + 2] = i / this._dimensions.device.canvas.width, e[t + 3] = a / this._dimensions.device.canvas.height, e[t + 4] = o, e[t + 5] = s, e[t + 6] = c, e[t + 7] = u;
	}
	_addRectangleFloat(e, t, n, r, i, a, o) {
		e[t] = n / this._dimensions.device.canvas.width, e[t + 1] = r / this._dimensions.device.canvas.height, e[t + 2] = i / this._dimensions.device.canvas.width, e[t + 3] = a / this._dimensions.device.canvas.height, e[t + 4] = o[0], e[t + 5] = o[1], e[t + 6] = o[2], e[t + 7] = o[3];
	}
	...
	constructor(t, n, r, i, a, o, s, c) {
		super(), this._container = n, this._alpha = a, this._coreBrowserService = o, this._optionsService = s, this._themeService = c, this._deviceCharWidth = 0, this._deviceCharHeight = 0, this._deviceCellWidth = 0, this._deviceCellHeight = 0, this._deviceCharLeft = 0, this._deviceCharTop = 0, this._canvas = this._coreBrowserService.mainDocument.createElement("canvas"), this._canvas.classList.add(`xterm-${r}-layer`), this._canvas.style.zIndex = i.toString(), this._initCanvas(), this._container.appendChild(this._canvas), this._register(this._themeService.onChangeColors((e) => {
			this._refreshCharAtlas(t, e), this.reset(t);
		})), this._register(g(() => {
			this._canvas.remove();
		}));
	}
	_initCanvas() {
		this._ctx = A(this._canvas.getContext("2d", { alpha: this._alpha })), this._alpha || this._clearAll();
	}
	handleBlur(e) {}
	handleFocus(e) {}
	handleCursorMove(e) {}
	...
	_setTransparency(e, t) {
		...
		let n = this._canvas;
		this._alpha = t, this._canvas = this._canvas.cloneNode(), this._initCanvas(), this._container.replaceChild(this._canvas, n), this._refreshCharAtlas(e, this._themeService.colors), this.handleGridChanged(e, 0, e.rows - 1);
	}
	_refreshCharAtlas(e, t) {
		this._deviceCharWidth <= 0 && this._deviceCharHeight <= 0 || (this._charAtlas = Y0(e, this._optionsService.rawOptions, t, this._deviceCellWidth, this._deviceCellHeight, this._deviceCharWidth, this._deviceCharHeight, this._coreBrowserService.dpr, 2048), this._charAtlas.warmUp());
	}
	resize(e, t) {
	...
	_fillBottomLineAtCells(e, t, n = 1) {
		this._ctx.fillRect(e * this._deviceCellWidth, (t + 1) * this._deviceCellHeight - this._coreBrowserService.dpr - 1, n * this._deviceCellWidth, this._coreBrowserService.dpr);
	}
	_clearAll() {
	...
	_clearCells(e, t, n, r) {
		this._alpha ? this._ctx.clearRect(e * this._deviceCellWidth, t * this._deviceCellHeight, n * this._deviceCellWidth, r * this._deviceCellHeight) : (this._ctx.fillStyle = this._themeService.colors.background.css, this._ctx.fillRect(e * this._deviceCellWidth, t * this._deviceCellHeight, n * this._deviceCellWidth, r * this._deviceCellHeight));
	}
	_fillCharTrueColor(e, t, n, r) {
		this._ctx.font = this._getFont(e, !1, !1), this._ctx.textBaseline = K0, this._clipCell(n, r, t.getWidth()), this._ctx.fillText(t.getChars(), n * this._deviceCellWidth + this._deviceCharLeft, r * this._deviceCellHeight + this._deviceCharTop + this._deviceCharHeight);
	}
	_clipCell(e, t, n) {
		this._ctx.beginPath(), this._ctx.rect(e * this._deviceCellWidth, t * this._deviceCellHeight, n * this._deviceCellWidth, this._deviceCellHeight), this._ctx.clip();
	}
	_getFont(e, t, n) {
	...
	constructor(e, t, n, r, i, a, o) {
		super(n, e, "link", t, !0, i, a, o), this._register(r.onShowLinkUnderline((e) => this._handleShowLinkUnderline(e))), this._register(r.onHideLinkUnderline((e) => this._handleHideLinkUnderline(e)));
	}
	resize(e, t) {
...
var Le = class extends W {
	constructor(t, n, r, i, o, s, c, u, d, p = !0, m) {
		...
		})), this._observerDisposable.value = ve(this._canvas, this._coreBrowserService.window, (e, t) => this._setCanvasDevicePixelDimensions(e, t)), this._register(this._coreBrowserService.onWindowChange((e) => {
			this._observerDisposable.value = ve(this._canvas, e, (e, t) => this._setCanvasDevicePixelDimensions(e, t));
		})), this._register(le(this._coreBrowserService.mainDocument, "mousedown", () => this._cursorBlinkStateManager.value?.restartBlinkAnimation())), this._core.screenElement.appendChild(this._canvas), [this._rectangleRenderer.value, this._glyphRenderer.value] = this._initializeWebGLState(), this._isAttached = this._core.screenElement.isConnected, this._register(g(() => {
	...
	handleResize(e, t) {
		...
		this._canvas.width = this.dimensions.device.canvas.width, this._canvas.height = this.dimensions.device.canvas.height, this._canvas.style.width = `${this.dimensions.css.canvas.width}px`, this._canvas.style.height = `${this.dimensions.css.canvas.height}px`, this._core.screenElement.style.width = `${this.dimensions.css.canvas.width}px`, this._core.screenElement.style.height = `${this.dimensions.css.canvas.height}px`, this._rectangleRenderer.value?.setDimensions(this.dimensions), this._rectangleRenderer.value?.handleResize(), this._glyphRenderer.value?.setDimensions(this.dimensions), this._glyphRenderer.value?.handleResize(), this._refreshCharAtlas(), this._clearModel(!1), this._onRequestRedraw.fire({
			start: 0,
	...
	_refreshCharAtlas() {
		...
		let e = Y0(this._terminal, this._optionsService.rawOptions, this._themeService.colors, this.dimensions.device.cell.width, this.dimensions.device.cell.height, this.dimensions.device.char.width, this.dimensions.device.char.height, this._coreBrowserService.dpr, this._deviceMaxTextureSize, this._customGlyphs);
		this._charAtlas !== e && (this._onChangeTextureAtlas.fire(e.pages[0].canvas), this._charAtlasDisposable.value = De(c0.forward(e.onAddTextureAtlasCanvas, this._onAddTextureAtlasCanvas), c0.forward(e.onRemoveTextureAtlasCanvas, this._onRemoveTextureAtlasCanvas))), this._charAtlas = e, this._charAtlas.warmUp(), this._glyphRenderer.value?.setAtlas(this._charAtlas);
	}
	_clearModel(e) {
	...
	clear() {
		...
		this._resetBlinkingRowState(), this._textBlinkStateManager.setNeedsBlinkInViewport(!1), this._cursorBlinkStateManager.value?.restartBlinkAnimation(), this._updateCursorBlink();
	}
	renderRows(e, t) {
		...
		let n = !1, r = 0;
		for (; this._charAtlas && r++ < 32 && this._glyphRenderer.value.beginFrame();) n = !0, this._clearModel(!0), this._updateModel(0, this._terminal.rows - 1);
		n && this._glyphRenderer.value.invalidateAtlasTextures(), this._rectangleRenderer.value.renderBackgrounds(), this._glyphRenderer.value.render(this._model), (!this._cursorBlinkStateManager.value || this._cursorBlinkStateManager.value.isCursorVisible) && this._rectangleRenderer.value.renderCursor();
	}
	_updateCursorBlink() {
	...
	_updateModel(e, t) {
		let n = this._core, r = this._workCell, i, a, o, s, c, u, d, p, m, h, _, v, y, b, C, T, E;
		e = pt(e, n.rows - 1, 0), t = pt(t, n.rows - 1, 0);
		let D = this._coreService.decPrivateModes.cursorStyle ?? n.options.cursorStyle ?? "block", O = this._terminal.buffer.active.baseY + this._terminal.buffer.active.cursorY, j = O - n.buffer.ydisp, M = Math.min(this._terminal.buffer.active.cursorX, n.cols - 1), F = -1, yr = this._coreService.isCursorInitialized && !this._coreService.isCursorHidden && (!this._cursorBlinkStateManager.value || this._cursorBlinkStateManager.value.isCursorVisible);
		this._model.cursor = void 0;
		let br = !1;
		for (a = e; a <= t; a++) {
			o = a + n.buffer.ydisp;
			let e = n.buffer.lines.get(o);
			if (!e) {
				for (this._model.lineLengths[a] = 0, T = 0; T < n.cols; T++) E = (a * n.cols + T) * 4, br = !0, this._nullModelCell(T, a, E, 0, 0, 0);
				this._setRowBlinkState(a, !1);
				continue;
			}
			s = e;
			let t = !1;
			for (this._model.lineLengths[a] = 0, _ = O === o, d = 0, c = this._characterJoinerService.getJoinedCharacters(o), T = 0; T < n.cols; T++) {
				if (i = this._cellColorResolver.result.bg, s.loadCell(T, r), T === 0 && (i = this._cellColorResolver.result.bg), u = !1, p = T >= d, m = T, c.length > 0 && T === c[0][0] && p) {
					h = c.shift();
					let e = this._model.selection.isCellSelected(this._terminal, h[0], o);
					for (C = h[0] + 1; C < h[1]; C++) p &&= e === this._model.selection.isCellSelected(this._terminal, C, o);
					p &&= !_ || M < h[0] || M >= h[1], p ? (u = !0, r = new Ae(r, s.translateToString(!0, h[0], h[1]), h[1] - h[0]), m = h[1] - 1) : d = h[1];
				}
				if (v = r.getChars(), y = r.getCode(), C = (a * n.cols + T) * 4, !t && r.isBlink() && (t = !0), this._cellColorResolver.resolve(r, T, o, this.dimensions.device.cell.width, this.dimensions.device.cell.height), yr && o === O && (T === M && (this._model.cursor = {
					x: M,
					y: j,
					width: r.getWidth(),
					style: this._coreBrowserService.isFocused ? D : n.options.cursorInactiveStyle,
					cursorWidth: n.options.cursorWidth,
					dpr: this._devicePixelRatio
				}, F = M + r.getWidth() - 1), T >= M && T <= F && (this._coreBrowserService.isFocused && D === "block" || this._coreBrowserService.isFocused === !1 && n.options.cursorInactiveStyle === "block") && (this._cellColorResolver.result.fg = 50331648 | this._themeService.colors.cursorAccent.rgba >> 8 & 16777215, this._cellColorResolver.result.bg = 50331648 | this._themeService.colors.cursor.rgba >> 8 & 16777215)), this._textBlinkStateManager.isEnabled && !this._textBlinkStateManager.isBlinkOn && r.isBlink() && (this._cellColorResolver.result.fg |= 1073741824), y !== 0 && (this._model.lineLengths[a] = T + 1), !(this._model.cells[C] === y && this._model.cells[C + 1] === this._cellColorResolver.result.bg && this._model.cells[C + 2] === this._cellColorResolver.result.fg && this._model.cells[C + 3] === this._cellColorResolver.result.ext) && (br = !0, v.length > 1 && (y |= lt), this._model.cells[C] = y, this._model.cells[C + 1] = this._cellColorResolver.result.bg, this._model.cells[C + 2] = this._cellColorResolver.result.fg, this._model.cells[C + 3] = this._cellColorResolver.result.ext, b = r.getWidth(), this._glyphRenderer.value.updateCell(T, a, y, this._cellColorResolver.result.bg, this._cellColorResolver.result.fg, this._cellColorResolver.result.ext, v, b, i), u)) {
					for (r = this._workCell, T++; T <= m; T++) E = (a * n.cols + T) * 4, this._nullModelCell(T, a, E, this._cellColorResolver.result.bg, this._cellColorResolver.result.fg, this._cellColorResolver.result.ext);
					T--;
				}
			}
			this._setRowBlinkState(a, t);
		}
		br && this._rectangleRenderer.value.updateBackgrounds(this._model), this._rectangleRenderer.value.updateCursor(this._model), this._updateTextBlinkState();
	}
	_nullModelCell(e, t, n, r, i, a) {
		this._glyphRenderer.value.updateCell(e, t, 0, r, i, a, "", 0, 0), this._model.cells[n] = 0, this._model.cells[n + 1] = r, this._model.cells[n + 2] = i, this._model.cells[n + 3] = a;
	}
	_resetBlinkingRowState() {
		this._rowHasBlinkingCells = Array(this._terminal.rows).fill(!1), this._rowHasBlinkingCellsCount = 0;
	}
	...
	_setCanvasDevicePixelDimensions(e, t) {
		this._canvas.width === e && this._canvas.height === t || (this._canvas.width = e, this._canvas.height = t, this._onRequestRedraw.fire({
			start: 0,
			end: this._terminal.rows - 1,
			sync: !0
		}));
...
var Ct = class extends W {
	...
	activate(t) {
		...
		this._terminal = t;
		let r = n.coreService, i = n.optionsService, a = n, o = a._renderService, s = a._characterJoinerService, c = a._charSizeService, u = a._coreBrowserService, d = a._decorationService, p = a._logService, m = a._themeService;
		this._renderer = this._register(new Le(t, s, c, u, r, d, p, i, m, this._customGlyphs, this._preserveDrawingBuffer)), this._register(c0.forward(this._renderer.onContextLoss, this._onContextLoss)), this._register(c0.forward(this._renderer.onChangeTextureAtlas, this._onChangeTextureAtlas)), this._register(c0.forward(this._renderer.onAddTextureAtlasCanvas, this._onAddTextureAtlasCanvas)), this._register(c0.forward(this._renderer.onRemoveTextureAtlasCanvas, this._onRemoveTextureAtlasCanvas)), o.setRenderer(this._renderer), this._register(g(() => {
			if (this._terminal._core._store._isDisposed) return;
			let e = this._terminal._core._renderService;
			e.setRenderer(this._terminal._core._createRenderer()), e.handleResize(t.cols, t.rows);
		}));
	}
	get textureAtlas() {
