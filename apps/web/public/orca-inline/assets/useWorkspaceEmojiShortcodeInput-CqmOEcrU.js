...
function WorkspaceEmojiSuggestionPopover({ anchorRef: e, commandValue: h, contentClassName: _, heading: T, onCommandValueChange: D, onOpenChange: O, onSelect: k, open: A, portalContainer: j, side: M = "top", suggestions: N }) {
	let P = (0, import_react.useRef)(null);
	return (0, import_react.useEffect)(() => {
		if (!A || !h) return;
		let e = Array.from(P.current?.querySelectorAll("[cmdk-item=\"\"]") ?? []).find((e) => e.getAttribute("data-value") === h);
		e && (e.parentElement?.firstElementChild === e && e.closest("[cmdk-group=\"\"]")?.querySelector("[cmdk-group-heading=\"\"]")?.scrollIntoView({ block: "nearest" }), e.scrollIntoView({ block: "nearest" }));
	}, [
		h,
		A,
		N
		...
		open: A,
		onOpenChange: O,
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PopoverAnchor, { virtualRef: e }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PopoverContent, {
			...
			avoidCollisions: !1,
			portalContainer: j,
			className: cn("popover-scroll-content flex max-h-56 w-[var(--radix-popover-trigger-width)] flex-col p-0", _),
			onOpenAutoFocus: (e) => e.preventDefault(),
			onPointerDown: (e) => e.preventDefault(),
			onPointerDownOutside: (h) => {
				e.current?.contains(h.target) && h.preventDefault();
			},
			onFocusOutside: (h) => {
				e.current?.contains(h.target) && h.preventDefault();
			},
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Command, {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CommandList, {
					...
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CommandGroup, {
						heading: T,
						className: "p-1",
						children: N.map((e) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(CommandItem, {
							value: `emoji:${e.shortcode}`,
							onSelect: () => k(e),
...
function setEmojiShortcodeDatasetLoader(e) {
	loadDataset = e;
}
var SKIN_TONE_SHORTCODE = /_tone\d(?:-\d)?$/, catalog = null;
function loadCatalog() {
	if (catalog) return catalog;
	if (!loadDataset) throw Error("Emoji shortcode dataset loader was never registered");
	let e = Object.entries(loadDataset()).flatMap(([e, h]) => {
		let g = (typeof h == "string" ? [h] : h).filter((e) => !SKIN_TONE_SHORTCODE.test(e));
		return g.length > 0 ? [{
			emoji: hexcodeToEmoji(e),
			shortcodes: g
		}] : [];
	});
	return catalog = {
		entries: e.flatMap(({ emoji: e, shortcodes: h }) => h.map((h) => ({
			emoji: e,
			shortcode: h
		}))),
		primaryShortcodeByEmoji: new Map(e.map(({ emoji: e, shortcodes: h }) => [normalizeEmojiLookup(e), primaryShortcode(h)])),
		segmenter: new Intl.Segmenter("en", { granularity: "grapheme" })
	}, catalog;
}
function getStandardEmojiShortcodeEntries() {
	return loadCatalog().entries;
}
function primaryShortcode(e) {
	let h = e.filter((e) => /^[a-z]/i.test(e));
	return h.find((e) => e.length >= 3 && !e.startsWith("flag_")) ?? h.find((e) => e.length >= 3) ?? h[0] ?? e[0];
}
function normalizeEmojiLookup(e) {
	return Array.from(e).filter((e) => {
		let h = e.codePointAt(0);
		return e !== "️" && (h === void 0 || h < 127995 || h > 127999);
	}).join("");
}
function hexcodeToEmoji(e) {
	return e.split("-").map((e) => String.fromCodePoint(Number.parseInt(e, 16))).join("");
}
setEmojiShortcodeDatasetLoader(() => emojibase_default);
var exactShortcode = null;
function exactShortcodeIndex() {
	return exactShortcode ??= new Map(getStandardEmojiShortcodeEntries().map(({ emoji: e, shortcode: h }) => [h, {
		emoji: e,
		shortcode: h
	}])), exactShortcode;
}
...
function matchTier(e, h) {
	let g = e.indexOf(h);
	return g === -1 ? null : g === 0 ? e.length === h.length ? MATCH_TIER.exact : MATCH_TIER.prefix : /[_-]/.test(e[g - 1]) ? MATCH_TIER.wordStart : MATCH_TIER.substring;
}
function searchWorkspaceEmojiShortcodes(e, h = 8) {
	let g = e.trim().toLowerCase();
	if (!g || h <= 0) return [];
	let _ = getStandardEmojiShortcodeEntries().flatMap((e) => {
		let h = matchTier(e.shortcode, g);
		return h === null ? [] : [{
			...e,
			tier: h
		}];
	}).sort((e, h) => e.tier - h.tier || e.shortcode.length - h.shortcode.length || e.shortcode.localeCompare(h.shortcode)), v = /* @__PURE__ */ new Set(), y = [];
	for (let { emoji: e, shortcode: g } of _) if (!v.has(e) && (v.add(e), y.push({
		emoji: e,
		shortcode: g
	}), y.length === h)) break;
	return y;
}
function getActiveWorkspaceEmojiShortcode(e, h) {
	if (h === null || h < 0 || h > e.length) return null;
	let g = e.slice(0, h).match(/(^|\s):([a-z0-9_+-]{1,40})$/i);
	return g ? {
		start: h - g[2].length - 1,
		end: h,
		query: g[2].toLowerCase()
	} : null;
}
function replaceCompletedWorkspaceEmojiShortcode(e, h) {
	if (h === null || h < 0 || h > e.length) return null;
	let g = e.slice(0, h).match(/(^|\s):([a-z0-9_+-]{1,40}):$/i);
	if (!g) return null;
	let _ = exactShortcodeIndex().get(g[2].toLowerCase());
	return _ ? replaceWorkspaceEmojiRange(e, h - g[2].length - 2, h, _.emoji) : null;
}
function applyWorkspaceEmojiSuggestion(e, h, g) {
	return replaceWorkspaceEmojiRange(e, h.start, h.end, g.emoji, !0);
}
function replaceWorkspaceEmojiRange(e, h, g, _, v = !1) {
	let y = /\s/.test(e[g] ?? ""), b = v && !y ? " " : "";
	return {
		value: `${e.slice(0, h)}${_}${b}${e.slice(g)}`,
		cursor: h + _.length + (v ? 1 : 0)
	};
}
function useWorkspaceEmojiShortcodeInput({ disabled: e = !1, inputRef: h, onValueChange: g, value: _ }) {
	let [v, y] = (0, import_react.useState)(null), [b, x] = (0, import_react.useState)(""), S = (0, import_react.useRef)(null), C = (0, import_react.useMemo)(() => getActiveWorkspaceEmojiShortcode(_, v), [v, _]), w = (0, import_react.useMemo)(() => C ? searchWorkspaceEmojiShortcodes(C.query) : [], [C]), E = !e && C !== null && w.length > 0, D = w.some((e) => `emoji:${e.shortcode}` === b) ? b : w[0] ? `emoji:${w[0].shortcode}` : "", O = w.find((e) => `emoji:${e.shortcode}` === D) ?? null, k = (0, import_react.useCallback)(() => {
		S.current !== null && (cancelAnimationFrame(S.current), S.current = null);
	}, []);
	(0, import_react.useEffect)(() => k, [k]);
	let A = (0, import_react.useCallback)((e) => {
		g(e.value), y(null), k(), S.current = requestAnimationFrame(() => {
			S.current = null, h.current?.focus({ preventScroll: !0 }), h.current?.setSelectionRange(e.cursor, e.cursor);
		});
	}, [
		k,
		h,
		g
	]), j = (0, import_react.useCallback)((e, _ = h.current?.selectionStart ?? e.length) => {
		let v = replaceCompletedWorkspaceEmojiShortcode(e, _);
		if (v) {
			A(v);
			return;
		}
		...
		h,
		g
	]), M = (0, import_react.useCallback)((e = h.current) => y(e?.selectionStart ?? null), [h]), N = (0, import_react.useCallback)(() => y(null), []), P = (0, import_react.useCallback)((e) => {
		C && A(applyWorkspaceEmojiSuggestion(_, C, e));
	}, [
		C,
		A,
		_
	...
	return {
		close: N,
		commandValue: D,
		handleKeyDown: (0, import_react.useCallback)((e) => E ? isImeCompositionKeyDown(e) ? (e.stopPropagation(), !0) : e.key === "ArrowDown" || e.key === "ArrowUp" ? (e.preventDefault(), e.stopPropagation(), x(`emoji:${w[(w.findIndex((e) => `emoji:${e.shortcode}` === D) + (e.key === "ArrowDown" ? 1 : -1) + w.length) % w.length].shortcode}`), !0) : (e.key === "Enter" && !e.metaKey && !e.ctrlKey && !e.altKey && !e.shiftKey || e.key === "Tab" && !e.shiftKey) && O ? (e.preventDefault(), e.stopPropagation(), P(O), !0) : e.key === "Escape" ? (e.stopPropagation(), N(), !0) : !1 : !1, [
			N,
			E,
			D,
			P,
