var isString = (e) => typeof e == "string", defer = () => {
	let e, T, E = new Promise((E, D) => {
		e = E, T = D;
	});
	return E.resolve = e, E.reject = T, E;
}, makeString = (e) => e == null ? "" : String(e), copy = (e, T, E) => {
	e.forEach((e) => {
		T[e] && (E[e] = T[e]);
	});
}, lastOfPathSeparatorRegExp = /###/g, cleanKey = (e) => e && e.includes("###") ? e.replace(lastOfPathSeparatorRegExp, ".") : e, canNotTraverseDeeper = (T) => !T || isString(T), getLastOfPath = (T, E, D) => {
	let O = isString(E) ? E.split(".") : E, j = 0;
	for (; j < O.length - 1;) {
		if (canNotTraverseDeeper(T)) return {};
		let e = cleanKey(O[j]);
		!T[e] && D && (T[e] = new D()), T = Object.prototype.hasOwnProperty.call(T, e) ? T[e] : {}, ++j;
	}
	return canNotTraverseDeeper(T) ? {} : {
		obj: T,
		k: cleanKey(O[j])
	};
}, setPath = (e, T, E) => {
	let { obj: D, k: O } = getLastOfPath(e, T, Object);
	if (D !== void 0 || T.length === 1) {
		D[O] = E;
		return;
	}
	let k = T[T.length - 1], A = T.slice(0, T.length - 1), M = getLastOfPath(e, A, Object);
	for (; M.obj === void 0 && A.length;) k = `${A[A.length - 1]}.${k}`, A = A.slice(0, A.length - 1), M = getLastOfPath(e, A, Object), M?.obj && M.obj[`${M.k}.${k}`] !== void 0 && (M.obj = void 0);
	M.obj[`${M.k}.${k}`] = E;
}, pushPath = (e, T, E, D) => {
	let { obj: O, k } = getLastOfPath(e, T, Object);
	O[k] = O[k] || [], O[k].push(E);
}, getPath = (e, T) => {
	let { obj: E, k: D } = getLastOfPath(e, T);
	if (E && Object.prototype.hasOwnProperty.call(E, D)) return E[D];
}, getPathWithDefaults = (e, T, E) => {
	let D = getPath(e, E);
	return D === void 0 ? getPath(T, E) : D;
}, deepExtend = (T, E, D) => {
	for (let O in E) O !== "__proto__" && O !== "constructor" && (O in T ? isString(T[O]) || T[O] instanceof String || isString(E[O]) || E[O] instanceof String ? D && (T[O] = E[O]) : deepExtend(T[O], E[O], D) : T[O] = E[O]);
	return T;
}, regexEscape = (e) => e.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"), _entityMap = {
	"&": "&amp;",
	"<": "&lt;",
	">": "&gt;",
	"\"": "&quot;",
	"'": "&#39;",
	"/": "&#x2F;"
}, escape = (T) => isString(T) ? T.replace(/[&<>"'\/]/g, (e) => _entityMap[e]) : T, RegExpCache = class {
	constructor(e) {
	...
	getRegExp(e) {
		...
		let E = new RegExp(e);
		return this.regExpQueue.length === this.capacity && this.regExpMap.delete(this.regExpQueue.shift()), this.regExpMap.set(e, E), this.regExpQueue.push(e), E;
	}
}, chars = [
	" ",
	",",
	...
	"!",
	";"
], looksLikeObjectPathRegExpCache = new RegExpCache(20), looksLikeObjectPath = (e, T, E) => {
	T ||= "", E ||= "";
	let D = chars.filter((e) => !T.includes(e) && !E.includes(e));
	if (D.length === 0) return !0;
	let O = looksLikeObjectPathRegExpCache.getRegExp(`(${D.map((e) => e === "?" ? "\\?" : e).join("|")})`), k = !O.test(e);
	if (!k) {
		let T = e.indexOf(E);
		T > 0 && !O.test(e.substring(0, T)) && (k = !0);
	}
	return k;
}, deepFind = (e, T, E = ".") => {
	if (!e) return;
	...
	for (let e = 0; e < D.length;) {
		...
		let T, k = "";
		for (let A = e; A < D.length; ++A) if (A !== e && (k += E), k += D[A], T = O[k], T !== void 0) {
			if ([
				"string",
				"number",
				"boolean"
			].includes(typeof T) && A < D.length - 1) continue;
			e += A - e + 1;
			break;
		}
		O = T;
	...
	return O;
}, getCleanedCode = (e) => e?.replace(/_/g, "-"), consoleLogger = {
	type: "logger",
	log(e) {
		this.output("log", e);
	},
...
}, baseLogger = new class T {
	constructor(e, T = {}) {
		this.init(e, T);
	}
	init(e, T = {}) {
		this.prefix = T.prefix || "i18next:", this.logger = e || consoleLogger, this.options = T, this.debug = T.debug;
	}
	log(...e) {
	...
	forward(T, E, D, O) {
		return O && !this.debug ? null : (T = T.map((T) => isString(T) ? T.replace(/[\r\n\x00-\x1F\x7F]/g, " ") : T), isString(T[0]) && (T[0] = `${D}${this.prefix} ${T[0]}`), this.logger[E](T));
	}
	create(e) {
	...
	on(e, T) {
		return e.split(" ").forEach((e) => {
			...
			let E = this.observers[e].get(T) || 0;
			this.observers[e].set(T, E + 1);
		}), this;
	}
	off(e, T) {
		if (this.observers[e]) {
	...
	once(e, T) {
		...
		return this.on(e, E), this;
	}
	emit(e, ...T) {
		this.observers[e] && Array.from(this.observers[e].entries()).forEach(([e, E]) => {
			for (let D = 0; D < E; D++) e(...T);
		}), this.observers["*"] && Array.from(this.observers["*"].entries()).forEach(([E, D]) => {
			for (let O = 0; O < D; O++) E(e, ...T);
		});
	}
	...
	getResource(T, E, D, O = {}) {
		let k = O.keySeparator === void 0 ? this.options.keySeparator : O.keySeparator, A = O.ignoreJSONStructure === void 0 ? this.options.ignoreJSONStructure : O.ignoreJSONStructure, j;
		T.includes(".") ? j = T.split(".") : (j = [T, E], D && (Array.isArray(D) ? j.push(...D) : isString(D) && k ? j.push(...D.split(k)) : j.push(D)));
		let M = getPath(this.data, j);
		return !M && !E && !D && T.includes(".") && (T = j[0], E = j[1], D = j.slice(2).join(".")), M || !A || !isString(D) ? M : deepFind(this.data?.[T]?.[E], D, k);
	}
	addResource(e, T, E, D, O = { silent: !1 }) {
		let k = O.keySeparator === void 0 ? this.options.keySeparator : O.keySeparator, A = [e, T];
		E && (A = A.concat(k ? E.split(k) : E)), e.includes(".") && (A = e.split("."), D = T, T = A[1]), this.addNamespaces(T), setPath(this.data, A, D), O.silent || this.emit("added", e, T, E, D);
	}
	addResources(T, E, D, O = { silent: !1 }) {
		for (let O in D) (isString(D[O]) || Array.isArray(D[O])) && this.addResource(T, E, O, D[O], { silent: !0 });
		O.silent || this.emit("added", T, E, D);
	}
	addResourceBundle(e, T, E, D, O, k = {
		silent: !1,
		skipCopy: !1
	}) {
		let A = [e, T];
		e.includes(".") && (A = e.split("."), D = E, E = T, T = A[1]), this.addNamespaces(T);
		let j = getPath(this.data, A) || {};
		k.skipCopy || (E = JSON.parse(JSON.stringify(E))), D ? deepExtend(j, E, O) : j = {
			...j,
			...E
		}, setPath(this.data, A, j), k.silent || this.emit("added", e, T, E);
	}
	...
	handle(e, T, E, D, O) {
		return e.forEach((e) => {
			T = this.processors[e]?.process(T, E, D, O) ?? T;
		}), T;
	}
}, PATH_KEY = Symbol("i18next/PATH_KEY");
function createProxy() {
	let e = [], T = Object.create(null), E;
	return T.get = (D, O) => (E?.revoke?.(), O === PATH_KEY ? e : (e.push(O), E = Proxy.revocable(D, T), E.proxy)), Proxy.revocable(Object.create(null), T).proxy;
}
function keysFromSelector(e, T) {
	let { [PATH_KEY]: E } = e(createProxy()), D = T?.keySeparator ?? ".", O = T?.nsSeparator ?? ":", k = T?.enableSelector === "strict";
	if (E.length > 1 && O) {
		let e = T?.ns, A = k ? Array.isArray(e) ? e : e ? [e] : null : Array.isArray(e) ? e : null;
		if (A && (k ? A : A.length > 1 ? A.slice(1) : []).includes(E[0])) return `${E[0]}${O}${E.slice(1).join(D)}`;
	}
	return E.join(D);
}
var shouldHandleAsObject = (T) => !isString(T) && typeof T != "boolean" && typeof T != "number", Translator = class T extends EventEmitter {
	constructor(e, T = {}) {
		super(), copy([
			"resourceStore",
			"languageUtils",
			...
			"i18nFormat",
			"utils"
		], e, this), this.options = T, this.options.keySeparator === void 0 && (this.options.keySeparator = "."), this.logger = baseLogger.create("translator"), this.checkedLoadedFor = {};
	}
	changeLanguage(e) {
		e && (this.language = e);
	}
	exists(e, T = { interpolation: {} }) {
		let E = { ...T };
		if (e == null) return !1;
		let D = this.resolve(e, E);
		if (D?.res === void 0) return !1;
		let O = shouldHandleAsObject(D.res);
		return !(E.returnObjects === !1 && O);
	}
	extractFromKey(T, E) {
		let D = E.nsSeparator === void 0 ? this.options.nsSeparator : E.nsSeparator;
		D === void 0 && (D = ":");
		let O = E.keySeparator === void 0 ? this.options.keySeparator : E.keySeparator, k = E.ns || this.options.defaultNS || [], A = D && T.includes(D), j = !this.options.userDefinedKeySeparator && !E.keySeparator && !this.options.userDefinedNsSeparator && !E.nsSeparator && !looksLikeObjectPath(T, D, O);
		if (A && !j) {
			let E = T.match(this.interpolator.nestingRegexp);
			if (E && E.length > 0) return {
				key: T,
				namespaces: isString(k) ? [k] : k
			};
			let A = T.split(D);
			(D !== O || D === O && this.options.ns.includes(A[0])) && (k = A.shift()), T = A.join(O);
		}
		return {
			key: T,
			namespaces: isString(k) ? [k] : k
	...
	translate(E, D, O) {
		let k = typeof D == "object" ? { ...D } : D;
		if (typeof k != "object" && this.options.overloadTranslationOptionHandler && (k = this.options.overloadTranslationOptionHandler(arguments)), typeof k == "object" && (k = { ...k }), k ||= {}, E == null) return "";
		typeof E == "function" && (E = keysFromSelector(E, {
			...this.options,
			...k
		})), Array.isArray(E) || (E = [String(E)]), E = E.map((e) => typeof e == "function" ? keysFromSelector(e, {
			...this.options,
			...k
		}) : String(e));
		let A = k.returnDetails === void 0 ? this.options.returnDetails : k.returnDetails, j = k.keySeparator === void 0 ? this.options.keySeparator : k.keySeparator, { key: M, namespaces: N } = this.extractFromKey(E[E.length - 1], k), P = N[N.length - 1], F = k.nsSeparator === void 0 ? this.options.nsSeparator : k.nsSeparator;
		F === void 0 && (F = ":");
		let I = k.lng || this.language, L = k.appendNamespaceToCIMode || this.options.appendNamespaceToCIMode;
		if (I?.toLowerCase() === "cimode") return L ? A ? {
			res: `${P}${F}${M}`,
			usedKey: M,
			exactUsedKey: M,
			usedLng: I,
			usedNS: P,
			usedParams: this.getUsedParamsDetails(k)
		} : `${P}${F}${M}` : A ? {
			res: M,
			usedKey: M,
			exactUsedKey: M,
		...
		let R = this.resolve(E, k), z = R?.res, B = R?.usedKey || M, V = R?.exactUsedKey || M, H = [
			"[object Number]",
			"[object Function]",
			"[object RegExp]"
		], U = k.joinArrays === void 0 ? this.options.joinArrays : k.joinArrays, W = !this.i18nFormat || this.i18nFormat.handleAsObject, G = k.count !== void 0 && !isString(k.count), K = T.hasDefaultValue(k), q = G ? this.pluralResolver.getSuffix(I, k.count, k) : "", J = k.ordinal && G ? this.pluralResolver.getSuffix(I, k.count, { ordinal: !1 }) : "", Y = G && !k.ordinal && k.count === 0, X = Y && k[`defaultValue${this.options.pluralSeparator}zero`] || k[`defaultValue${q}`] || k[`defaultValue${J}`] || k.defaultValue, Z = z;
		W && !z && K && (Z = X);
		let Q = shouldHandleAsObject(Z), $ = Object.prototype.toString.apply(Z);
		if (W && Z && Q && !H.includes($) && !(isString(U) && Array.isArray(Z))) {
			if (!k.returnObjects && !this.options.returnObjects) {
				this.options.returnedObjectHandler || this.logger.warn("accessing an object - but returnObjects options is not enabled!");
				let e = this.options.returnedObjectHandler ? this.options.returnedObjectHandler(B, Z, {
					...k,
					ns: N
				}) : `key '${M} (${this.language})' returned an object instead of string.`;
				return A ? (R.res = e, R.usedParams = this.getUsedParamsDetails(k), R) : e;
			}
			if (j) {
				let e = Array.isArray(Z), T = e ? [] : {}, E = e ? V : B;
				for (let e in Z) if (Object.prototype.hasOwnProperty.call(Z, e)) {
					let D = `${E}${j}${e}`;
					K && !z ? T[e] = this.translate(D, {
						...k,
						defaultValue: shouldHandleAsObject(X) ? X[e] : void 0,
						joinArrays: !1,
						ns: N
					}) : T[e] = this.translate(D, {
						...k,
						joinArrays: !1,
						ns: N
					}), T[e] === D && (T[e] = Z[e]);
		...
		} else if (W && isString(U) && Array.isArray(z)) z = z.join(U), z &&= this.extendTranslation(z, E, k, O);
		else {
			let e = !1, T = !1;
			!this.isValidLookup(z) && K && (e = !0, z = X), this.isValidLookup(z) || (T = !0, z = M);
			let D = (k.missingKeyNoValueFallbackToKey || this.options.missingKeyNoValueFallbackToKey) && T ? void 0 : z, A = K && X !== z && this.options.updateMissing;
			if (T || e || A) {
				if (this.logger.log(A ? "updateKey" : "missingKey", I, P, G && !A ? `${M}${this.pluralResolver.getSuffix(I, k.count, k)}` : M, A ? X : z), j) {
					let e = this.resolve(M, {
						...k,
						keySeparator: !1
					});
					e && e.res && this.logger.warn("Seems the loaded translations were in flat JSON format instead of nested. Either set keySeparator: false on init or make sure your translations are published in nested format.");
				}
				let e = [], T = this.languageUtils.getFallbackCodes(this.options.fallbackLng, k.lng || this.language);
				if (this.options.saveMissingTo === "fallback" && T && T[0]) for (let E = 0; E < T.length; E++) e.push(T[E]);
				else this.options.saveMissingTo === "all" ? e = this.languageUtils.toResolveHierarchy(k.lng || this.language) : e.push(k.lng || this.language);
				let E = (e, T, E) => {
					let O = K && E !== z ? E : D;
					this.options.missingKeyHandler ? this.options.missingKeyHandler(e, P, T, O, A, k) : this.backendConnector?.saveMissing && this.backendConnector.saveMissing(e, P, T, O, A, k), this.emit("missingKey", e, P, T, z);
				};
				this.options.saveMissing && (this.options.saveMissingPlurals && G ? e.forEach((e) => {
					let T = this.pluralResolver.getSuffixes(e, k);
					Y && k[`defaultValue${this.options.pluralSeparator}zero`] && !T.includes(`${this.options.pluralSeparator}zero`) && T.push(`${this.options.pluralSeparator}zero`), T.forEach((T) => {
						E([e], M + T, k[`defaultValue${T}`] || X);
					});
				}) : E(e, M, X));
			}
			z = this.extendTranslation(z, E, k, R, O), T && z === M && this.options.appendNamespaceToMissingKey && (z = `${P}${F}${M}`), (T || e) && this.options.parseMissingKeyHandler && (z = this.options.parseMissingKeyHandler(this.options.appendNamespaceToMissingKey ? `${P}${F}${M}` : M, e ? z : void 0, k));
		}
		return A ? (R.res = z, R.usedParams = this.getUsedParamsDetails(k), R) : z;
	}
	extendTranslation(T, E, D, O, k) {
		if (this.i18nFormat?.parse) T = this.i18nFormat.parse(T, {
			...this.options.interpolation.defaultVariables,
			...D
		}, D.lng || this.language || O.usedLng, O.usedNS, O.usedKey, { resolved: O });
			...
			let A = isString(T) && (D?.interpolation?.skipOnVariables === void 0 ? this.options.interpolation.skipOnVariables : D.interpolation.skipOnVariables), j;
			if (A) {
				let e = T.match(this.interpolator.nestingRegexp);
				j = e && e.length;
			}
			let M = D.replace && !isString(D.replace) ? D.replace : D;
			if (this.options.interpolation.defaultVariables && (M = {
				...this.options.interpolation.defaultVariables,
				...M
			}), T = this.interpolator.interpolate(T, M, D.lng || this.language || O.usedLng, D), A) {
				let e = T.match(this.interpolator.nestingRegexp), E = e && e.length;
				j < E && (D.nest = !1);
			}
			!D.lng && O && O.res && (D.lng = this.language || O.usedLng), D.nest !== !1 && (T = this.interpolator.nest(T, (...e) => k?.[0] === e[0] && !D.context ? (this.logger.warn(`It seems you are nesting recursively key: ${e[0]} in key: ${E[0]}`), null) : this.translate(...e, E), D)), D.interpolation && this.interpolator.reset();
		}
		let A = D.postProcess || this.options.postProcess, j = isString(A) ? [A] : A;
		return T != null && j?.length && D.applyPostProcessor !== !1 && (T = postProcessor.handle(j, T, E, this.options && this.options.postProcessPassResolved ? {
			i18nResolved: {
				...O,
	...
	resolve(T, E = {}) {
		let D, O, k, A, j;
		return isString(T) && (T = [T]), Array.isArray(T) && (T = T.map((e) => typeof e == "function" ? keysFromSelector(e, {
			...this.options,
			...E
		}) : e)), T.forEach((T) => {
			if (this.isValidLookup(D)) return;
			let M = this.extractFromKey(T, E), N = M.key;
			O = N;
			let P = M.namespaces;
			this.options.fallbackNS && (P = P.concat(this.options.fallbackNS));
			let F = E.count !== void 0 && !isString(E.count), I = F && !E.ordinal && E.count === 0, L = E.context !== void 0 && (isString(E.context) || typeof E.context == "number") && E.context !== "", R = E.lngs ? E.lngs : this.languageUtils.toResolveHierarchy(E.lng || this.language, E.fallbackLng);
			P.forEach((e) => {
				this.isValidLookup(D) || (j = e, !this.checkedLoadedFor[`${R[0]}-${e}`] && this.utils?.hasLoadedNamespace && !this.utils?.hasLoadedNamespace(j) && (this.checkedLoadedFor[`${R[0]}-${e}`] = !0, this.logger.warn(`key "${O}" for languages "${R.join(", ")}" won't get resolved as namespace "${j}" was not yet loaded`, "This means something IS WRONG in your setup. You access the t function before i18next.init / i18next.loadNamespace / i18next.changeLanguage was done. Wait for the callback or Promise to resolve before accessing it!!!")), R.forEach((T) => {
					if (this.isValidLookup(D)) return;
					A = T;
					let O = [N];
					if (this.i18nFormat?.addLookupKeys) this.i18nFormat.addLookupKeys(O, N, T, e, E);
					else {
						let e;
						F && (e = this.pluralResolver.getSuffix(T, E.count, E));
						let D = `${this.options.pluralSeparator}zero`, k = `${this.options.pluralSeparator}ordinal${this.options.pluralSeparator}`;
						if (F && (E.ordinal && e.startsWith(k) && O.push(N + e.replace(k, this.options.pluralSeparator)), O.push(N + e), I && O.push(N + D)), L) {
							let T = `${N}${this.options.contextSeparator || "_"}${E.context}`;
							O.push(T), F && (E.ordinal && e.startsWith(k) && O.push(T + e.replace(k, this.options.pluralSeparator)), O.push(T + e), I && O.push(T + D));
						}
					}
					let j;
					for (; j = O.pop();) this.isValidLookup(D) || (k = j, D = this.getResource(T, e, j, E));
				}));
			});
		}), {
			res: D,
	...
	getResource(e, T, E, D = {}) {
		return this.i18nFormat?.getResource ? this.i18nFormat.getResource(e, T, E, D) : this.resourceStore.getResource(e, T, E, D);
	}
	getUsedParamsDetails(T = {}) {
		let E = [
			...
			"postProcess",
			"interpolation"
		], D = T.replace && !isString(T.replace), O = D ? T.replace : T;
		if (D && T.count !== void 0 && (O.count = T.count), this.options.interpolation.defaultVariables && (O = {
			...this.options.interpolation.defaultVariables,
			...O
		}), !D) {
			O = { ...O };
			for (let e of E) delete O[e];
		}
		return O;
	}
	...
	constructor(e) {
		this.options = e, this.supportedLngs = this.options.supportedLngs || !1, this.logger = baseLogger.create("languageUtils");
	}
	getScriptPartFromCode(e) {
		if (e = getCleanedCode(e), !e || !e.includes("-")) return null;
		let T = e.split("-");
		return T.length === 2 || (T.pop(), T[T.length - 1].toLowerCase() === "x") ? null : this.formatLanguageCode(T.join("-"));
	}
	getLanguagePartFromCode(e) {
		if (e = getCleanedCode(e), !e || !e.includes("-")) return e;
		let T = e.split("-");
	...
	getBestMatchFromCodes(e) {
		...
		return e.forEach((e) => {
			if (T) return;
			let E = this.formatLanguageCode(e);
			(!this.options.supportedLngs || this.isSupportedCode(E)) && (T = E);
		}), !T && this.options.supportedLngs && e.forEach((e) => {
			if (T) return;
			let E = this.getScriptPartFromCode(e);
			if (this.isSupportedCode(E)) return T = E;
			let D = this.getLanguagePartFromCode(e);
			if (this.isSupportedCode(D)) return T = D;
			T = this.options.supportedLngs.find((e) => e === D ? !0 : !e.includes("-") && !D.includes("-") ? !1 : !!(e.includes("-") && !D.includes("-") && e.slice(0, e.indexOf("-")) === D || e.startsWith(D) && D.length > 1));
		}), T ||= this.getFallbackCodes(this.options.fallbackLng)[0], T;
	}
	getFallbackCodes(T, E) {
		if (!T) return [];
		if (typeof T == "function" && (T = T(E)), isString(T) && (T = [T]), Array.isArray(T)) return T;
		if (!E) return T.default || [];
		let D = T[E];
		return D ||= T[this.getScriptPartFromCode(E)], D ||= T[this.formatLanguageCode(E)], D ||= T[this.getLanguagePartFromCode(E)], D ||= T.default, D || [];
	}
	toResolveHierarchy(T, E) {
		let D = this.getFallbackCodes((E === !1 ? [] : E) || this.options.fallbackLng || [], T), O = [], k = (e) => {
			e && (this.isSupportedCode(e) ? O.push(e) : this.logger.warn(`rejecting language code not found in supportedLngs: ${e}`));
		};
		return isString(T) && (T.includes("-") || T.includes("_")) ? (this.options.load !== "languageOnly" && k(this.formatLanguageCode(T)), this.options.load !== "languageOnly" && this.options.load !== "currentOnly" && k(this.getScriptPartFromCode(T)), this.options.load !== "currentOnly" && k(this.getLanguagePartFromCode(T))) : isString(T) && k(this.formatLanguageCode(T)), D.forEach((e) => {
			O.includes(e) || k(this.formatLanguageCode(e));
		}), O;
	}
}, suffixesOrder = {
	zero: 0,
	...
	constructor(e, T = {}) {
		this.languageUtils = e, this.options = T, this.logger = baseLogger.create("pluralResolver"), this.pluralRulesCache = {};
	}
	clearCache() {
		this.pluralRulesCache = {};
	}
	getRule(e, T = {}) {
		let E = getCleanedCode(e === "dev" ? "en" : e), D = T.ordinal ? "ordinal" : "cardinal", O = JSON.stringify({
			cleanedCode: E,
			type: D
		});
		...
		let k;
		try {
			k = new Intl.PluralRules(E, { type: D });
		} catch {
			if (typeof Intl > "u") return this.logger.error("No Intl support, please use an Intl polyfill!"), dummyRule;
			if (!e.match(/-|_/)) return dummyRule;
			let E = this.languageUtils.getLanguagePartFromCode(e);
			k = this.getRule(E, T);
		}
		return this.pluralRulesCache[O] = k, k;
	}
	needsPlural(e, T = {}) {
		let E = this.getRule(e, T);
		return E ||= this.getRule("dev", T), E?.resolvedOptions().pluralCategories.length > 1;
	}
	getPluralFormsOfKey(e, T, E = {}) {
		return this.getSuffixes(e, E).map((e) => `${T}${e}`);
	}
	getSuffixes(e, T = {}) {
		let E = this.getRule(e, T);
		return E ||= this.getRule("dev", T), E ? E.resolvedOptions().pluralCategories.sort((e, T) => suffixesOrder[e] - suffixesOrder[T]).map((e) => `${this.options.prepend}${T.ordinal ? `ordinal${this.options.prepend}` : ""}${e}`) : [];
	}
	getSuffix(e, T, E = {}) {
		let D = this.getRule(e, E);
		return D ? `${this.options.prepend}${E.ordinal ? `ordinal${this.options.prepend}` : ""}${D.select(T)}` : (this.logger.warn(`no plural rule found for: ${e}`), this.getSuffix("dev", T, E));
	}
}, deepFindWithDefaults = (T, E, D, O = ".", k = !0) => {
	let A = getPathWithDefaults(T, E, D);
	return !A && k && isString(D) && (A = deepFind(T, D, O), A === void 0 && (A = deepFind(E, D, O))), A;
}, regexSafe = (e) => e.replace(/\$/g, "$$$$"), Interpolator = class {
	constructor(e = {}) {
		this.logger = baseLogger.create("interpolator"), this.options = e, this.format = e?.interpolation?.format || ((e) => e), this.init(e);
	}
	init(e = {}) {
		e.interpolation ||= { escapeValue: !0 };
		let { escape: T, escapeValue: E, useRawValueToEscape: D, prefix: O, prefixEscaped: k, suffix: A, suffixEscaped: j, formatSeparator: M, unescapeSuffix: N, unescapePrefix: P, nestingPrefix: F, nestingPrefixEscaped: I, nestingSuffix: R, nestingSuffixEscaped: B, nestingOptionsSeparator: V, maxReplaces: H, alwaysFormat: U } = e.interpolation;
		this.escape = T === void 0 ? escape : T, this.escapeValue = E === void 0 ? !0 : E, this.useRawValueToEscape = D === void 0 ? !1 : D, this.prefix = O ? regexEscape(O) : k || "{{", this.suffix = A ? regexEscape(A) : j || "}}", this.formatSeparator = M || ",", this.unescapePrefix = N ? "" : P ? regexEscape(P) : "-", this.unescapeSuffix = this.unescapePrefix ? "" : N ? regexEscape(N) : "", this.nestingPrefix = F ? regexEscape(F) : I || regexEscape("$t("), this.nestingSuffix = R ? regexEscape(R) : B || regexEscape(")"), this.nestingOptionsSeparator = V || ",", this.maxReplaces = H || 1e3, this.alwaysFormat = U === void 0 ? !1 : U, this.resetRegExp();
	}
	reset() {
	...
	interpolate(T, D, O, k) {
		let A, j, M, N = this.options && this.options.interpolation && this.options.interpolation.defaultVariables || {}, P = (e) => {
			if (!e.includes(this.formatSeparator)) {
				let T = deepFindWithDefaults(D, N, e, this.options.keySeparator, this.options.ignoreJSONStructure);
			...
			let T = e.split(this.formatSeparator), E = T.shift().trim(), A = T.join(this.formatSeparator).trim();
			return this.format(deepFindWithDefaults(D, N, E, this.options.keySeparator, this.options.ignoreJSONStructure), A, O, {
				...k,
				...D,
				interpolationkey: E
			});
		...
		let F = k?.missingInterpolationHandler || this.options.missingInterpolationHandler, I = k?.interpolation?.skipOnVariables === void 0 ? this.options.interpolation.skipOnVariables : k.interpolation.skipOnVariables;
		return [{
			...
			for (M = 0; A = D.regex.exec(T);) {
				let O = A[1].trim();
				if (j = P(O), j === void 0) if (typeof F == "function") {
					let E = F(T, A, k);
					j = isString(E) ? E : "";
				} else if (k && Object.prototype.hasOwnProperty.call(k, O)) j = "";
				else if (I) {
					j = A[0];
					continue;
				} else this.logger.warn(`missed to pass in variable ${O} for interpolating ${T}`), j = "";
				else !isString(j) && !this.useRawValueToEscape && (j = makeString(j));
				let N = D.safeValue(j);
				if (T = T.replace(A[0], N), I ? (D.regex.lastIndex += j.length, D.regex.lastIndex -= A[0].length) : D.regex.lastIndex = 0, M++, M >= this.maxReplaces) break;
			}
		}), T;
	}
	nest(T, D, O = {}) {
		let k, A, j, M = (e, T) => {
			let E = this.nestingOptionsSeparator;
			if (!e.includes(E)) return e;
			let D = e.split(/* @__PURE__ */ RegExp(`${regexEscape(E)}[ ]*{`)), O = `{${D[1]}`;
			e = D[0], O = this.interpolate(O, j);
			let k = O.match(/'/g), A = O.match(/"/g);
			((k?.length ?? 0) % 2 == 0 && !A || (A?.length ?? 0) % 2 != 0) && (O = O.replace(/'/g, "\""));
			try {
				j = JSON.parse(O), T && (j = {
					...T,
					...j
				...
				return this.logger.warn(`failed parsing options string in nesting for key ${e}`, T), `${e}${E}${O}`;
			}
			return j.defaultValue && j.defaultValue.includes(this.prefix) && delete j.defaultValue, e;
		};
		for (; k = this.nestingRegexp.exec(T);) {
			let N = [];
			j = { ...O }, j = j.replace && !isString(j.replace) ? j.replace : j, j.applyPostProcessor = !1, delete j.defaultValue;
			let P = /{.*}/.test(k[1]) ? k[1].lastIndexOf("}") + 1 : k[1].indexOf(this.formatSeparator);
			if (P !== -1 && (N = k[1].slice(P).split(this.formatSeparator).map((e) => e.trim()).filter(Boolean), k[1] = k[1].slice(0, P)), A = D(M.call(this, k[1].trim(), j), j), A && k[0] === T && !isString(A)) return A;
			isString(A) || (A = makeString(A)), A ||= (this.logger.warn(`missed to resolve ${k[1]} for nesting ${T}`), ""), N.length && (A = N.reduce((e, T) => this.format(e, T, O.lng, {
				...O,
				interpolationkey: k[1].trim()
			}), A.trim())), T = T.replace(k[0], A), this.regexp.lastIndex = 0;
		}
		return T;
	}
}, parseFormatStr = (e) => {
	let T = e.toLowerCase().trim(), E = {};
	if (e.includes("(")) {
		let D = e.split("(");
		T = D[0].toLowerCase().trim();
		let O = D[1].slice(0, -1);
		T === "currency" && !O.includes(":") ? E.currency ||= O.trim() : T === "relativetime" && !O.includes(":") ? E.range ||= O.trim() : O.split(";").forEach((e) => {
			if (e) {
				let [T, ...D] = e.split(":"), O = D.join(":").trim().replace(/^'+|'+$/g, ""), k = T.trim();
				E[k] || (E[k] = O), O === "false" && (E[k] = !1), O === "true" && (E[k] = !0), isNaN(O) || (E[k] = parseInt(O, 10));
			}
		});
	}
	return {
...
}, createCachedFormatter = (e) => {
	let T = {};
	return (E, D, O) => {
		let k = O;
		O && O.interpolationkey && O.formatParams && O.formatParams[O.interpolationkey] && O[O.interpolationkey] && (k = {
			...k,
			[O.interpolationkey]: void 0
		});
		let A = D + JSON.stringify(k), j = T[A];
		return j || (j = e(getCleanedCode(D), O), T[A] = j), j(E);
	};
}, createNonCachedFormatter = (e) => (T, E, D) => e(getCleanedCode(E), D)(T), Formatter = class {
	constructor(e = {}) {
		this.logger = baseLogger.create("formatter"), this.options = e, this.init(e);
	}
	init(e, T = { interpolation: {} }) {
		this.formatSeparator = T.interpolation.formatSeparator || ",";
		let E = T.cacheInBuiltFormats ? createCachedFormatter : createNonCachedFormatter;
		this.formats = {
			number: E((e, T) => {
				let E = new Intl.NumberFormat(e, { ...T });
				return (e) => E.format(e);
			}),
			currency: E((e, T) => {
				let E = new Intl.NumberFormat(e, {
					...T,
					style: "currency"
				});
				return (e) => E.format(e);
			...
			datetime: E((e, T) => {
				let E = new Intl.DateTimeFormat(e, { ...T });
				return (e) => E.format(e);
			}),
			relativetime: E((e, T) => {
				let E = new Intl.RelativeTimeFormat(e, { ...T });
				return (e) => E.format(e, T.range || "day");
			}),
			list: E((e, T) => {
				let E = new Intl.ListFormat(e, { ...T });
				return (e) => E.format(e);
			})
		};
	}
	...
	format(e, T, E, D = {}) {
		...
		if (O.length > 1 && O[0].indexOf("(") > 1 && !O[0].includes(")") && O.find((e) => e.includes(")"))) {
			let e = O.findIndex((e) => e.includes(")"));
			O[0] = [O[0], ...O.splice(1, e)].join(this.formatSeparator);
		}
		return O.reduce((e, T) => {
			let { formatName: O, formatOptions: k } = parseFormatStr(T);
			if (this.formats[O]) {
				let T = e;
				try {
					let A = D?.formatParams?.[D.interpolationkey] || {}, j = A.locale || A.lng || D.locale || D.lng || E;
					T = this.formats[O](e, j, {
						...k,
						...D,
						...A
					});
	...
	constructor(e, T, E, D = {}) {
		super(), this.backend = e, this.store = T, this.services = E, this.languageUtils = E.languageUtils, this.options = D, this.logger = baseLogger.create("backendConnector"), this.waitingReads = [], this.maxParallelReads = D.maxParallelReads || 10, this.readingCalls = 0, this.maxRetries = D.maxRetries >= 0 ? D.maxRetries : 5, this.retryTimeout = D.retryTimeout >= 1 ? D.retryTimeout : 350, this.state = {}, this.queue = [], this.backend?.init?.(E, D.backend, D);
	}
	queueLoad(e, T, E, D) {
		let O = {}, k = {}, A = {}, j = {};
		return e.forEach((e) => {
			...
			T.forEach((T) => {
				let A = `${e}|${T}`;
				!E.reload && this.store.hasResourceBundle(e, T) ? this.state[A] = 2 : this.state[A] < 0 || (this.state[A] === 1 ? k[A] === void 0 && (k[A] = !0) : (this.state[A] = 1, D = !1, k[A] === void 0 && (k[A] = !0), O[A] === void 0 && (O[A] = !0), j[T] === void 0 && (j[T] = !0)));
			}), D || (A[e] = !0);
		}), (Object.keys(O).length || Object.keys(k).length) && this.queue.push({
			pending: k,
			...
			toLoadNamespaces: Object.keys(j)
		};
	}
	loaded(e, T, E) {
		let D = e.split("|"), O = D[0], k = D[1];
		T && this.emit("failedLoading", O, k, T), !T && E && this.store.addResourceBundle(O, k, E, void 0, void 0, { skipCopy: !0 }), this.state[e] = T ? -1 : 2, T && E && (this.state[e] = 0);
		let A = {};
		this.queue.forEach((E) => {
			pushPath(E.loaded, [O], k), removePending(E, e), T && E.errors.push(T), E.pendingCount === 0 && !E.done && (Object.keys(E.loaded).forEach((e) => {
				A[e] || (A[e] = {});
				let T = E.loaded[e];
				T.length && T.forEach((T) => {
					A[e][T] === void 0 && (A[e][T] = !0);
				});
			}), E.done = !0, E.errors.length ? E.callback(E.errors) : E.callback());
		}), this.emit("loaded", A), this.queue = this.queue.filter((e) => !e.done);
	}
	read(e, T, E, D = 0, O = this.retryTimeout, k) {
		if (!e.length) return k(null, {});
		...
		let A = (A, j) => {
			...
			if (A && j && D < this.maxRetries) {
				setTimeout(() => {
					this.read(e, T, E, D + 1, O * 2, k);
				}, O);
				return;
			}
			k(A, j);
		}, j = this.backend[E].bind(this.backend);
		if (j.length === 2) {
			try {
				let E = j(e, T);
				E && typeof E.then == "function" ? E.then((e) => A(null, e)).catch(A) : A(null, E);
			} catch (e) {
				A(e);
			}
			return;
	...
	prepareLoading(T, E, D = {}, O) {
		if (!this.backend) return this.logger.warn("No backend was added via i18next.use. Will not load resources."), O && O();
		isString(T) && (T = this.languageUtils.toResolveHierarchy(T)), isString(E) && (E = [E]);
		let k = this.queueLoad(T, E, D, O);
		if (!k.toLoad.length) return k.pending.length || O(), null;
		k.toLoad.forEach((e) => {
			this.loadOne(e);
		});
	}
	load(e, T, E) {
		this.prepareLoading(e, T, {}, E);
	}
	reload(e, T, E) {
		this.prepareLoading(e, T, { reload: !0 }, E);
	}
	loadOne(e, T = "") {
		let E = e.split("|"), D = E[0], O = E[1];
		this.read(D, O, "read", void 0, void 0, (E, k) => {
			E && this.logger.warn(`${T}loading namespace ${O} for language ${D} failed`, E), !E && k && this.logger.log(`${T}loaded namespace ${O} for language ${D}`, k), this.loaded(e, E, k);
		});
	}
	saveMissing(e, T, E, D, O, k = {}, A = () => {}) {
		if (this.services?.utils?.hasLoadedNamespace && !this.services?.utils?.hasLoadedNamespace(T)) {
			this.logger.warn(`did not save key "${E}" as the namespace "${T}" was not yet loaded`, "This means something IS WRONG in your setup. You access the t function before i18next.init / i18next.loadNamespace / i18next.changeLanguage was done. Wait for the callback or Promise to resolve before accessing it!!!");
			return;
		}
		if (!(E == null || E === "")) {
			if (this.backend?.create) {
				...
				if (M.length < 6) try {
					let O;
					O = M.length === 5 ? M(e, T, E, D, j) : M(e, T, E, D), O && typeof O.then == "function" ? O.then((e) => A(null, e)).catch(A) : A(null, O);
				} catch (e) {
					A(e);
				}
				else M(e, T, E, D, A, j);
			}
			!e || !e[0] || this.store.addResource(e[0], T, E, D);
		}
	}
}, get = () => ({
	debug: !1,
	...
	overloadTranslationOptionHandler: (T) => {
		let E = {};
		if (typeof T[1] == "object" && (E = T[1]), isString(T[1]) && (E.defaultValue = T[1]), isString(T[2]) && (E.tDescription = T[2]), typeof T[2] == "object" || typeof T[3] == "object") {
			let e = T[3] || T[2];
...
}), transformOptions = (T) => (isString(T.ns) && (T.ns = [T.ns]), isString(T.fallbackLng) && (T.fallbackLng = [T.fallbackLng]), isString(T.fallbackNS) && (T.fallbackNS = [T.fallbackNS]), T.supportedLngs && !T.supportedLngs.includes("cimode") && (T.supportedLngs = T.supportedLngs.concat(["cimode"])), T), noop = () => {}, bindMemberFunctions = (e) => {
	Object.getOwnPropertyNames(Object.getPrototypeOf(e)).forEach((T) => {
		typeof e[T] == "function" && (e[T] = e[T].bind(e));
	});
}, instance = class E extends EventEmitter {
	constructor(e = {}, T) {
		if (super(), this.options = transformOptions(e), this.services = {}, this.logger = baseLogger, this.modules = { external: [] }, bindMemberFunctions(this), T && !this.isInitialized && !e.isClone) {
	...
	init(E = {}, D) {
		this.isInitializing = !0, typeof E == "function" && (D = E, E = {}), E.defaultNS == null && E.ns && (isString(E.ns) ? E.defaultNS = E.ns : E.ns.includes("translation") || (E.defaultNS = E.ns[0]));
		let O = get();
		this.options = {
			...O,
			...this.options,
			...
			...O.interpolation,
			...this.options.interpolation
		}, E.keySeparator !== void 0 && (this.options.userDefinedKeySeparator = E.keySeparator), E.nsSeparator !== void 0 && (this.options.userDefinedNsSeparator = E.nsSeparator), typeof this.options.overloadTranslationOptionHandler != "function" && (this.options.overloadTranslationOptionHandler = O.overloadTranslationOptionHandler);
		let k = (e) => e ? typeof e == "function" ? new e() : e : null;
		if (!this.options.isClone) {
			this.modules.logger ? baseLogger.init(k(this.modules.logger), this.options) : baseLogger.init(null, this.options);
			let e;
			e = this.modules.formatter ? this.modules.formatter : Formatter;
			let T = new LanguageUtil(this.options);
			this.store = new ResourceStore(this.options.resources, this.options);
			let E = this.services;
			E.logger = baseLogger, E.resourceStore = this.store, E.languageUtils = T, E.pluralResolver = new PluralResolver(T, { prepend: this.options.pluralSeparator }), e && (E.formatter = k(e), E.formatter.init && E.formatter.init(E, this.options), this.options.interpolation.format = E.formatter.format.bind(E.formatter)), E.interpolator = new Interpolator(this.options), E.utils = { hasLoadedNamespace: this.hasLoadedNamespace.bind(this) }, E.backendConnector = new Connector(k(this.modules.backend), E.resourceStore, E, this.options), E.backendConnector.on("*", (e, ...T) => {
				this.emit(e, ...T);
			}), this.modules.languageDetector && (E.languageDetector = k(this.modules.languageDetector), E.languageDetector.init && E.languageDetector.init(E, this.options.detection, this.options)), this.modules.i18nFormat && (E.i18nFormat = k(this.modules.i18nFormat), E.i18nFormat.init && E.i18nFormat.init(this)), this.translator = new Translator(this.services, this.options), this.translator.on("*", (e, ...T) => {
				this.emit(e, ...T);
			}), this.modules.external.forEach((e) => {
				e.init && e.init(this);
			});
		}
		if (this.format = this.options.interpolation.format, D ||= noop, this.options.fallbackLng && !this.services.languageDetector && !this.options.lng) {
		...
		let A = defer(), j = () => {
			let e = (e, T) => {
				this.isInitializing = !1, this.isInitialized && !this.initializedStoreOnce && this.logger.warn("init: i18next is already initialized. You should call init just once!"), this.isInitialized = !0, this.options.isClone || this.logger.log("initialized", this.options), this.emit("initialized", this.options), A.resolve(T), D(e, T);
			};
			if ((this.languages || this.isLanguageChangingTo) && !this.isInitialized) return e(null, this.t.bind(this));
			this.changeLanguage(this.options.lng, e);
		};
		return this.options.resources || !this.options.initAsync ? j() : setTimeout(j, 0), A;
	}
	loadResources(T, E = noop) {
		let D = E, O = isString(T) ? T : this.language;
		if (typeof T == "function" && (D = T), !this.options.resources || this.options.partialBundledLanguages) {
			if (O?.toLowerCase() === "cimode" && (!this.options.preload || this.options.preload.length === 0)) return D();
			let e = [], T = (T) => {
				T && T !== "cimode" && this.services.languageUtils.toResolveHierarchy(T).forEach((T) => {
					T !== "cimode" && (e.includes(T) || e.push(T));
				});
			};
			O ? T(O) : this.services.languageUtils.getFallbackCodes(this.options.fallbackLng).forEach((e) => T(e)), this.options.preload?.forEach?.((e) => T(e)), this.services.backendConnector.load(e, this.options.ns, (e) => {
				!e && !this.resolvedLanguage && this.language && this.setResolvedLanguage(this.language), D(e);
			});
		} else D(null);
	}
	reloadResources(e, E, D) {
		let O = defer();
		return typeof e == "function" && (D = e, e = void 0), typeof E == "function" && (D = E, E = void 0), e ||= this.languages, E ||= this.options.ns, D ||= noop, this.services.backendConnector.reload(e, E, (e) => {
			O.resolve(), D(e);
		}), O;
	}
	use(e) {
		...
		return e.type === "backend" && (this.modules.backend = e), (e.type === "logger" || e.log && e.warn && e.error) && (this.modules.logger = e), e.type === "languageDetector" && (this.modules.languageDetector = e), e.type === "i18nFormat" && (this.modules.i18nFormat = e), e.type === "postProcessor" && postProcessor.addPostProcessor(e), e.type === "formatter" && (this.modules.formatter = e), e.type === "3rdParty" && this.modules.external.push(e), this;
	}
	...
	changeLanguage(E, D) {
		...
		let k = (e) => {
			...
			T ? this.isLanguageChangingTo === E && (k(T), this.translator.changeLanguage(T), this.isLanguageChangingTo = void 0, this.emit("languageChanged", T), this.logger.log("languageChanged", T)) : this.isLanguageChangingTo = void 0, O.resolve((...e) => this.t(...e)), D && D(e, (...e) => this.t(...e));
		}, j = (T) => {
			!E && !T && this.services.languageDetector && (T = []);
			let D = isString(T) ? T : T && T[0], O = this.store.hasLanguageSomeTranslations(D) ? D : this.services.languageUtils.getBestMatchFromCodes(isString(T) ? [T] : T);
			O && (this.language || k(O), this.translator.language || this.translator.changeLanguage(O), this.services.languageDetector?.cacheUserLanguage?.(O)), this.loadResources(O, (e) => {
				A(e, O);
			});
		};
		return !E && this.services.languageDetector && !this.services.languageDetector.async ? j(this.services.languageDetector.detect()) : !E && this.services.languageDetector && this.services.languageDetector.async ? this.services.languageDetector.detect.length === 0 ? this.services.languageDetector.detect().then(j) : this.services.languageDetector.detect(j) : j(E), O;
	}
	getFixedT(T, E, D, O) {
		let k = O?.scopeNs, A = (e, T, ...E) => {
			let O;
			O = typeof T == "object" ? { ...T } : this.options.overloadTranslationOptionHandler([e, T].concat(E)), O.lng = O.lng || A.lng, O.lngs = O.lngs || A.lngs;
			let j = O.ns !== void 0 && O.ns !== null;
			O.ns = O.ns || A.ns, O.keyPrefix !== "" && (O.keyPrefix = O.keyPrefix || D || A.keyPrefix);
			let M = {
				...this.options,
				...O
			};
			Array.isArray(k) && !j && (M.ns = k), typeof O.keyPrefix == "function" && (O.keyPrefix = keysFromSelector(O.keyPrefix, M));
			let N = this.options.keySeparator || ".", P;
			return O.keyPrefix && Array.isArray(e) ? P = e.map((e) => (typeof e == "function" && (e = keysFromSelector(e, M)), `${O.keyPrefix}${N}${e}`)) : (typeof e == "function" && (e = keysFromSelector(e, M)), P = O.keyPrefix ? `${O.keyPrefix}${N}${e}` : e), this.t(P, O);
		};
		return isString(T) ? A.lng = T : A.lngs = T, A.ns = E, A.keyPrefix = D, A;
	}
	t(...e) {
	...
	hasLoadedNamespace(e, T = {}) {
		...
		let E = T.lng || this.resolvedLanguage || this.languages[0], D = this.options ? this.options.fallbackLng : !1, O = this.languages[this.languages.length - 1];
		if (E.toLowerCase() === "cimode") return !0;
		let k = (e, T) => {
			let E = this.services.backendConnector.state[`${e}|${T}`];
			return E === -1 || E === 0 || E === 2;
		};
		if (T.precheck) {
			let e = T.precheck(this, k);
			if (e !== void 0) return e;
		}
		return !!(this.hasResourceBundle(E, e) || !this.services.backendConnector.backend || this.options.resources && !this.options.partialBundledLanguages || k(E, e) && (!D || k(O, e)));
	}
	loadNamespaces(E, D) {
		let O = defer();
		return this.options.ns ? (isString(E) && (E = [E]), E.forEach((e) => {
			this.options.ns.includes(e) || this.options.ns.push(e);
		}), this.loadResources((e) => {
			O.resolve(), D && D(e);
		}), O) : (D && D(), Promise.resolve());
	}
	loadLanguages(E, D) {
		let O = defer();
		isString(E) && (E = [E]);
		let k = this.options.preload || [], A = E.filter((e) => !k.includes(e) && this.services.languageUtils.isSupportedCode(e));
		return A.length ? (this.options.preload = k.concat(A), this.loadResources((e) => {
			O.resolve(), D && D(e);
		}), O) : (D && D(), Promise.resolve());
	}
	dir(e) {
		...
		let T = /* @__PURE__ */ "ar.shu.sqr.ssh.xaa.yhd.yud.aao.abh.abv.acm.acq.acw.acx.acy.adf.ads.aeb.aec.afb.ajp.apc.apd.arb.arq.ars.ary.arz.auz.avl.ayh.ayl.ayn.ayp.bbz.pga.he.iw.ps.pbt.pbu.pst.prp.prd.ug.ur.ydd.yds.yih.ji.yi.hbo.men.xmn.fa.jpr.peo.pes.prs.dv.sam.ckb".split("."), E = this.services?.languageUtils || new LanguageUtil(get());
		return e.toLowerCase().indexOf("-latn") > 1 ? "ltr" : T.includes(E.getLanguagePartFromCode(e)) || e.toLowerCase().indexOf("-arab") > 1 ? "rtl" : "ltr";
	}
	static createInstance(e = {}, T) {
		let D = new E(e, T);
		return D.createInstance = E.createInstance, D;
	}
	cloneInstance(e = {}, T = noop) {
		let D = e.forkResourceStore;
		D && delete e.forkResourceStore;
		let O = {
			...this.options,
			...e,
			isClone: !0
		}, k = new E(O);
		if ((e.debug !== void 0 || e.prefix !== void 0) && (k.logger = k.logger.clone(e)), [
			"store",
			"services",
			"language"
		].forEach((e) => {
			k[e] = this[e];
		}), k.services = { ...this.services }, k.services.utils = { hasLoadedNamespace: k.hasLoadedNamespace.bind(k) }, D && (k.store = new ResourceStore(Object.keys(this.store.data).reduce((e, T) => (e[T] = { ...this.store.data[T] }, e[T] = Object.keys(e[T]).reduce((E, D) => (E[D] = { ...e[T][D] }, E), e[T]), e), {}), O), k.services.resourceStore = k.store), e.interpolation) {
			let T = {
				...get().interpolation,
				...this.options.interpolation,
				...e.interpolation
			...
			k.services.interpolator = new Interpolator(E);
		}
		return k.translator = new Translator(k.services, O), k.translator.on("*", (e, ...T) => {
			k.emit(e, ...T);
		}), k.init(O, T), k.translator.options = O, k.translator.backendConnector.services.utils = { hasLoadedNamespace: k.hasLoadedNamespace.bind(k) }, k;
	}
	toJSON() {
		return {
...
instance.createInstance, instance.dir, instance.init, instance.loadResources, instance.reloadResources, instance.use, instance.changeLanguage, instance.getFixedT, instance.t, instance.exists, instance.setDefaultNamespace, instance.hasLoadedNamespace, instance.loadNamespaces, instance.loadLanguages;
var matchHtmlEntity = /&(?:amp|#38|lt|#60|gt|#62|apos|#39|quot|#34|nbsp|#160|copy|#169|reg|#174|hellip|#8230|#x2F|#47);/g, htmlEntities = {
	"&amp;": "&",
	"&#38;": "&",
	"&lt;": "<",
	"&#60;": "<",
	...
	"&#x2F;": "/",
	"&#47;": "/"
}, unescapeHtmlEntity = (e) => htmlEntities[e], defaultOptions = {
	...
	useSuspense: !0,
	unescape: (e) => e.replace(matchHtmlEntity, unescapeHtmlEntity),
...
var en_runtime_required_default = {
	...
	components: {
		...
		onboarding: { integrations: { capabilities: {
		...
		workspace: { cleanup: {
			browse: {
				...
				selectAll: "Select all matching workspaces",
				selectableOnly: "Only workspaces I can delete now",
				sort: { tier: "Safety" },
				tier: {
					protected: "Protected",
					ready: "Ready",
			...
			scan: {
				...
				readyOneMany: "1 workspace found, with {{value0}} cleanup suggestions.",
	...
	runtimeRpc: { startupFailure: { guidance: {
	...
	settings: { browser: {
		clientHostedRemote: {
			...
			title: "Host remote browser pages on this device"
		},
		sshWorkspaceRouting: {
			...
			enableHost: "Route again",
			probeAgain: "Check again",
			probeSkippedHosts: "Hosts routed without the connection check (Try anyway):",
	...
	worktreeJumpPalette: {
		...
		matchLabel: { automation: "Run" }
	}
}, PSEUDO_LOCALE = "en-XA";
function pseudoLocalizeString(e) {
	return !e || e.startsWith("[") ? e : `[${e}]`;
}
function isPseudoLocalizationLocale(e) {
	return e === PSEUDO_LOCALE;
}
const UI_LANGUAGE_SYSTEM = "system";
var UI_LANGUAGE_VALUES = new Set([
	...
	"es",
	"fr"
]), PLUGIN_UI_LANGUAGE_RE = /^plugin:[a-z0-9]+(?:-[a-z0-9]+)*\.[a-z0-9]+(?:-[a-z0-9]+)*\/[a-z]{2,3}(?:-[a-z0-9]{2,8})*$/i;
function isPluginUiLanguage(e) {
	return typeof e == "string" && PLUGIN_UI_LANGUAGE_RE.test(e);
}
function normalizeUiLanguage(e) {
	return isPluginUiLanguage(e) || UI_LANGUAGE_VALUES.has(e) ? e : UI_LANGUAGE_SYSTEM;
}
...
function normalizeLocaleTag(e) {
	return (e ?? "en").trim().toLowerCase().replace(/_/g, "-");
}
function normalizeSupportedUiLocale(e) {
	let T = normalizeLocaleTag(e), E = T.split("-")[0];
	return E === "zh" ? T.startsWith("zh-tw") || T.startsWith("zh-hk") || T.startsWith("zh-hant") ? "en" : "zh" : SUPPORTED_UI_LOCALES.includes(E) ? E : "en";
}
function resolveUiLocale$1(e, T = "en") {
	return isPluginUiLanguage(e) ? e : e === "en" ? "en" : e === "zh" ? "zh" : e === "ko" ? "ko" : e === "ja" ? "ja" : e === "es" ? "es" : e === "fr" ? "fr" : normalizeSupportedUiLocale(T);
}
function getRendererSystemLocale() {
	return typeof navigator < "u" && navigator.language ? navigator.language : "en";
}
function resolveRendererUiLocale(e) {
	return resolveUiLocale$1(e, e === "system" ? getRendererSystemLocale() : "en");
}
const SHOW_UI_LANGUAGE_SETTING = !0, UI_LANGUAGE_CHOICES = [
	{
	...
	{
		value: "ko",
		labelKey: "settings.appearance.language.korean"
	},
	{
		value: "ja",
		labelKey: "settings.appearance.language.japanese"
	},
	{
		value: "es",
		labelKey: "settings.appearance.language.spanish"
	},
	{
		value: "fr",
		labelKey: "settings.appearance.language.french"
	}
];
var UI_LANGUAGE_CHOICE_FALLBACKS = {
...
function getUiLanguageChoiceLabel(e, T) {
	return T(e.labelKey, UI_LANGUAGE_CHOICE_FALLBACKS[e.value]);
}
function resolveUiLocale(e) {
	return resolveRendererUiLocale(e);
}
const i18n = instance.createInstance();
var NON_DEFAULT_LOCALE_LOADERS = {
	es: () => import("./es-mWQdxiOS.js"),
	fr: () => import("./fr-Cy2iEMIB.js"),
	ja: () => import("./ja-VN4GYiqs.js"),
	ko: () => import("./ko-BUM-zKkb.js"),
	zh: () => import("./zh-B2NoMhnU.js")
};
i18n.use({
	type: "backend",
	init: () => {},
	read: (e, T, E) => {
		let D = NON_DEFAULT_LOCALE_LOADERS[e];
		if (!D) {
			E(null, !1);
			return;
		}
		D().then((e) => E(null, e.default), (e) => E(e instanceof Error ? e : Error(String(e)), !1));
	}
}).use(initReactI18next).init({
	fallbackLng: "en",
	lng: "en",
	partialBundledLanguages: !0,
	resources: { en: { translation: en_runtime_required_default } },
	interpolation: { escapeValue: !1 },
	react: { useSuspense: !1 }
});
function translate(e, T, E) {
	let D = i18n.t(e, {
		defaultValue: T,
		...E
	});
	return isPseudoLocalizationLocale(i18n.language) ? pseudoLocalizeString(D) : D;
}
var registeredPluginLanguages = /* @__PURE__ */ new Set(), pluginLanguagePacks = [];
function getIntlLocale() {
	let e = i18n.language, T = pluginLanguagePacks.find((T) => T.resourceLanguage === e)?.locale ?? e;
	try {
		return Intl.DateTimeFormat.supportedLocalesOf(T)[0] ?? "en";
	} catch {
		return "en";
	}
}
function setRendererPluginLanguagePacks(e) {
	for (let e of registeredPluginLanguages) i18n.removeResourceBundle(e, "translation");
	registeredPluginLanguages.clear(), pluginLanguagePacks = e;
	for (let T of e) i18n.addResourceBundle(T.resourceLanguage, "translation", T.catalog, !0, !0), registeredPluginLanguages.add(T.resourceLanguage);
}
export { SHOW_UI_LANGUAGE_SETTING as a, resolveUiLocale as c, normalizeUiLanguage as d, getI18n as f, translate as i, UI_LANGUAGE_SYSTEM as l, i18n as n, UI_LANGUAGE_CHOICES as o, getDefaults as p, setRendererPluginLanguagePacks as r, getUiLanguageChoiceLabel as s, getIntlLocale as t, isPluginUiLanguage as u };
