...
import { $ as darken_default, b as getConfig, et as lighten_default, f as defaultConfig_default, k as getUserDefinedConfig, nt as is_dark_default, x as getConfig2, z as sanitizeText } from "./chunk-DU6HZSFF-Rlm_otCx.js";
...
var mindmap_default = parser, MAX_SECTIONS = 12, nodeType = {
	...
	static #e = __name(this, "MindmapDB");
	clear() {
	...
	addNode(e, n, r, i) {
		...
		let s = getConfig2(), u = s.mindmap?.padding ?? defaultConfig_default.mindmap.padding;
		switch (i) {
		...
		let d = {
			id: this.count++,
			nodeId: sanitizeText(n, s),
			level: e,
			descr: sanitizeText(r, s),
			type: i,
			children: [],
			width: s.mindmap?.maxNodeWidth ?? defaultConfig_default.mindmap.maxNodeWidth,
			padding: u,
			isRoot: o
		}, f = this.getParent(e);
	...
	flattenNodes(t, n) {
		...
		if (n.push(s), t.children) for (let e of t.children) this.flattenNodes(e, n);
	}
	generateEdges(e, t) {
	...
	getData() {
		let e = this.getMindmap(), n = getConfig2(), r = getUserDefinedConfig().layout !== void 0, i = n;
		if (r || (i.layout = "cose-bilkent"), !e) return {
			nodes: [],
			edges: [],
			config: i
		};
		...
		let a = [], s = [];
		this.flattenNodes(e, a), this.generateEdges(e, s), log.debug(`getData: processed ${a.length} nodes and ${s.length} edges`);
...
}, mindmapRenderer_default = { draw: /* @__PURE__ */ __name(async (e, n, i, o) => {
	log.debug("Rendering mindmap diagram\n" + e);
	let s = o.db, c = s.getData(), l = getDiagramElement(n, c.config.securityLevel);
	if (c.type = o.type, c.layoutAlgorithm = getRegisteredLayoutAlgorithm(c.config.layout, { fallback: "cose-bilkent" }), c.diagramId = n, !s.getMindmap()) return;
	c.nodes.forEach((e) => {
	...
	for (let n = 0; n < e.THEME_COLOR_LIMIT; n++) {
		let i = "" + (r === "neo" ? Math.max(10 - (n - 1) * 2, 2) : 17 - 3 * n);
		a += `
