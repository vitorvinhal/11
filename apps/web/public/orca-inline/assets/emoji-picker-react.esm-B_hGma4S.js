...
function isPsuedoSelector(r) {
	return r.startsWith(":");
}
function isStyleCondition(r) {
	return isString(r) && (r === "*" || r.length > 1 && ":>~.+*".includes(r.slice(0, 1)) || isImmediatePostcondition(r));
}
function isValidProperty(r, M) {
	return (isString(M) || typeof M == "number") && !isCssVariables(r) && !isPsuedoSelector(r) && !isMediaQuery(r);
}
function isMediaQuery(r) {
	return r.startsWith("@media");
}
function isDirectClass(r) {
	return r === ".";
}
function isCssVariables(r) {
	return r === "--";
}
function isString(r) {
	return r + "" === r;
}
function isImmediatePostcondition(r) {
	return isString(r) && (r.startsWith("&") || isPsuedoSelector(r));
}
function joinTruthy(r, M = "") {
	return r.filter(Boolean).join(M);
}
function stableHash(r, M) {
	let N = 0;
	if (M.length === 0) return N.toString();
	for (let r = 0; r < M.length; r++) {
		let P = M.charCodeAt(r);
		N = (N << 5) - N + P, N &= N;
	}
	return `${r ?? "cl"}_${N.toString(36)}`;
}
function handlePropertyValue(r, M) {
	return r === "content" ? `"${M}"` : M;
}
function camelCaseToDash(r) {
	return r.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase();
}
function joinedProperty(r, M) {
	return `${r}:${M}`;
}
function toClass(r) {
	return r ? `.${r}` : "";
}
function appendString(r, M) {
	return r ? `${r}
${M}` : M;
}
var Rule = class r {
	constructor(r, M, N, P) {
		this.sheet = r, this.property = M, this.value = N, this.selector = P, this.property = M, this.value = N, this.joined = joinedProperty(M, N);
		let F = this.selector.preconditions.concat(this.selector.postconditions);
		this.hash = this.selector.hasConditions ? this.selector.scopeClassName : stableHash(this.sheet.name, this.joined), this.key = joinTruthy([
			this.joined,
			F,
			this.hash
		]);
	...
	toString() {
		let M = mergeSelectors(this.selector.preconditions, { right: this.hash });
		return M = mergeSelectors(this.selector.postconditions, { left: M }), `${M} {${r.genRule(this.property, this.value)}}`;
	}
	static genRule(r, M) {
		return joinedProperty(camelCaseToDash(r), handlePropertyValue(r, M)) + ";";
	}
};
function mergeSelectors(r, { left: M = "", right: N = "" } = {}) {
	return joinTruthy([r.reduce((r, M) => isPsuedoSelector(M) ? r + M : isImmediatePostcondition(M) ? r + M.slice(1) : joinTruthy([r, M], " "), M), toClass(N)], " ");
}
var Selector = class r {
	constructor(r, M = null, { preconditions: P, postconditions: F } = {}) {
		this.sheet = r, this.preconditions = [], this.scopeClassName = null, this.scopeName = null, this.postconditions = [], this.preconditions = P ? asArray(P) : [], this.postconditions = F ? asArray(F) : [], this.setScope(M);
	}
	setScope(r) {
		return r && (this.scopeClassName ||= (this.scopeName = r, stableHash(this.sheet.name, r + this.sheet.count))), this;
	}
	get hasConditions() {
		return this.preconditions.length > 0 || this.postconditions.length > 0;
	}
	addScope(M) {
		return new r(this.sheet, M, {
			preconditions: this.preconditions,
			postconditions: this.postconditions
		});
	}
	addPrecondition(M) {
		return new r(this.sheet, this.scopeClassName, {
			postconditions: this.postconditions,
			preconditions: this.preconditions.concat(M)
		});
	}
	addPostcondition(M) {
		return new r(this.sheet, this.scopeClassName, {
			preconditions: this.preconditions,
			postconditions: this.postconditions.concat(M)
		});
	}
	createRule(r, M) {
		return new Rule(this.sheet, r, M, this);
	}
}, Sheet = class {
	constructor(r, M) {
		this.name = r, this.rootNode = M, this.storedStyles = {}, this.storedClasses = {}, this.style = "", this.count = 0, this.id = `flairup-${r}`, this.styleTag = this.createStyleTag();
	}
	getStyle() {
		return this.style;
	}
	append(r) {
		this.style = appendString(this.style, r);
	}
	apply() {
	...
	createStyleTag() {
		...
		return r.type = "text/css", r.id = this.id, (this.rootNode ?? document.head).appendChild(r), r;
	}
	addRule(r) {
		let M = this.storedClasses[r.key];
		return isString(M) ? M : (this.storedClasses[r.key] = r.hash, this.storedStyles[r.hash] = [r.property, r.value], this.append(r.toString()), r.hash);
	}
};
function forIn(r, M) {
	for (let N in r) M(N.trim(), r[N]);
}
function cx(...r) {
	return joinTruthy(r.reduce((r, M) => (M instanceof Set ? r.push(...M) : typeof M == "string" ? r.push(M) : Array.isArray(M) ? r.push(cx(...M)) : typeof M == "object" && Object.entries(M).forEach(([M, N]) => {
		N && r.push(M);
	}), r), []), " ").trim();
}
function createSheet(r, M) {
	let N = new Sheet(r, M);
	return {
		create: P,
	...
	function P(r) {
		let M = {};
		return iteratePreconditions(N, r, new Selector(N)).forEach(([r, M, F]) => {
			iterateStyles(N, M, F).forEach((M) => {
				P(r, M);
			});
		}), N.apply(), M;
		function P(r, N) {
			M[r] = M[r] ?? /* @__PURE__ */ new Set(), M[r].add(N);
		}
	}
}
function iteratePreconditions(r, M, N) {
	let P = [];
	return forIn(M, (I, L) => {
		if (isStyleCondition(I)) return iteratePreconditions(r, L, N.addPrecondition(I)).forEach((r) => P.push(r));
		P.push([
			I,
			M[I],
			N.addScope(I)
...
function iterateStyles(r, M, P) {
	let B = /* @__PURE__ */ new Set();
	return forIn(M, (M, V) => {
		let H = [];
		if (isStyleCondition(M)) H = iterateStyles(r, V, P.addPostcondition(M));
		else if (isDirectClass(M)) H = asArray(V);
		else if (isMediaQuery(M)) H = handleMediaQuery(r, V, M, P);
		else if (isCssVariables(M)) H = cssVariablesBlock(r, V, P);
		else if (isValidProperty(M, V)) {
			let N = P.createRule(M, V);
			r.addRule(N), B.add(N.hash);
		}
		return addEachClass(H, B);
	}), B;
}
function addEachClass(r, M) {
	return r.forEach((r) => M.add(r)), M;
}
function cssVariablesBlock(r, M, N) {
	let P = /* @__PURE__ */ new Set(), F = [];
	if (forIn(M, (M, L) => {
		if (isValidProperty(M, L)) {
			F.push(Rule.genRule(M, L));
			return;
		}
		addEachClass(iterateStyles(r, L ?? {}, N), P);
	}), !N.scopeClassName) return P;
	if (F.length) {
		let M = F.join(" ");
		r.append(`${mergeSelectors(N.preconditions, { right: N.scopeClassName })} {${M}}`);
	}
	return P.add(N.scopeClassName), P;
}
function handleMediaQuery(r, M, N, P) {
	r.append(N + " {");
	let F = iterateStyles(r, M, P);
	return r.append("}"), F;
}
var import_react = /* @__PURE__ */ __toESM(require_react());
function _arrayLikeToArray(r, M) {
	(M == null || M > r.length) && (M = r.length);
	for (var N = 0, P = Array(M); N < M; N++) P[N] = r[N];
	return P;
}
function _createForOfIteratorHelperLoose(r, M) {
	var N = typeof Symbol < "u" && r[Symbol.iterator] || r["@@iterator"];
	if (N) return (N = N.call(r)).next.bind(N);
	if (Array.isArray(r) || (N = _unsupportedIterableToArray(r)) || M && r && typeof r.length == "number") {
		N && (r = N);
		var P = 0;
		return function() {
			return P >= r.length ? { done: !0 } : {
				done: !1,
				value: r[P++]
			};
		};
...
function _inheritsLoose(r, M) {
	r.prototype = Object.create(M.prototype), r.prototype.constructor = r, _setPrototypeOf(r, M);
}
function _objectWithoutPropertiesLoose(r, M) {
	if (r == null) return {};
	var N = {};
	for (var P in r) if ({}.hasOwnProperty.call(r, P)) {
		if (M.indexOf(P) !== -1) continue;
		N[P] = r[P];
	}
	return N;
}
function _setPrototypeOf(r, M) {
	return _setPrototypeOf = Object.setPrototypeOf ? Object.setPrototypeOf.bind() : function(r, M) {
		return r.__proto__ = M, r;
	}, _setPrototypeOf(r, M);
}
function _unsupportedIterableToArray(r, M) {
	if (r) {
		if (typeof r == "string") return _arrayLikeToArray(r, M);
		var N = {}.toString.call(r).slice(8, -1);
		return N === "Object" && r.constructor && (N = r.constructor.name), N === "Map" || N === "Set" ? Array.from(r) : N === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(N) ? _arrayLikeToArray(r, M) : void 0;
	}
}
var ClassNames;
(function(r) {
	r.hiddenOnSearch = "epr-hidden-on-search", r.searchActive = "epr-search-active", r.hidden = "epr-hidden", r.visible = "epr-visible", r.active = "epr-active", r.emoji = "epr-emoji", r.category = "epr-emoji-category", r.label = "epr-emoji-category-label", r.categoryContent = "epr-emoji-category-content", r.emojiHasVariations = "epr-emoji-has-variations", r.scrollBody = "epr-body", r.emojiList = "epr-emoji-list", r.external = "__EmojiPicker__", r.emojiPicker = "EmojiPickerReact", r.open = "epr-open", r.vertical = "epr-vertical", r.horizontal = "epr-horizontal", r.variationPicker = "epr-emoji-variation-picker", r.darkTheme = "epr-dark-theme", r.autoTheme = "epr-auto-theme";
})(ClassNames ||= {});
function asSelectors() {
	return [...arguments].map(function(r) {
		return "." + r;
	}).join("");
}
var stylesheet = /* @__PURE__ */ createSheet("epr", null), hidden = {
	display: "none",
	opacity: "0",
	pointerEvents: "none",
	visibility: "hidden",
	overflow: "hidden"
}, commonStyles = /* @__PURE__ */ stylesheet.create({ hidden: /* @__PURE__ */ _extends({ ".": ClassNames.hidden }, hidden) }), PickerStyleTag = /* @__PURE__ */ (0, import_react.memo)(function(r) {
	var M = r.nonce;
...
}), commonInteractionStyles = /* @__PURE__ */ stylesheet.create({
	".epr-main": {
...
function darkMode(r, M) {
	var N, P;
	return {
		".epr-dark-theme": (N = {}, N[r] = M, N),
		".epr-auto-theme": (P = {}, P[r] = { "@media (prefers-color-scheme: dark)": M }, P)
	};
}
function compareConfig(r, M) {
	var N = r.customEmojis ?? [], P = M.customEmojis ?? [];
	return r.open === M.open && r.emojiVersion === M.emojiVersion && r.reactionsDefaultOpen === M.reactionsDefaultOpen && r.searchPlaceHolder === M.searchPlaceHolder && r.searchPlaceholder === M.searchPlaceholder && r.searchClearButtonLabel === M.searchClearButtonLabel && r.defaultSkinTone === M.defaultSkinTone && r.skinTonesDisabled === M.skinTonesDisabled && r.autoFocusSearch === M.autoFocusSearch && r.emojiStyle === M.emojiStyle && r.theme === M.theme && r.suggestedEmojisMode === M.suggestedEmojisMode && r.lazyLoadEmojis === M.lazyLoadEmojis && r.className === M.className && r.height === M.height && r.width === M.width && r.style === M.style && r.searchDisabled === M.searchDisabled && r.skinTonePickerLocation === M.skinTonePickerLocation && N.length === P.length && r.emojiData === M.emojiData;
}
...
(function(r) {
	r.RECENT = "recent", r.FREQUENT = "frequent";
})(SuggestionMode ||= {});
var EmojiStyle;
(function(r) {
	r.NATIVE = "native", r.APPLE = "apple", r.TWITTER = "twitter", r.GOOGLE = "google", r.FACEBOOK = "facebook";
})(EmojiStyle ||= {});
var Theme;
(function(r) {
	r.DARK = "dark", r.LIGHT = "light", r.AUTO = "auto";
})(Theme ||= {});
var SkinTones;
(function(r) {
	r.NEUTRAL = "neutral", r.LIGHT = "1f3fb", r.MEDIUM_LIGHT = "1f3fc", r.MEDIUM = "1f3fd", r.MEDIUM_DARK = "1f3fe", r.DARK = "1f3ff";
})(SkinTones ||= {});
var Categories;
(function(r) {
	r.SUGGESTED = "suggested", r.CUSTOM = "custom", r.SMILEYS_PEOPLE = "smileys_people", r.ANIMALS_NATURE = "animals_nature", r.FOOD_DRINK = "food_drink", r.TRAVEL_PLACES = "travel_places", r.ACTIVITIES = "activities", r.OBJECTS = "objects", r.SYMBOLS = "symbols", r.FLAGS = "flags";
})(Categories ||= {});
var SkinTonePickerLocation;
(function(r) {
	r.SEARCH = "SEARCH", r.PREVIEW = "PREVIEW";
})(SkinTonePickerLocation ||= {});
var CDN_URL_APPLE = "https://cdn.jsdelivr.net/npm/emoji-datasource-apple/img/apple/64/", CDN_URL_FACEBOOK = "https://cdn.jsdelivr.net/npm/emoji-datasource-facebook/img/facebook/64/", CDN_URL_TWITTER = "https://cdn.jsdelivr.net/npm/emoji-datasource-twitter/img/twitter/64/", CDN_URL_GOOGLE = "https://cdn.jsdelivr.net/npm/emoji-datasource-google/img/google/64/";
function cdnUrl(r) {
	switch (r) {
...
var skinToneVariations = [
	...
	SkinTones.DARK
], skinTonesNamed = /* @__PURE__ */ Object.entries(SkinTones).reduce(function(r, M) {
	var N = M[0], P = M[1];
	return r[P] = N, r;
}, {}), skinTonesMapped = /* @__PURE__ */ skinToneVariations.reduce(function(r, M) {
	var N;
	return Object.assign(r, (N = {}, N[M] = M, N));
}, {}), EmojiProperties;
(function(r) {
	r.name = "n", r.unified = "u", r.variations = "v", r.added_in = "a", r.imgUrl = "imgUrl";
})(EmojiProperties ||= {});
function emojiNames(r) {
	return r[EmojiProperties.name] ?? [];
}
function addedIn(r) {
	return parseFloat(r[EmojiProperties.added_in] || "0");
}
function emojiName(r) {
	if (!r) return "";
	var M = emojiNames(r);
	return M[M.length - 1];
}
function unifiedWithoutSkinTone(r) {
	var M = r.split("-");
	return skinTonesMapped[M.splice(1, 1)[0]] ? M.join("-") : r;
}
function emojiUnified(r, M) {
	var N = r[EmojiProperties.unified];
	return !M || !emojiHasVariations(r) ? N : emojiVariationUnified(r, M) ?? N;
}
function emojiUrlByUnified(r, M) {
	return "" + cdnUrl(M) + r + ".png";
}
function emojiVariations(r) {
	return r[EmojiProperties.variations] ?? [];
}
function emojiHasVariations(r) {
	return emojiVariations(r).length > 0;
}
function emojiVariationUnified(r, M) {
	return M ? emojiVariations(r).find(function(r) {
		return r.includes(M);
	}) : emojiUnified(r);
}
function activeVariationFromUnified(r) {
	var M = r.split("-")[1];
	return Object.keys(skinTonesMapped).includes(M) ? M : null;
}
var _configByCategory, categoriesOrdered = [
...
}, configByCategory = (_configByCategory = {}, _configByCategory[Categories.SUGGESTED] = {
	category: Categories.SUGGESTED,
...
function baseCategoriesConfig(r) {
	return categoriesOrdered.map(function(M) {
		return _extends({}, configByCategory[M], r && r[M] && r[M]);
	});
}
function categoryFromCategoryConfig(r) {
	return r.category;
}
function categoryNameFromCategoryConfig(r) {
	return r.name;
}
function mergeCategoriesConfig(r, M, N) {
	var P;
	r === void 0 && (r = []), M === void 0 && (M = {});
	var F = function() {
		var r = N != null && N.categories ? Object.fromEntries(Object.entries(N.categories).filter(function(r) {
			return !!r[1];
		})) : {};
		if (M.suggestionMode === SuggestionMode.RECENT) {
			var P = N?.categories?.suggested_recent;
			r[Categories.SUGGESTED] = P ? {
				category: Categories.SUGGESTED,
				name: P.name
			} : SuggestedRecent;
		}
		return r;
	}(), I = baseCategoriesConfig(F);
	return (P = r) != null && P.length ? r.map(function(r) {
		return typeof r == "string" ? getBaseConfigByCategory(r, F[r]) : _extends({}, getBaseConfigByCategory(r.category, F[r.category]), r);
	}) : I;
}
function getBaseConfigByCategory(r, M) {
	return M === void 0 && (M = {}), Object.assign(configByCategory[r], M);
}
var KNOWN_FAILING_EMOJIS = [
	"2640-fe0f",
	"2642-fe0f",
	"2695-fe0f"
], DEFAULT_SEARCH_PLACEHOLDER = "Search", DEFAULT_SEARCH_CLEAR_BUTTON_LABEL = "Clear", SEARCH_RESULTS_NO_RESULTS_FOUND = "No results found", SEARCH_RESULTS_SUFFIX = " found. Use up and down arrow keys to navigate.", SEARCH_RESULTS_ONE_RESULT_FOUND = "1 result" + SEARCH_RESULTS_SUFFIX, SEARCH_RESULTS_MULTIPLE_RESULTS_FOUND = "%n results" + SEARCH_RESULTS_SUFFIX;
function mergeConfig(r) {
	var M, N, P;
	r === void 0 && (r = {});
	var F = basePickerConfig(), I = (M = r.emojiData) == null || (N = M.categories) == null ? void 0 : N.preview_mood?.name, L = _extends({}, F.previewConfig, I && !((P = r.previewConfig) != null && P.defaultCaption) ? { defaultCaption: I } : {}, r.previewConfig ?? {}), R = Object.assign(F, r), z = mergeCategoriesConfig(r.categories, { suggestionMode: R.suggestedEmojisMode }, r.emojiData);
	R.hiddenEmojis.forEach(function(r) {
		R.unicodeToHide.add(r);
	});
	var B = R.searchDisabled ? SkinTonePickerLocation.PREVIEW : R.skinTonePickerLocation;
	return _extends({}, R, {
		categories: z,
		previewConfig: L,
		skinTonePickerLocation: B
	});
}
...
var basePreviewConfig = {
	...
	showPreview: !0
}, _excluded = ["children"], ConfigContext = /* @__PURE__ */ (0, import_react.createContext)(/* @__PURE__ */ basePickerConfig());
function PickerConfigProvider(r) {
	var M = r.children, N = useSetConfig(_objectWithoutPropertiesLoose(r, _excluded));
	return (0, import_react.createElement)(ConfigContext.Provider, { value: N }, M);
}
function useSetConfig(r) {
	var M = (0, import_react.useState)(function() {
		return mergeConfig(r);
	}), N = M[0], P = M[1];
	return (0, import_react.useEffect)(function() {
		compareConfig(N, r) || P(mergeConfig(r));
	}, [
		r.customEmojis?.length,
		r.open,
		r.emojiVersion,
		r.reactionsDefaultOpen,
		r.searchPlaceHolder,
		r.searchPlaceholder,
		r.searchClearButtonLabel,
		r.defaultSkinTone,
		r.skinTonesDisabled,
		r.autoFocusSearch,
		r.emojiStyle,
		r.theme,
		r.suggestedEmojisMode,
		r.lazyLoadEmojis,
		r.className,
		r.height,
		r.width,
		r.searchDisabled,
		r.skinTonePickerLocation,
		r.allowExpandReactions,
		r.emojiData
	]), N;
}
...
function useDebouncedState(r, M) {
	M === void 0 && (M = 0);
	var N = (0, import_react.useState)(r), P = N[0], F = N[1], I = (0, import_react.useRef)(null);
	function L(r) {
		return new Promise(function(N) {
			I.current && clearTimeout(I.current), I.current = window?.setTimeout(function() {
				F(r), N(r);
			}, M);
		});
	}
...
}, SUGGESTED_LS_KEY = "epr_suggested";
function getSuggested(r) {
	try {
		var M;
		if (!((M = window) != null && M.localStorage)) return [];
		var N = JSON.parse(window?.localStorage.getItem(SUGGESTED_LS_KEY) ?? "[]");
		return r === SuggestionMode.FREQUENT ? N.sort(function(r, M) {
			return M.count - r.count;
		}) : N;
	} catch {
		return [];
	}
}
function setSuggested(r, M) {
	var N = getSuggested(), P = emojiUnified$1(r, M), F = emojiUnified$1(r), I = N.find(function(r) {
		return r.unified === P;
	}), L;
	I ? L = [I].concat(N.filter(function(r) {
		return r !== I;
	})) : (I = {
		unified: P,
		original: F,
		count: 0
	}, L = [I].concat(N)), I.count++, L.length = Math.min(L.length, 14);
	try {
		var R;
		(R = window) == null || R.localStorage.setItem(SUGGESTED_LS_KEY, JSON.stringify(L));
	} catch {}
}
function emojiNames$1(r) {
	return r[EmojiProperties.name] ?? [];
}
function emojiName$1(r) {
	if (!r) return "";
	var M = emojiNames$1(r);
	return M[M.length - 1];
}
function unifiedWithoutSkinTone$1(r) {
	var M = r.split("-");
	return skinTonesMapped[M.splice(1, 1)[0]] ? M.join("-") : r;
}
function emojiUnified$1(r, M) {
	var N = r[EmojiProperties.unified];
	return !M || !emojiHasVariations$1(r) ? N : emojiVariationUnified$1(r, M) ?? N;
}
function useGetEmojisByCategory() {
	var r = useSuggestedEmojisModeConfig(), M = useUpdateSuggested()[0], N = import_react.useMemo(function() {
		return (getSuggested(r) ?? []).map(function(r) {
			return emojiByUnified(r.unified);
		}).filter(Boolean);
	}, [M, r]);
	return function(r) {
		return r === Categories.SUGGESTED ? N : defaultEmojiData.emojis[r] ?? [];
	};
}
function emojiVariations$1(r) {
	return r[EmojiProperties.variations] ?? [];
}
function emojiHasVariations$1(r) {
	return emojiVariations$1(r).length > 0;
}
function emojiVariationUnified$1(r, M) {
	return M ? emojiVariations$1(r).find(function(r) {
		return r.includes(M);
	}) : emojiUnified$1(r);
}
function emojiByUnified(r) {
	if (r) return allEmojisByUnified[r] ? allEmojisByUnified[r] : allEmojisByUnified[unifiedWithoutSkinTone$1(r)];
}
var allEmojis = /* @__PURE__ */ Object.values(defaultEmojiData.emojis).flat(), allEmojisByUnified = {};
allEmojis.reduce(function(r, M) {
	return r[emojiUnified$1(M)] = M, emojiHasVariations$1(M) && emojiVariations$1(M).forEach(function(N) {
		r[N] = M;
	}), r;
}, allEmojisByUnified);
function activeVariationFromUnified$1(r) {
	var M = r.split("-")[1];
	return skinToneVariations.includes(M) ? M : null;
}
...
function PickerDataProvider(r) {
	var M = r.children, N = usePickerConfig(), P = N.customEmojis, F = N.emojiData, I = import_react.useMemo(function() {
		var r = F || defaultEmojiData, M = JSON.parse(JSON.stringify(r));
		P && P.length > 0 && (M.emojis[Categories.CUSTOM] = P.map(customToRegularEmoji));
		var N = M.emojis || {}, I = Object.values(N).flat(), L = {}, R = {};
		return I.forEach(function(r) {
			var M = r[EmojiProperties.unified];
			if (L[M] = r, r[EmojiProperties.variations]) {
				var N;
				(N = r[EmojiProperties.variations]) == null || N.forEach(function(M) {
					L[M] = r;
				});
			}
			(r[EmojiProperties.name] || []).join("").toLowerCase().split("").forEach(function(N) {
				R[N] = R[N] ?? {}, R[N][M] = r;
			});
		}), {
			emojiData: M,
			allEmojis: I,
			allEmojisByUnified: L,
			searchIndex: R
		};
	}, [F, P]), L = import_react.useCallback(function(r) {
		if (r) return I.allEmojisByUnified[r] ?? I.allEmojisByUnified[unifiedWithoutSkinTone(r)];
	}, [I.allEmojisByUnified]);
	return import_react.createElement(PickerDataContext.Provider, { value: _extends({}, I, {
		emojiByUnified: L,
...
function useGetEmojisByCategory$1() {
	var r = usePickerDataContext(), M = r.emojiData, N = r.emojiByUnified, P = useSuggestedEmojisModeConfig(), F = useUpdateSuggested()[0], I = import_react.useMemo(function() {
		return (getSuggested(P) ?? []).map(function(r) {
			var M, P = N(r.unified);
			if (P) return _extends({}, P, (M = {}, M[EmojiProperties.unified] = r.unified, M));
		}).filter(Boolean);
	}, [
		F,
		P,
	...
	return function(r) {
		return r === Categories.SUGGESTED ? I : M.emojis?.[r] ?? [];
	};
}
function customToRegularEmoji(r) {
	var M;
	return M = {}, M[EmojiProperties.name] = r.names.map(function(r) {
		return r.toLowerCase();
	}), M[EmojiProperties.unified] = r.id.toLowerCase(), M[EmojiProperties.added_in] = "0", M[EmojiProperties.imgUrl] = r.imgUrl, M;
}
function useIsUnicodeHidden() {
	var r = useUnicodeToHide();
	return function(M) {
		return r.has(M);
	};
}
function useDisallowedEmojis() {
	var r = (0, import_react.useRef)({}), M = useEmojiVersionConfig(), N = usePickerDataContext().allEmojis;
	return (0, import_react.useMemo)(function() {
		var P = parseFloat("" + M);
		return !M || Number.isNaN(P) ? r.current : N.reduce(function(r, M) {
			return addedInNewerVersion(M, P) && (r[emojiUnified(M)] = !0), r;
		}, r.current);
	}, [M, N]);
}
function useIsEmojiDisallowed() {
	var r = useDisallowedEmojis(), M = useIsUnicodeHidden();
	return function(N) {
		var P = unifiedWithoutSkinTone(emojiUnified(N));
		return !!(r[P] || M(P));
	};
}
function addedInNewerVersion(r, M) {
	return addedIn(r) > M;
}
function useMarkInitialLoad(r) {
	(0, import_react.useEffect)(function() {
		r(!0);
	}, [r]);
}
function PickerContextProvider(r) {
	var M = r.children, N = useDisallowedEmojis(), P = useDefaultSkinToneConfig(), F = useReactionsOpenConfig(), I = usePickerDataContext().searchIndex, L = (0, import_react.useRef)(I);
	(0, import_react.useEffect)(function() {
		L.current = I;
	}, [I]);
	var R = (0, import_react.useRef)(!1), z = (0, import_react.useRef)(!1), B = (0, import_react.useRef)(N), V = useDebouncedState(Date.now(), 200), H = useDebouncedState("", 100), U = (0, import_react.useState)(!1), W = (0, import_react.useState)(P), G = (0, import_react.useState)(null), K = (0, import_react.useState)(/* @__PURE__ */ new Set()), q = (0, import_react.useState)(null), J = (0, import_react.useState)(F), Y = (0, import_react.useState)(!1), X = Y[0], Z = Y[1], Q = (0, import_react.useState)([]), $ = (0, import_react.useState)(null);
	return useMarkInitialLoad(Z), (0, import_react.createElement)(PickerContext.Provider, { value: {
		activeCategoryState: G,
		activeSkinTone: W,
		disallowClickRef: R,
		disallowMouseRef: z,
		disallowedEmojisRef: B,
		emojiVariationPickerState: q,
		emojisThatFailedToLoadState: K,
		filterRef: L,
		isPastInitialLoad: X,
		searchTerm: H,
		skinToneFanOpenState: U,
		suggestedUpdateState: V,
		reactionsModeState: J,
		visibleCategoriesState: Q,
		emojiSizeState: $
	} }, M);
}
...
function useUpdateSuggested() {
	var r = (0, import_react.useContext)(PickerContext).suggestedUpdateState, M = r[0], N = r[1];
	return [M, function() {
		N(Date.now());
	}];
}
...
function useDefineMutableConfig(r) {
	var M = import_react.useRef({
		onEmojiClick: r.onEmojiClick || emptyFunc,
		onReactionClick: r.onReactionClick || r.onEmojiClick,
		onSkinToneChange: r.onSkinToneChange || emptyFunc
	});
	return import_react.useEffect(function() {
		M.current.onEmojiClick = r.onEmojiClick || emptyFunc, M.current.onReactionClick = r.onReactionClick || r.onEmojiClick;
	}, [r.onEmojiClick, r.onReactionClick]), import_react.useEffect(function() {
		M.current.onSkinToneChange = r.onSkinToneChange || emptyFunc;
	}, [r.onSkinToneChange]), M;
}
function emptyFunc() {}
var MOUSE_EVENT_SOURCE;
(function(r) {
	r.REACTIONS = "reactions", r.PICKER = "picker";
})(MOUSE_EVENT_SOURCE ||= {});
function useSearchPlaceHolderConfig() {
	var r = usePickerConfig();
	return [r.searchPlaceHolder, r.searchPlaceholder].find(function(r) {
		return r !== DEFAULT_SEARCH_PLACEHOLDER;
	}) ?? DEFAULT_SEARCH_PLACEHOLDER;
}
function useSearchClearButtonLabelConfig() {
	return usePickerConfig().searchClearButtonLabel ?? DEFAULT_SEARCH_CLEAR_BUTTON_LABEL;
}
...
function useOnEmojiClickConfig(r) {
	var M = useMutableConfig().current, N = useReactionsModeState()[1], P = M.onEmojiClick || function() {}, F = M.onReactionClick;
	return r === MOUSE_EVENT_SOURCE.REACTIONS && F ? function() {
		var r = [...arguments];
		return F.apply(void 0, r.concat([{ collapseToReactions: function() {
			N(function(r) {
				return r;
			});
		} }]));
		...
		var r = [...arguments];
		P.apply(void 0, r.concat([{ collapseToReactions: function() {
			N(!0);
		} }]));
	};
}
...
function useStyleConfig() {
	var r = usePickerConfig(), M = r.height, N = r.width, P = r.style;
	return _extends({
		height: getDimension(M),
		width: getDimension(N)
	}, P);
}
...
function getDimension(r) {
	return typeof r == "number" ? r + "px" : r;
}
function useSearchResultsConfig(r) {
	return r > 0 ? r > 1 ? SEARCH_RESULTS_MULTIPLE_RESULTS_FOUND.replace("%n", r.toString()) : SEARCH_RESULTS_ONE_RESULT_FOUND : SEARCH_RESULTS_NO_RESULTS_FOUND;
}
...
function focusPrevElementSibling(r) {
	if (r) {
		var M = r.previousElementSibling;
		focusElement(M);
	}
}
function focusNextElementSibling(r) {
	if (r) {
		var M = r.nextElementSibling;
		focusElement(M);
	}
}
function focusFirstElementChild(r) {
	if (r) {
...
function ElementRefContextProvider(r) {
	var M = r.children, N = (0, import_react.useRef)(null), P = (0, import_react.useRef)(null), F = (0, import_react.useRef)(null), I = (0, import_react.useRef)(null), L = (0, import_react.useRef)(null), R = (0, import_react.useRef)(null), z = (0, import_react.useRef)(null), B = (0, import_react.useRef)(null), V = (0, import_react.useRef)(null);
	return (0, import_react.createElement)(ElementRefContext.Provider, { value: {
		AnchoredEmojiRef: P,
		BodyRef: F,
		EmojiListRef: I,
		CategoryNavigationRef: z,
		PickerMainRef: N,
		SearchInputRef: L,
		SkinTonePickerRef: R,
		VariationPickerRef: B,
		ReactionsRef: V
	} }, M);
}
...
function useSetAnchoredEmojiRef() {
	var r = useAnchoredEmojiRef();
	return function(M) {
		M === null && r.current !== null && focusElement(r.current), r.current = M;
	};
}
function useBodyRef() {
...
function scrollTo(r, M) {
	M === void 0 && (M = 0);
	var N = queryScrollBody(r);
	N && requestAnimationFrame(function() {
		N.scrollTop = M;
	});
}
function scrollBy(r, M) {
	var N = queryScrollBody(r);
	N && requestAnimationFrame(function() {
		N.scrollTop += M;
	});
...
function useScrollTo() {
	var r = useBodyRef();
	return (0, import_react.useCallback)(function(M) {
		requestAnimationFrame(function() {
			r.current && (r.current.scrollTop = M);
		});
	}, [r]);
}
function scrollEmojiAboveLabel(r) {
	if (!(!r || !isEmojiBehindLabel(r)) && !r.closest(asSelectors(ClassNames.variationPicker))) {
		var M = closestScrollBody(r), N = emojiDistanceFromScrollTop(r);
		scrollBy(M, -(categoryLabelHeight(closestCategory(r)) - N));
	}
}
function focusFirstVisibleEmoji(r) {
	var M = firstVisibleEmoji(r);
	focusElement(M), scrollEmojiAboveLabel(M);
}
function focusAndClickFirstVisibleEmoji(r) {
	var M = firstVisibleEmoji(r);
	focusElement(M), M?.click();
}
function focusLastVisibleEmoji(r) {
	focusElement(lastVisibleEmoji(r));
}
function focusNextVisibleEmoji(r) {
	if (r) {
		var M = nextVisibleEmoji(r);
		if (!M) return focusFirstVisibleEmoji(nextCategory(r));
		focusElement(M), scrollEmojiAboveLabel(M);
	}
}
function focusPrevVisibleEmoji(r) {
	if (r) {
		var M = prevVisibleEmoji(r);
		if (!M) return focusLastVisibleEmoji(prevCategory(r));
		focusElement(M), scrollEmojiAboveLabel(M);
	}
}
function focusVisibleEmojiOneRowUp(r, M) {
	if (r) {
		var N = visibleEmojiOneRowUp(r);
		if (!N) return M();
		focusElement(N), scrollEmojiAboveLabel(N);
	}
}
function focusVisibleEmojiOneRowDown(r) {
	if (r) return focusElement(visibleEmojiOneRowDown(r));
}
function visibleEmojiOneRowUp(r) {
	if (!r) return null;
	var M = closestCategoryContent(r), N = closestCategory(M), P = elementCountInRow(M, r), F = allVisibleEmojis(N), I = F.indexOf(r), L = I % P;
	if (I === -1) return null;
	if (F[I - P]) return F[I - P];
	var R = prevCategory(N);
	if (!R) return null;
	var z = allVisibleEmojis(R);
	if (L > z.length % P - 1) return z.at(-1);
	for (var B = z.length - 1; B >= 0; B--) if (B % P === L) return z[B];
	return z.at(-1);
}
function visibleEmojiOneRowDown(r) {
	if (!r) return null;
	var M = closestCategoryContent(r), N = closestCategory(M), P = elementCountInRow(M, r), F = allVisibleEmojis(N), I = F.indexOf(r);
	if (I === -1) return null;
	if (F[I + (P - I % P - 1) + 1]) {
		for (var L = I + P; L % P >= 0; L--) if (F[L]) return F[L];
	}
	var R = I % P, z = allVisibleEmojis(nextCategory(N));
	return z[R] ? z[R] : z.at(0) ?? null;
}
function useCloseAllOpenToggles() {
	var r = useEmojiVariationPickerState(), M = r[0], N = r[1], P = useSkinToneFanOpenState(), F = P[0], I = P[1];
...
function useHasOpenToggles() {
	var r = useEmojiVariationPickerState()[0], M = useSkinToneFanOpenState()[0];
	return function() {
		return !!r || M;
	};
}
function useDisallowMouseMove() {
	var r = useDisallowMouseRef();
	return function() {
		r.current = !0;
	};
}
...
function useOnMouseMove() {
	var r = useBodyRef(), M = useAllowMouseMove(), N = useIsMouseDisallowed();
	(0, import_react.useEffect)(function() {
		var P = r.current;
		P?.addEventListener("mousemove", F, { passive: !0 });
		function F() {
			N() && M();
		}
		return function() {
...
function useFocusSearchInput() {
	var r = useSearchInputRef();
	return (0, import_react.useCallback)(function() {
		focusElement(r.current);
	}, [r]);
}
function useFocusSkinTonePicker() {
	var r = useSkinTonePickerRef();
	return (0, import_react.useCallback)(function() {
		r.current && focusFirstElementChild(r.current);
	}, [r]);
}
function useFocusCategoryNavigation() {
	var r = useCategoryNavigationRef();
	return (0, import_react.useCallback)(function() {
		r.current && focusFirstElementChild(r.current);
	}, [r]);
}
function useSetFilterRef() {
	var r = useFilterRef();
	return function M(N) {
		if (typeof N == "function") return M(N(r.current));
		r.current = N;
	};
}
function useClearSearch() {
	var r = useApplySearch(), M = useSearchInputRef(), N = useFocusSearchInput();
	return function() {
		M.current && (M.current.value = ""), r(""), N();
	};
}
function useAppendSearch() {
	var r = useSearchInputRef(), M = useApplySearch();
	return function(N) {
		r.current ? (r.current.value = "" + r.current.value + N, M(getNormalizedSearchTerm(r.current.value))) : M(getNormalizedSearchTerm(N));
	};
}
function useFilter() {
	var r = useSearchInputRef(), M = useFilterRef(), N = useSetFilterRef(), P = useApplySearch(), F = useSearchTermState()[0];
	return {
		onChange: I,
		searchTerm: F,
		SearchInputRef: r,
		statusSearchResults: getStatusSearchResults(M.current, F)
	};
	function I(r) {
		var F = M.current, I = r.toLowerCase();
		if (F != null && F[I] || I.length <= 1) return P(I);
		var L = findLongestMatch(I, F);
		if (!L) return P(I);
		N(function(r) {
			var M;
			return Object.assign(r, (M = {}, M[I] = filterEmojiObjectByKeyword(L, I), M));
		}), P(I);
	}
}
function useApplySearch() {
	var r = useSearchTermState()[1], M = usePickerMainRef();
	return function(N) {
		requestAnimationFrame(function() {
			r(N && N?.toLowerCase()).then(function() {
				scrollTo(M.current, 0);
			});
		});
	};
}
function filterEmojiObjectByKeyword(r, M) {
	var N = {};
	for (var P in r) {
		var F = r[P];
		hasMatch(F, M) && (N[P] = F);
...
function hasMatch(r, M) {
	return emojiNames(r).some(function(r) {
		return r.includes(M);
	});
}
function useIsEmojiFiltered() {
	var r = useFilterRef().current, M = useSearchTermState()[0];
	return function(N) {
		return isEmojiFilteredBySearchTerm(N, r, M);
	};
}
function isEmojiFilteredBySearchTerm(r, M, N) {
	var P;
	return !M || !N ? !1 : !((P = M[N]) != null && P[r]);
}
function findLongestMatch(r, M) {
	if (!M) return null;
	if (M[r]) return M[r];
	var N = Object.keys(M).sort(function(r, M) {
...
function getNormalizedSearchTerm(r) {
	return !r || typeof r != "string" ? "" : r.trim().toLowerCase();
}
function getStatusSearchResults(r, M) {
	return r != null && r[M] ? useSearchResultsConfig(Object.entries(r?.[M])?.length || 0) : "";
}
function useSetVariationPicker() {
	var r = useSetAnchoredEmojiRef(), M = useEmojiVariationPickerState()[1];
	return function(N) {
		var P = emojiFromElement(N)[0];
		P && (r(N), M(P));
	};
}
...
var KeyboardEvents;
(function(r) {
	r.ArrowDown = "ArrowDown", r.ArrowUp = "ArrowUp", r.ArrowLeft = "ArrowLeft", r.ArrowRight = "ArrowRight", r.Escape = "Escape", r.Enter = "Enter", r.Space = " ";
})(KeyboardEvents ||= {});
function useKeyboardNavigation() {
	usePickerMainKeyboardEvents(), useSearchInputKeyboardEvents(), useSkinTonePickerKeyboardEvents(), useCategoryNavigationKeyboardEvents(), useBodyKeyboardEvents();
}
function usePickerMainKeyboardEvents() {
	var r = usePickerMainRef(), M = useClearSearch(), N = useScrollTo(), P = useSearchInputRef(), F = useFocusSearchInput(), I = useHasOpenToggles(), L = useDisallowMouseMove(), R = useCloseAllOpenToggles(), z = (0, import_react.useMemo)(function() {
		return function(r) {
			var P = r.key;
			switch (L(), P) {
	...
	(0, import_react.useEffect)(function() {
		var M = r.current;
		if (M) return M.addEventListener("keydown", z), function() {
			M.removeEventListener("keydown", z);
		};
	}, [
		r,
		P,
...
function useSearchInputKeyboardEvents() {
	var r = useFocusSkinTonePicker(), M = usePickerMainRef(), N = useBodyRef(), P = useSearchInputRef(), F = useSkinToneFanOpenState()[1], I = useGoDownFromSearchInput(), L = useIsSkinToneInSearch(), R = (0, import_react.useMemo)(function() {
		return function(M) {
			switch (M.key) {
				case KeyboardEvents.ArrowRight:
					if (!L) return;
					M.preventDefault(), F(!0), r();
					break;
				case KeyboardEvents.ArrowDown:
					M.preventDefault(), I();
					break;
				case KeyboardEvents.Enter:
					M.preventDefault(), focusAndClickFirstVisibleEmoji(N.current);
					break;
			}
		};
	}, [
	...
	(0, import_react.useEffect)(function() {
		var r = P.current;
		if (r) return r.addEventListener("keydown", R), function() {
			r.removeEventListener("keydown", R);
		};
	}, [
		M,
		P,
...
function useSkinTonePickerKeyboardEvents() {
	var r = useSkinTonePickerRef(), M = useFocusSearchInput(), N = useSearchInputRef(), P = useGoDownFromSearchInput(), F = useSkinToneFanOpenState(), I = F[0], L = F[1], R = useIsSkinToneInPreview(), z = useIsSkinToneInSearch(), B = useOnType(), V = (0, import_react.useMemo)(function() {
		return (function(r) {
			var N = r.key;
			if (z) switch (N) {
				case KeyboardEvents.ArrowLeft:
					if (r.preventDefault(), !I) return M();
					focusNextSkinTone(M);
					break;
				case KeyboardEvents.ArrowRight:
					if (r.preventDefault(), !I) return M();
	...
	(0, import_react.useEffect)(function() {
		var M = r.current;
		if (M) return M.addEventListener("keydown", V), function() {
...
function useCategoryNavigationKeyboardEvents() {
	var r = useFocusSearchInput(), M = useCategoryNavigationRef(), N = useBodyRef(), P = useOnType(), F = (0, import_react.useMemo)(function() {
		return function(M) {
			switch (M.key) {
				case KeyboardEvents.ArrowUp:
					M.preventDefault(), r();
					break;
				case KeyboardEvents.ArrowRight:
					M.preventDefault(), focusNextElementSibling(getActiveElement());
					break;
				case KeyboardEvents.ArrowLeft:
					M.preventDefault(), focusPrevElementSibling(getActiveElement());
					break;
				case KeyboardEvents.ArrowDown:
					M.preventDefault(), focusFirstVisibleEmoji(N.current);
					break;
				default:
					P(M);
					break;
	...
	(0, import_react.useEffect)(function() {
		var r = M.current;
		if (r) return r.addEventListener("keydown", F), function() {
...
function useBodyKeyboardEvents() {
	var r = useBodyRef(), M = useGoUpFromBody(), N = useSetVariationPicker(), P = useHasOpenToggles(), F = useCloseAllOpenToggles(), I = useOnType(), L = (0, import_react.useMemo)(function() {
		return (function(r) {
			var L = r.key, R = buttonFromTarget(getActiveElement());
			switch (L) {
				case KeyboardEvents.ArrowRight:
					r.preventDefault(), focusNextVisibleEmoji(R);
					break;
				case KeyboardEvents.ArrowLeft:
					r.preventDefault(), focusPrevVisibleEmoji(R);
					break;
				case KeyboardEvents.ArrowDown:
					if (r.preventDefault(), P()) {
						F();
						break;
					}
					focusVisibleEmojiOneRowDown(R);
					break;
				case KeyboardEvents.ArrowUp:
					if (r.preventDefault(), P()) {
						F();
					...
					focusVisibleEmojiOneRowUp(R, M);
					break;
				case KeyboardEvents.Space:
					r.preventDefault(), N(r.target);
					break;
				default:
					I(r);
					break;
	...
	(0, import_react.useEffect)(function() {
		var M = r.current;
		if (M) return M.addEventListener("keydown", L), function() {
			M.removeEventListener("keydown", L);
		};
	}, [r, L]);
}
function useGoDownFromSearchInput() {
	var r = useFocusCategoryNavigation(), M = useIsSearchMode(), N = useBodyRef();
	return (0, import_react.useCallback)(function() {
		return M ? focusFirstVisibleEmoji(N.current) : r();
	}, [
		N,
		r,
		M
...
function useGoUpFromBody() {
	var r = useFocusSearchInput(), M = useFocusCategoryNavigation(), N = useIsSearchMode();
	return (0, import_react.useCallback)(function() {
		return N ? r() : M();
	}, [
		r,
...
function focusNextSkinTone(r) {
	var M = getActiveElement();
	M && (hasNextElementSibling(M) || r(), focusNextElementSibling(M));
}
function focusPrevSkinTone() {
	var r = getActiveElement();
	r && focusPrevElementSibling(r);
}
function useOnType() {
	var r = useAppendSearch(), M = useFocusSearchInput(), N = useSearchDisabledConfig(), P = useCloseAllOpenToggles();
	return function(F) {
		var I = F.key;
		hasModifier(F) || N || I.match(/(^[a-zA-Z0-9]$){1}/) && (F.preventDefault(), P(), M(), r(I));
	};
}
function hasModifier(r) {
	var M = r.metaKey, N = r.ctrlKey, P = r.altKey;
	return M || N || P;
}
function preloadEmojiIfNeeded(r, M, N, P, F, I, L, R) {
	if (r && M !== EmojiStyle.NATIVE) {
		var z = emojiUnified(r);
		preloadedEmojs.has(z) || !I || !L || setTimeout(function() {
			var z = F + I.top, B = N + P;
			z >= B && z < B + L.emojiSize * 2 && preloadEmoji(R, r, M);
		});
	}
}
function preloadEmoji(r, M, N) {
	if (M) {
		var P = emojiUnified(M);
		preloadedEmojs.has(P) || (preloadedEmojs.add(P), emojiVariations(M).concat(P).forEach(function(M) {
			preloadImage(r(M, N));
		}));
	}
}
...
function useOnFocus() {
	var r = useBodyRef(), M = useEmojiStyleConfig(), N = useGetEmojiUrlConfig();
	(0, import_react.useEffect)(function() {
		if (M === EmojiStyle.NATIVE) return;
		var P = r.current;
		return P?.addEventListener("focusin", F), function() {
			P?.removeEventListener("focusin", F);
		};
		function F(r) {
			var P = buttonFromTarget(r.target);
			if (P) {
				var F = emojiFromElement(P)[0];
				F && emojiHasVariations$1(F) && preloadEmoji(N, F, M);
			}
		}
	}, [
		r,
...
var _excluded$1 = ["width", "height"], DEFAULT_LABEL_HEIGHT = 40;
function PickerMain(r) {
	var M = r.children;
	return (0, import_react.createElement)(PickerContextProvider, null, (0, import_react.createElement)(PickerRootElement, null, M));
}
function PickerRootElement(r) {
	var M, N = r.children, P = useReactionsModeState()[0], F = useThemeConfig(), I = useIsSearchMode(), L = usePickerMainRef(), R = useClassNameConfig(), z = useStyleConfig();
	useKeyboardNavigation(), useOnFocus();
	var B = z || {}, V = B.width, H = B.height, U = _objectWithoutPropertiesLoose(B, _excluded$1);
	return (0, import_react.createElement)("aside", {
		className: cx(styles.main, styles.baseVariables, F === Theme.DARK && styles.darkTheme, F === Theme.AUTO && styles.autoThemeDark, (M = {}, M[ClassNames.searchActive] = I, M), P && styles.reactionsMenu, R),
		ref: L,
		style: _extends({}, U, !P && {
			height: H,
			width: V
...
}, styles = /* @__PURE__ */ stylesheet.create({
	main: {
...
function elementCountInRow(r, M) {
	if (!r || !M) return 0;
	var N = r.getBoundingClientRect().width, P = M.getBoundingClientRect().width;
	return Math.floor(N / P);
}
function firstVisibleElementInContainer(r, M, N) {
	if (N === void 0 && (N = 0), !r || !M.length) return null;
	var P = r.getBoundingClientRect().top, F = r.getBoundingClientRect().bottom, I = P + getLabelHeight(r);
	return M.find(function(r) {
		var M = r.getBoundingClientRect().top, L = r.getBoundingClientRect().bottom, R = r.clientHeight * N, z = M + R, B = L - R;
		return z < I ? !1 : z >= P && z <= F || B >= P && B <= F;
	}) || null;
}
function hasNextElementSibling(r) {
	return !!r.nextElementSibling;
}
function getLabelHeight(r) {
	if (!r) return DEFAULT_LABEL_HEIGHT;
	var M = r.querySelector(asSelectors(ClassNames.label));
	if (M) {
		var N = M.getBoundingClientRect().height;
		if (N > 0) return N;
	}
	return DEFAULT_LABEL_HEIGHT;
}
var EmojiButtonSelector = "button" + /* @__PURE__ */ asSelectors(ClassNames.emoji), VisibleEmojiSelector = /* @__PURE__ */ [
	EmojiButtonSelector,
	asSelectors(ClassNames.visible),
	":not(" + asSelectors(ClassNames.hidden) + ")"
].join("");
function buttonFromTarget(r) {
	return r?.closest(EmojiButtonSelector) ?? null;
}
function emojiFromElement(r) {
	var M = originalUnifiedFromEmojiElement(r), N = unifiedFromEmojiElement(r);
	if (!M) return [];
	var P = emojiByUnified(N ?? M);
	return P ? [P, N] : [];
}
function isEmojiElement(r) {
	return !!(r?.matches(EmojiButtonSelector) || r != null && r.parentElement?.matches(EmojiButtonSelector));
}
function elementHeight(r) {
	return r?.clientHeight ?? 0;
}
function emojiTrueOffsetTop(r) {
	if (!r) return 0;
	var M = buttonFromTarget(r), N = closestCategory(M), P = categoryLabelHeight(N);
	return elementOffsetTop(M) + elementOffsetTop(N) + P;
}
function categoryLabelHeight(r) {
	if (!r) return 0;
	var M = r.querySelector(asSelectors(ClassNames.categoryContent));
	return (r?.clientHeight ?? 0) - (M?.clientHeight ?? 0);
}
function isEmojiBehindLabel(r) {
	return r ? emojiDistanceFromScrollTop(r) < categoryLabelHeight(closestCategory(r)) : !1;
}
function queryScrollBody(r) {
	return r ? r.matches(asSelectors(ClassNames.scrollBody)) ? r : r.querySelector(asSelectors(ClassNames.scrollBody)) : null;
}
function emojiDistanceFromScrollTop(r) {
	return r ? emojiTrueOffsetTop(r) - (closestScrollBody(r)?.scrollTop ?? 0) : 0;
}
function closestScrollBody(r) {
	return r ? r.closest(asSelectors(ClassNames.scrollBody)) ?? null : null;
}
function emojiTruOffsetLeft(r) {
	var M = buttonFromTarget(r), N = closestCategory(M);
	return elementOffsetLeft(M) + elementOffsetLeft(N);
}
function elementOffsetTop(r) {
	return r?.offsetTop ?? 0;
}
function elementOffsetLeft(r) {
	return r?.offsetLeft ?? 0;
}
function unifiedFromEmojiElement(r) {
	return elementDataSetKey(buttonFromTarget(r), "unified") ?? null;
}
function originalUnifiedFromEmojiElement(r) {
	var M = unifiedFromEmojiElement(r);
	return M ? unifiedWithoutSkinTone$1(M) : null;
}
function allUnifiedFromEmojiElement(r) {
	return r ? {
		unified: unifiedFromEmojiElement(r),
		originalUnified: originalUnifiedFromEmojiElement(r)
	} : {
		unified: null,
		originalUnified: null
	};
}
function elementDataSetKey(r, M) {
	return elementDataSet(r)[M] ?? null;
}
function elementDataSet(r) {
	return r?.dataset ?? {};
}
function isVisibleEmoji(r) {
	return r.classList.contains(ClassNames.visible);
}
function isHidden(r) {
	return r ? r.classList.contains(ClassNames.hidden) : !0;
}
function allVisibleEmojis(r) {
	return r ? Array.from(r.querySelectorAll(VisibleEmojiSelector)) : [];
}
function lastVisibleEmoji(r) {
	if (!r) return null;
	var M = allVisibleEmojis(r).slice(-1)[0];
	return M ? isVisibleEmoji(M) ? M : prevVisibleEmoji(M) : null;
}
function nextVisibleEmoji(r) {
	var M = r.nextElementSibling;
	return M ? isVisibleEmoji(M) ? M : nextVisibleEmoji(M) : firstVisibleEmoji(nextCategory(r));
}
function prevVisibleEmoji(r) {
	var M = r.previousElementSibling;
	return M ? isVisibleEmoji(M) ? M : prevVisibleEmoji(M) : lastVisibleEmoji(prevCategory(r));
}
function firstVisibleEmoji(r) {
	return r ? firstVisibleElementInContainer(r, allVisibleEmojis(r), .1) : null;
}
function prevCategory(r) {
	var M = closestCategory(r);
	if (!M) return null;
	var N = M.previousElementSibling;
	return N ? isHidden(N) ? prevCategory(N) : N : null;
}
function nextCategory(r) {
	var M = closestCategory(r);
	if (!M) return null;
	var N = M.nextElementSibling;
	return N ? isHidden(N) ? nextCategory(N) : N : null;
}
function closestCategory(r) {
	return r ? r.closest(asSelectors(ClassNames.category)) : null;
}
function closestCategoryContent(r) {
	return r ? r.closest(asSelectors(ClassNames.categoryContent)) : null;
}
function parseNativeEmoji(r) {
	return r.split("-").map(function(r) {
		return String.fromCodePoint(parseInt(r, 16));
	}).join("");
}
function isCustomCategory(r) {
	return r.category === Categories.CUSTOM;
}
function isCustomEmoji(r) {
	return r.imgUrl !== void 0;
}
function useMouseDownHandlers(r, M) {
	var N = (0, import_react.useRef)(), P = useSetVariationPicker(), F = useDisallowClickRef(), I = useEmojiVariationPickerState()[1], L = useCloseAllOpenToggles(), R = useActiveSkinToneState()[0], z = useOnEmojiClickConfig(M), B = useUpdateSuggested()[1], V = useGetEmojiUrlConfig(), H = useEmojiStyleConfig(), U = usePickerDataContext().emojiByUnified, W = (0, import_react.useCallback)(function(r) {
		if (!F.current) {
			L();
			var M = emojiFromEvent(r, U), N = M[0], P = M[1];
			if (!(!N || !P)) {
				var I = activeVariationFromUnified$1(P) || R;
				B(), setSuggested(N, I), z(emojiClickOutput(N, I, H, V), r);
			}
		}
	}, [
		R,
		...
		V,
		H
	]), G = (0, import_react.useCallback)(function(r) {
		N.current && clearTimeout(N.current);
		var M = emojiFromEvent(r, U)[0];
		!M || !emojiHasVariations$1(M) || (N.current = window?.setTimeout(function() {
			F.current = !0, N.current = void 0, L(), P(r.target), I(M);
		}, 500));
	}, [
		F,
		U,
		...
		P,
		I
	]), K = (0, import_react.useCallback)(function() {
		N.current ? (clearTimeout(N.current), N.current = void 0) : F.current && requestAnimationFrame(function() {
			F.current = !1;
		});
	}, [F]);
	(0, import_react.useEffect)(function() {
		if (r.current) {
			var M = r.current;
			return M.addEventListener("click", W, { passive: !0 }), M.addEventListener("mousedown", G, { passive: !0 }), M.addEventListener("mouseup", K, { passive: !0 }), function() {
				M?.removeEventListener("click", W), M?.removeEventListener("mousedown", G), M?.removeEventListener("mouseup", K);
			};
		}
	}, [
		r,
...
function emojiFromEvent(r, M) {
	var N = r?.target;
	if (!isEmojiElement(N)) return [];
	var P = allUnifiedFromEmojiElement(N), F = P.unified, I = P.originalUnified, L = F ?? I;
	if (!L) return [];
	var R = M(L);
	return R ? [R, F ?? L] : [];
}
function emojiClickOutput(r, M, N, P) {
	var F = emojiNames$1(r);
	if (isCustomEmoji(r)) {
		var I = emojiUnified$1(r);
		return {
			activeSkinTone: M,
			emoji: I,
	...
	var L = emojiUnified$1(r, M);
	return {
		activeSkinTone: M,
		emoji: parseNativeEmoji(L),
		getImageUrl: function(r) {
			return r === void 0 && (r = N ?? EmojiStyle.APPLE), P(L, r);
		},
		imageUrl: P(L, N ?? EmojiStyle.APPLE),
		isCustom: !1,
		names: F,
		unified: L,
		unifiedWithoutSkinTone: emojiUnified$1(r)
	};
}
function Button(r) {
	return (0, import_react.createElement)("button", Object.assign({ type: "button" }, r, { className: cx(styles$1.button, r.className) }), r.children);
}
...
function ClickableEmojiButton(r) {
	var M, N = r.emojiNames, P = r.unified, F = r.hidden, I = r.hiddenOnSearch, L = r.showVariations, R = L === void 0 ? !0 : L, z = r.hasVariations, B = r.children, V = r.className, H = r.noBackground, U = H === void 0 ? !1 : H, W = r.style;
	return (0, import_react.createElement)(Button, {
		className: cx(styles$2.emoji, F && commonStyles.hidden, I && commonInteractionStyles.hiddenOnSearch, (M = {}, M[ClassNames.visible] = !F && !I, M), !!(z && R) && styles$2.hasVariations, U && styles$2.noBackground, V),
		"data-unified": P,
...
}), emojiStyles = /* @__PURE__ */ stylesheet.create({
	external: {
		".": ClassNames.external,
		fontSize: "0"
	},
...
function EmojiImg(r) {
	var M = r.emojiName, N = r.style, P = r.lazyLoad, F = P === void 0 ? !1 : P, I = r.imgUrl, L = r.onError, R = r.className;
	return (0, import_react.createElement)("img", {
		src: I,
		alt: M,
		className: cx(styles$3.emojiImag, emojiStyles.external, emojiStyles.common, R),
...
function NativeEmoji(r) {
	var M = r.unified, N = r.style, P = r.className;
	return (0, import_react.createElement)("span", {
		className: cx(styles$4.nativeEmoji, emojiStyles.common, emojiStyles.external, P),
		"data-unified": M,
		style: N
	}, parseNativeEmoji(M));
}
...
function ViewOnlyEmoji(r) {
	var M = r.emoji, N = r.unified, P = r.emojiStyle, F = r.size, I = r.lazyLoad, L = r.getEmojiUrl, R = L === void 0 ? emojiUrlByUnified : L, z = r.className, B = useEmojisThatFailedToLoadState()[1], V = usePickerDataContext().emojiByUnified, H = {};
	F && (H.width = H.height = H.fontSize = F + "px");
	var U = M || V(N);
	if (!U) return null;
	if (isCustomEmoji(U)) return (0, import_react.createElement)(EmojiImg, {
		style: H,
		emojiName: N,
	...
	return (0, import_react.createElement)(import_react.Fragment, null, P === EmojiStyle.NATIVE ? (0, import_react.createElement)(NativeEmoji, {
		unified: N,
		style: H,
		className: z
	}) : (0, import_react.createElement)(EmojiImg, {
		style: H,
		emojiName: emojiName(U),
		emojiStyle: P,
		lazyLoad: I,
		imgUrl: R(N, P),
		onError: W,
	...
	function W() {
		B(function(r) {
			return new Set(r).add(N);
		});
	}
}
function ClickableEmoji(r) {
	var M = r.emoji, N = r.unified, P = r.hidden, F = r.hiddenOnSearch, I = r.emojiStyle, L = r.showVariations, R = L === void 0 ? !0 : L, z = r.size, B = r.lazyLoad, V = r.getEmojiUrl, H = r.className, U = r.noBackground, W = U === void 0 ? !1 : U, G = r.style;
	return (0, import_react.createElement)(ClickableEmojiButton, {
		hasVariations: emojiHasVariations$1(M),
		showVariations: R,
		hidden: P,
		hiddenOnSearch: F,
		emojiNames: emojiNames$1(M),
		unified: N,
		noBackground: W,
		style: G
...
function Reactions() {
	var r = useReactionsModeState()[0], M = useReactionsRef(), N = useReactionsConfig();
	useMouseDownHandlers(M, MOUSE_EVENT_SOURCE.REACTIONS);
	var P = useEmojiStyleConfig(), F = useAllowExpandReactions(), I = useGetEmojiUrlConfig();
	return r ? (0, import_react.createElement)("ul", {
		className: cx(styles$6.list, !r && commonStyles.hidden),
		ref: M,
		"aria-label": "Reactions"
	}, N.map(function(r) {
		var M = emojiByUnified(r);
		return M ? (0, import_react.createElement)("li", { key: r }, (0, import_react.createElement)(ClickableEmoji, {
			emoji: M,
			emojiStyle: P,
			unified: r,
			showVariations: !1,
	...
	}), F ? (0, import_react.createElement)("li", null, (0, import_react.createElement)(BtnPlus, null)) : null) : null;
}
...
function useOnScroll(r) {
	var M = useCloseAllOpenToggles(), N = (0, import_react.useState)(0), P = N[0], F = N[1];
	return (0, import_react.useEffect)(function() {
		var N = r.current;
		if (!N) return;
		N.addEventListener("scroll", P, { passive: !0 });
		function P() {
			F(N?.scrollTop ?? 0), M();
		}
		return function() {
			N?.removeEventListener("scroll", P);
		};
	}, [r, M]), P;
}
function shouldVirtualize(r) {
	var M = r.scrollTop, N = r.clientHeight, P = r.topOffset, F = r.style, I = r.dimensions;
	if (!F || !I) return !1;
	var L = P + F.top;
	return !(L + I.emojiSize + I.emojiSize * 2 >= M && L <= M + N + I.emojiSize);
}
function getEmojiPositionStyle(r, M) {
	return r ? {
		top: Math.floor(M / r.emojisPerRow) * r.emojiSize,
		left: M % r.emojisPerRow * r.emojiSize
	} : void 0;
}
var EMOJI_SIZE_DEFAULT = 40;
function useCategoryHeight(r) {
	var M = useEmojiListRef(), N = useReactionsModeState()[0], P = usePickerMainRef(), F = (0, import_react.useRef)(), I = useVisibleCategoriesState()[0], L = useEmojiSizeState()[0], R = (0, import_react.useState)(), z = R[0], B = R[1], V = (0, import_react.useCallback)(function() {
		var N = M.current;
		if (N) {
			var P = N.querySelector(EmojiButtonSelector)?.clientHeight;
			P && (F.current = P);
			var I = L || P || F.current || EMOJI_SIZE_DEFAULT, R = N.clientWidth;
			if (!(R === 0 || I === 0)) {
				var z = Math.max(1, Math.floor(R / I));
				B({
					categoryHeight: Math.ceil(r / z) * I,
					emojisPerRow: z,
					emojiSize: I
				});
			}
	...
	return (0, import_react.useEffect)(function() {
		...
		if (r) {
			var M = function(r) {
				var M = r.propertyName;
				(M === "width" || M === "max-width" || M === "min-width" || M === "height" || M === "max-height" || M === "min-height") && (typeof queueMicrotask == "function" ? queueMicrotask(function() {
					return V();
				}) : requestAnimationFrame(function() {
					return V();
				}));
			};
			return r.addEventListener("transitionend", M, { passive: !0 }), function() {
				r.removeEventListener("transitionend", M);
			};
		}
	}, [P, V]), z;
}
function useIsEmojiHidden() {
	var r = useEmojisThatFailedToLoadState()[0], M = useIsEmojiFiltered();
	return function(N) {
		var P = emojiUnified(N), F = r.has(P), I = M(P);
		return {
			failedToLoad: F,
			filteredOut: I,
			hidden: F || I
...
function useEmojiVirtualization(r) {
	var M = r.categoryEmojis, N = r.topOffset, P = r.onHeightReady, F = r.scrollTop, I = r.isCategoryVisible, L = useIsEmojiHidden(), R = useLazyLoadEmojisConfig(), z = useEmojiStyleConfig(), B = useActiveSkinToneState()[0], V = useIsEmojiDisallowed(), H = useGetEmojiUrlConfig(), U = !useSkinTonesDisabledConfig(), W = useBodyRef(), G = 0, K = M.filter(function(r) {
		var M = V(r), N = L(r), P = N.failedToLoad, F = N.filteredOut, I = N.hidden;
		return !P && !F && !I && !M;
	}), q = useCategoryHeight(K.length);
	(0, import_react.useEffect)(function() {
		q && P(q.categoryHeight);
	}, [
		q,
		P,
		K.length
	...
	var J = function(r) {
		return q && W.current && shouldVirtualize({
			scrollTop: F,
			clientHeight: W.current?.clientHeight ?? 0,
			topOffset: N,
			style: r,
			dimensions: q
		});
	}, Y = K.reduce(function(r, M, P) {
		var L = emojiUnified(M, B), V = getEmojiPositionStyle(q, P);
		return J(V) ? (G++, preloadEmojiIfNeeded(M, z, F, W.current?.clientHeight ?? 0, N, V, q, H), r) : I ? (r.push((0, import_react.createElement)(ClickableEmoji, {
			showVariations: U,
			key: L,
			emoji: M,
			unified: L,
			...
			lazyLoad: R,
			getEmojiUrl: H,
			style: _extends({}, V, { position: "absolute" })
		})), r) : (G++, r);
	}, []);
	return {
...
function EmojiCategory(r) {
	var M = r.categoryConfig, N = r.children, P = r.hidden, F = r.hiddenOnSearch, I = r.height, L = categoryFromCategoryConfig(M), R = categoryNameFromCategoryConfig(M);
	return (0, import_react.createElement)("li", {
		className: cx(styles$7.category, P && commonStyles.hidden, F && commonInteractionStyles.hiddenOnSearch),
		"data-name": L,
		"aria-label": R
	}, (0, import_react.createElement)("h2", { className: cx(styles$7.label) }, R), (0, import_react.createElement)("div", {
...
function MeasureEmoji() {
	var r = useCategoriesConfig(), M = useGetEmojisByCategory(), N = useEmojiStyleConfig(), P = useGetEmojiUrlConfig(), F = useLazyLoadEmojisConfig(), I = useActiveSkinToneState()[0], L = useEmojiSizeState(), R = L[0], z = L[1], B = (0, import_react.useRef)(null);
	if ((0, import_react.useLayoutEffect)(function() {
		B.current && z(B.current.clientHeight);
	}), R) return null;
	var V = r[0], H = M(categoryFromCategoryConfig(V))[0], U = H ? emojiUnified$1(H, I) : "";
	return H ? (0, import_react.createElement)("div", { ref: B }, (0, import_react.createElement)(ClickableEmoji, {
		emoji: H,
		unified: U,
		emojiStyle: N,
		getEmojiUrl: P,
...
function EmojiList(r) {
	var M = r.scrollTop, N = useCategoriesConfig(), P = (0, import_react.useState)({}), F = P[0], I = P[1], L = useEmojiListRef(), R = useGetEmojisByCategory$1(), z = getLabelHeight(L.current), B = 0;
	return (0, import_react.createElement)("ul", {
		className: cx(styles$8.emojiList),
		ref: L
	}, (0, import_react.createElement)(MeasureEmoji, null), N.map(function(r) {
		var N = categoryFromCategoryConfig(r), P = B, L = F[N];
		return L && (B += L + z), (0, import_react.createElement)(import_react.Suspense, { key: N }, (0, import_react.createElement)(RenderCategory, {
			categoryEmojis: R(N),
			categoryConfig: r,
			topOffset: P,
			onHeightReady: function(r) {
				F[N] !== r && I(function(M) {
					var P;
					return _extends({}, M, (P = {}, P[N] = r, P));
				});
			},
			scrollTop: M
		}));
...
function RenderCategory(r) {
	var M = r.categoryEmojis, N = r.categoryConfig, P = r.topOffset, F = r.onHeightReady, I = r.scrollTop, L = useVisibleCategoriesState()[0], R = useEmojiVirtualization({
		categoryEmojis: M,
		topOffset: P,
		onHeightReady: F,
		scrollTop: I,
		isCategoryVisible: L.includes(N.category)
	}), z = R.virtualizedCounter, B = R.emojis, V = R.dimensions;
	return (0, import_react.createElement)(EmojiCategory, {
		categoryConfig: N,
		height: V?.categoryHeight,
		hidden: !B.length && z === 0
	}, B);
}
var styles$8 = /* @__PURE__ */ stylesheet.create({ emojiList: {
	...
	margin: "0",
	padding: "0"
} }), SVGTriangle = "data:image/svg+xml;base64,", Direction;
(function(r) {
	r[r.Up = 0] = "Up", r[r.Down = 1] = "Down";
})(Direction ||= {});
function EmojiVariationPicker() {
	var r = useAnchoredEmojiRef(), M = useVariationPickerRef(), N = useEmojiVariationPickerState()[0], P = useEmojiStyleConfig(), F = useVariationPickerTop(M), I = F.getTop, L = F.getMenuDirection, R = useSetAnchoredEmojiRef(), z = usePointerStyle(M), B = useGetEmojiUrlConfig(), V = buttonFromTarget(r.current), H = !!(N && V && emojiHasVariations$1(N) && V.classList.contains(ClassNames.emojiHasVariations));
	(0, import_react.useEffect)(function() {
		H && focusFirstVisibleEmoji(M.current);
	}, [
		M,
		H,
		r
	...
	var U, W;
	return !H && r.current ? R(null) : (U = I(), W = z()), (0, import_react.createElement)("div", {
		ref: M,
		className: cx(styles$9.variationPicker, L() === Direction.Down && styles$9.pointingUp, H && styles$9.visible),
		style: { top: U }
	}, H && N ? [emojiUnified$1(N)].concat(emojiVariations$1(N)).slice(0, 6).map(function(r) {
...
function usePointerStyle(r) {
	var M = useAnchoredEmojiRef();
	return function() {
		var N = {};
		if (!r.current) return N;
		if (M.current) {
			var P = buttonFromTarget(M.current), F = emojiTruOffsetLeft(P);
			if (!P) return N;
			N.left = F + P?.clientWidth / 2;
		}
		return N;
	};
}
function useVariationPickerTop(r) {
	var M = useAnchoredEmojiRef(), N = useBodyRef(), P = Direction.Up;
	return {
		getMenuDirection: F,
		getTop: I
	};
	...
	function I() {
		...
		var I = elementHeight(r.current);
		if (M.current) {
			var L = N.current, R = buttonFromTarget(M.current), z = elementHeight(R);
			F = emojiTrueOffsetTop(R), (L?.scrollTop ?? 0) > F - I && (P = Direction.Down, F += z + I);
		}
		return F - I;
	}
}
...
function Body() {
	var r = useBodyRef(), M = useOnScroll(r);
	return useMouseDownHandlers(r, MOUSE_EVENT_SOURCE.PICKER), useOnMouseMove(), (0, import_react.createElement)("div", {
		className: cx(styles$a.body, commonInteractionStyles.hiddenOnReactions),
		ref: r
	}, (0, import_react.createElement)(EmojiVariationPicker, null), (0, import_react.createElement)(EmojiList, { scrollTop: M }));
}
...
function detectEmojyPartiallyBelowFold(r, M) {
	if (!r || !M) return 0;
	var N = r.getBoundingClientRect(), P = M.getBoundingClientRect();
	return P.height - (N.y - P.y);
}
function useEmojiPreviewEvents(r, M) {
	var N = useBodyRef(), P = useIsMouseDisallowed();
	(0, import_react.useEffect)(function() {
		if (!r) return;
		var F = N.current;
		F?.addEventListener("keydown", R, { passive: !0 }), F?.addEventListener("mouseover", z, !0), F?.addEventListener("focus", I, !0), F?.addEventListener("mouseout", L, { passive: !0 }), F?.addEventListener("blur", L, !0);
		function I(r) {
			var N = buttonFromTarget(r.target);
			if (!N) return L();
			var P = allUnifiedFromEmojiElement(N), F = P.unified, I = P.originalUnified;
			if (!F || !I) return L();
			M({
				unified: F,
				originalUnified: I
		...
		function L(r) {
			if (r) {
				var N = r.relatedTarget;
				if (!buttonFromTarget(N)) return M(null);
			}
			M(null);
		}
		function R(r) {
		...
		function z(r) {
			if (!P()) {
				var N = buttonFromTarget(r.target);
				if (N) {
					if (detectEmojyPartiallyBelowFold(N, F) < N.getBoundingClientRect().height) return handlePartiallyVisibleElementFocus(N, M);
					focusElement(N);
				}
			}
		}
		return function() {
			F?.removeEventListener("mouseover", z), F?.removeEventListener("mouseout", L), F?.removeEventListener("focus", I, !0), F?.removeEventListener("blur", L, !0), F?.removeEventListener("keydown", R);
		};
	}, [
		N,
		r,
...
function handlePartiallyVisibleElementFocus(r, M) {
	var N, P = allUnifiedFromEmojiElement(r), F = P.unified, I = P.originalUnified;
	!F || !I || ((N = document.activeElement) == null || N.blur == null || N.blur(), M({
		unified: F,
		originalUnified: I
	}));
}
var _stylesheet$create, FlexDirection;
(function(r) {
	r.ROW = "FlexRow", r.COLUMN = "FlexColumn";
})(FlexDirection ||= {});
function Flex(r) {
	var M = r.children, N = r.className, P = r.style, F = P === void 0 ? {} : P, I = r.direction, L = I === void 0 ? FlexDirection.ROW : I;
	return (0, import_react.createElement)("div", {
		style: _extends({}, F),
		className: cx(styles$b.flex, N, styles$b[L])
	}, M);
}
...
function Space(r) {
	var M = r.className, N = r.style;
	return (0, import_react.createElement)("div", {
		style: _extends({ flex: 1 }, N === void 0 ? {} : N),
		className: cx(M)
	});
}
function Absolute(r) {
	var M = r.children, N = r.className, P = r.style;
	return (0, import_react.createElement)("div", {
		style: _extends({}, P, { position: "absolute" }),
		className: N
	}, M);
}
function Relative(r) {
	var M = r.children, N = r.className, P = r.style;
	return (0, import_react.createElement)("div", {
		style: _extends({}, P, { position: "relative" }),
		className: N
	}, M);
}
function BtnSkinToneVariation(r) {
	var M = r.isOpen, N = r.onClick, P = r.isActive, F = r.skinToneVariation, I = r.style;
	return (0, import_react.createElement)(Button, {
		style: I,
		onClick: N,
		className: cx("epr-tone-" + F, styles$c.tone, !M && styles$c.closedTone, P && styles$c.active),
		"aria-pressed": P,
		"aria-label": "Skin tone " + skinTonesNamed[F]
	});
}
...
function SkinTonePicker(r) {
	var M = r.direction, N = M === void 0 ? SkinTonePickerDirection.HORIZONTAL : M, P = useSkinTonePickerRef(), F = useSkinTonesDisabledConfig(), I = useSkinToneFanOpenState(), L = I[0], R = I[1], z = useActiveSkinToneState(), B = z[0], V = z[1], H = useOnSkinToneChangeConfig(), U = useCloseAllOpenToggles(), W = useFocusSearchInput();
	if (F) return null;
	var G = ITEM_SIZE * skinToneVariations.length + "px", K = L ? G : ITEM_SIZE + "px", q = N === SkinTonePickerDirection.VERTICAL;
	return (0, import_react.createElement)(Relative, {
		className: cx(styles$d.skinTones, q && styles$d.vertical, L && styles$d.open, q && L && styles$d.verticalShadow),
		style: q ? {
			flexBasis: K,
			height: K
		} : { flexBasis: K }
		...
		ref: P
	}, skinToneVariations.map(function(r, M) {
		var N = r === B;
		return (0, import_react.createElement)(BtnSkinToneVariation, {
			key: r,
			skinToneVariation: r,
			isOpen: L,
			style: { transform: cx(q ? "translateY(-" + M * (L ? ITEM_SIZE : 0) + "px)" : "translateX(-" + M * (L ? ITEM_SIZE : 0) + "px)", L && N && "scale(1.3)") },
			isActive: N,
			onClick: function() {
				L ? (V(r), H(r), W()) : R(!0), U();
			}
...
(function(r) {
	r.VERTICAL = "epr-vertical", r.HORIZONTAL = "epr-horizontal";
...
function Preview() {
	var r = usePreviewConfig(), M = useIsSkinToneInPreview(), N = useReactionsModeState()[0];
	return r.showPreview ? (0, import_react.createElement)(Flex, { className: cx(styles$e.preview, commonInteractionStyles.hiddenOnReactions, N && styles$e.hideOnReactions) }, (0, import_react.createElement)(PreviewBody, null), (0, import_react.createElement)(Space, null), M ? (0, import_react.createElement)(SkinTonePickerMenu, null) : null) : null;
}
function PreviewBody() {
	var r = usePreviewConfig(), M = (0, import_react.useState)(null), N = M[0], P = M[1], F = useEmojiStyleConfig(), I = useEmojiVariationPickerState()[0], L = useGetEmojiUrlConfig(), R = usePickerDataContext().emojiByUnified;
	useEmojiPreviewEvents(r.showPreview, P);
	var z = R(N?.unified ?? N?.originalUnified), B = z != null && N != null;
	return (0, import_react.createElement)(V, null);
	function V() {
		var M = I ?? R(r.defaultEmoji);
		if (!M) return null;
		var P = I ? emojiName$1(I) : r.defaultCaption;
		return (0, import_react.createElement)(import_react.Fragment, null, (0, import_react.createElement)("div", null, B ? (0, import_react.createElement)(ViewOnlyEmoji, {
			unified: N?.unified,
			emoji: z,
			emojiStyle: F,
			size: 45,
		...
		}) : M ? (0, import_react.createElement)(ViewOnlyEmoji, {
			unified: emojiUnified$1(M),
			emoji: M,
			emojiStyle: F,
			size: 45,
			getEmojiUrl: L,
			className: cx(styles$e.emoji)
		}) : null), (0, import_react.createElement)("div", { className: cx(styles$e.label) }, B ? emojiName$1(z) : P));
	}
}
...
function categoryNameFromDom(r) {
	return r?.getAttribute("data-name") ?? null;
}
function useActiveCategoryScrollDetection(r) {
	var M = r.setActiveCategory, N = r.setVisibleCategories, P = useBodyRef();
	(0, import_react.useEffect)(function() {
		var r = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), I = P.current, L = new IntersectionObserver(function(P) {
			if (I) {
				for (var L = _createForOfIteratorHelperLoose(P), R; !(R = L()).done;) {
					var z = R.value, B = categoryNameFromDom(z.target);
					B && (r.set(B, z.intersectionRatio), F.set(B, z.isIntersecting));
				}
				var V = Array.from(r);
				N(V.filter(function(r) {
					var M = r[0];
					return r[1] > 0 || F.get(M);
				}).map(function(r) {
					return r[0];
				}));
				var H = V[V.length - 1];
				if (H?.[1] == 1) return M(H[0]);
				for (var U = 0, W = V; U < W.length; U++) {
					var G = W[U], K = G[0];
					if (G[1]) {
						M(K);
						break;
		...
		return I?.querySelectorAll(asSelectors(ClassNames.category)).forEach(function(r) {
			L.observe(r);
		}), function() {
			L.disconnect();
		};
...
function useScrollCategoryIntoView() {
	var r = useBodyRef(), M = usePickerMainRef();
	return function(N) {
		if (r.current) {
			var P = r.current?.querySelector("[data-name=\"" + N + "\"]");
			if (P) {
				var F = P.offsetTop || 0;
				scrollTo(M.current, F);
			}
		}
	};
}
function useShouldHideCustomEmojis() {
	var r = useCustomEmojisConfig();
...
function CategoryButton(r) {
	var M, N = r.isActiveCategory, P = r.category, F = r.allowNavigation, I = r.categoryConfig, L = r.onClick, R = r.customIcon, z = I.icon ?? R, B = z != null;
	return (0, import_react.createElement)(Button, {
		tabIndex: F ? 0 : -1,
		className: cx(styles$f.catBtn, commonInteractionStyles.categoryBtn, B ? styles$f.customIcon : "epr-icn-" + P, (M = {}, M[ClassNames.active] = N, M)),
		onClick: L,
		"aria-label": categoryNameFromCategoryConfig(I),
		"aria-selected": N,
		role: "tab",
		"aria-controls": "epr-category-nav-id"
	}, B ? z : null);
}
var DarkActivePositionY = { backgroundPositionY: "calc(var(--epr-category-navigation-button-size) * 3)" }, DarkPositionY = { backgroundPositionY: "calc(var(--epr-category-navigation-button-size) * 2)" }, DarkInactivePosition = { ":not(.epr-search-active)": { catBtn: {
	":hover": DarkActivePositionY,
	"&.epr-active": DarkActivePositionY
} } }, styles$f = /* @__PURE__ */ stylesheet.create(/* @__PURE__ */ _extends({
	catBtn: {
		".": "epr-cat-btn",
...
function CategoryNavigation() {
	var r = (0, import_react.useState)(null), M = r[0], N = r[1], P = useVisibleCategoriesState()[1], F = useScrollCategoryIntoView();
	useActiveCategoryScrollDetection({
		setActiveCategory: N,
		setVisibleCategories: P
	});
	var I = useIsSearchMode(), L = useCategoriesConfig(), R = useCategoryIconsConfig(), z = useCategoryNavigationRef(), B = useShouldHideCustomEmojis();
	return (0, import_react.createElement)("div", {
		...
		var P = categoryFromCategoryConfig(r), L = P === M;
		return isCustomCategory(r) && B ? null : (0, import_react.createElement)(CategoryButton, {
			key: P,
			category: P,
			isActiveCategory: L,
			allowNavigation: !I && !L,
...
var HoverDark = { ":hover": { "> .epr-icn-clear-search": { backgroundPositionY: "-60px" } } }, styles$h = /* @__PURE__ */ stylesheet.create(/* @__PURE__ */ _extends({
	btnClearSearch: {
...
function SearchContainer() {
	var r = useSearchDisabledConfig(), M = useIsSkinToneInSearch();
	return r ? null : (0, import_react.createElement)(Flex, { className: cx(styles$j.overlay) }, (0, import_react.createElement)(Search, null), M ? (0, import_react.createElement)(SkinTonePicker, null) : null);
}
function Search() {
	var r = useCloseAllOpenToggles(), M = useSearchInputRef(), N = useSearchPlaceHolderConfig(), P = useAutoFocusSearchConfig(), F = useFilter(), I = F.statusSearchResults, L = F.searchTerm, R = F.onChange, z = M?.current?.value;
	return (0, import_react.createElement)(Relative, { className: cx(styles$j.searchContainer) }, (0, import_react.createElement)("input", {
		...
		ref: M
	}), L ? (0, import_react.createElement)("div", {
		role: "status",
		className: cx("epr-status-search-results", styles$j.visuallyHidden),
		"aria-live": "polite",
		id: "epr-search-id",
		"aria-atomic": "true"
	}, I) : null, (0, import_react.createElement)(IcnSearch, null), (0, import_react.createElement)(BtnClearSearch, null));
}
...
function EmojiPicker(r) {
	return (0, import_react.createElement)(ElementRefContextProvider, null, (0, import_react.createElement)(PickerStyleTag, { nonce: r.nonce }), (0, import_react.createElement)(PickerConfigProvider, Object.assign({}, r), (0, import_react.createElement)(PickerDataProvider, null, (0, import_react.createElement)(ContentControl, null))));
}
function ContentControl() {
	var r = useReactionsModeState()[0], M = useAllowExpandReactions(), N = (0, import_react.useState)(!r), P = N[0], F = N[1], I = useOpenConfig();
	return (0, import_react.useEffect)(function() {
		r && !M || P || F(!0);
	}, [
		P,
		M,
		r
	]), I ? (0, import_react.createElement)(PickerMain, null, (0, import_react.createElement)(Reactions, null), (0, import_react.createElement)(ExpandedPickerContent, { renderAll: P })) : null;
}
function ExpandedPickerContent(r) {
	return r.renderAll ? (0, import_react.createElement)(import_react.Fragment, null, (0, import_react.createElement)(Header, null), (0, import_react.createElement)(Body, null), (0, import_react.createElement)(Preview, null)) : null;
}
var EmojiPickerReact = /* @__PURE__ */ (0, import_react.memo)(EmojiPicker, compareConfig), ErrorBoundary = /* @__PURE__ */ function(r) {
	_inheritsLoose(M, r);
	function M(M) {
		var N = r.call(this, M) || this;
		return N.state = { hasError: !1 }, N;
	}
	M.getDerivedStateFromError = function() {
		return { hasError: !0 };
	};
	var N = M.prototype;
	return N.componentDidCatch = function(r, M) {
		console.error("Emoji Picker React failed to render:", r, M);
	}, N.render = function() {
		return this.state.hasError ? null : this.props.children;
	}, M;
}(import_react.Component);
function EmojiPicker$1(r) {
	var M = useDefineMutableConfig({
		onEmojiClick: r.onEmojiClick,
		onReactionClick: r.onReactionClick,
		onSkinToneChange: r.onSkinToneChange
	});
	return (0, import_react.createElement)(ErrorBoundary, null, (0, import_react.createElement)(MutableConfigContext.Provider, { value: M }, (0, import_react.createElement)(EmojiPickerReact, Object.assign({}, r))));
}
