...
export async function POST(req: Request) {
  try {
    ...
    if (!reply) return NextResponse.json({ error: 'Nenhum provedor respondeu' }, { status: 502 });
...
async function persistChat(
  ...
  try {
    ...
    const ops: Promise<unknown>[] = [
      sb.from('messages').insert({ session_id: sessionId, user_id: userId ?? null, role: last.role, content: last.content }).then(() => undefined) as Promise<unknown>,
      sb.from('messages').insert({ session_id: sessionId, user_id: userId ?? null, role: 'assistant', content: reply }).then(() => undefined) as Promise<unknown>,
    ];
    if (body.memory && userId) {
      ...
      if (memory) {
        ops.push(sb.from('memories').insert({
          user_id: userId,
          kind: 'conversation',
...
async function injectMemory(messages: ChatMsg[], userId: string): Promise<ChatMsg[]> {
  ...
  const { data } = await sb
    ...
    .limit(50);
  const mems: string[] = (data ?? []).map((m: any) => m.content).filter(Boolean);
  ...
  return [...(systemMsgs.length ? [] : [{ role: 'system' as const, content: system }]), ...messages];
}
...
function compactMemory(text: string): string | null {
  const stop = 'âš ï¸';
  const w = text.slice(0, 700).replace(/\s+/g, ' ').replace(/^.*?:\/\//, '');
  if (w.includes('O que posso fazer') || w.includes('olÃ¡') || w.includes('oi')) return null;
 ...
 */
async function contextualizeWebSearch(messages: ChatMsg[], webSearch?: boolean): Promise<ChatMsg[]> {
  ...
  try {
    ...
    return [{ role: 'system', content: ctx }, ...(system ? [{ role: 'system' as const, content: system.content }] : []), ...rest];
  } catch {
    return messages;
  }
}

function contextualize(messages: ChatMsg[], files: { label: string; snippet?: string }[]): ChatMsg[] {
  if (!files.length) return messages;
  const ctx = files
    .map((f) => `<anexo "${f.label}">${f.snippet ? '\n' + f.snippet : ''}</anexo>`)
    .join('\n\n');
  const system = messages.find((m) => m.role === 'system');
  const rest = messages.filter((m) => m.role !== 'system');
  return [{ role: 'system', content: `Anexos fornecidos pelo usuÃ¡rio:\n${ctx}` }, ...(system ? [{ role: 'system' as const, content: system.content }] : []), ...rest];
}

async function routeByProvider(provider: string, messages: ChatMsg[], body: ChatBody): Promise<string | null> {
  if (provider.startsWith('9router/')) {
    const model = provider.replace('9router/', '');
    return route9Router(messages, model, body.nineRouterKey);
  }
  switch (provider) {
    case '9router': return route9Router(messages, undefined, body.nineRouterKey);
    case 'gemini': return routeGemini(messages, body.geminiKey);
    case 'anthropic': return routeAnthropic(messages, body.anthropicKey);
    case 'astra':
    case 'minimax':
    default: {
      // 'astra' e 'minimax' nÃ£o tÃªm chave prÃ³pria â†’ sempre via 9Router (combo padrÃ£o ou fallbacks).
      return route9Router(messages, undefined, body.nineRouterKey);
    }
  }
}

async function route9Router(messages: ChatMsg[], model?: string, userKey?: string): Promise<string | null> {
  ...
  const envFallbacks = (process.env['9ROUTER_FALLBACK_MODELS'] ?? '')
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  const requested = model && model.trim() ? model.trim() : process.env['9ROUTER_MODEL'];
  const models = [
    ...(requested ? [requested] : []),
    ...envFallbacks,
    'kr/glm-5',
...
async function routeGemini(messages: ChatMsg[], userKey?: string): Promise<string | null> {
...
async function routeAnthropic(messages: ChatMsg[], userKey?: string): Promise<string | null> {
  ...
  try {
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model, max_tokens: 2048, ...(system ? { system } : {}), messages: conversation }),
      signal: AbortSignal.timeout(60_000),
    });
    if (!res.ok) return null;
    const data = (await res.json()) as any;
