...
export async function DELETE(req: Request) {
  try {
    ...
    if (!admin) {
      return NextResponse.json({ ok: false, error: 'Servidor sem service role configurado' }, { status: 500 });
    }

    // Apaga dados do usuário (best-effort por tabela).
    const tables = ['messages', 'memories', 'skills', 'projects', 'media', 'connectors', 'user_settings', 'sessions'];
    for (const t of tables) {
      try { await admin.from(t).delete().eq('user_id', userId); } catch { /* tabela pode não existir */ }
    }
