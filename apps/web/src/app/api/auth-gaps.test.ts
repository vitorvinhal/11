/**
 * AUTH-GAPS-002 — testes de regressão das 10 rotas que estavam sem requireUser.
 * Cenário: request SEM cookie/token de sessão → 401 em TODOS os handlers exportados.
 * Mock: auth-unify.requireUser → null (sem sessão), simulando produção sem JWT.
 */
...
describe("AUTH-GAPS-002 — 10 rotas exigem sessão (401 sem cookie/token)", () => {
  ...
  test("métodos alternativos de /api/media também retornam 401 sem sessão", async () => {
  ...
  test("método alternativo: POST /api/stt e POST /api/tts → 401 (já cobertos) + handlers alternativos inexistente não quebram import", () => {
