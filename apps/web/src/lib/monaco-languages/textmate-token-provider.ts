...
export type TextMateGrammarLoader = (
  scopeName: string,
...
async function loadBrowserOniguruma(): Promise<IOnigLib> {
  browserOnigurumaPromise ??= (async () => {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load TextMate regex engine from ${onigurumaWasmUrl}`,
      );
    }
    ...
    tokenize(line, state) {
      const textMateState =
        state instanceof TextMateTokenizerState
          ? state
          : new TextMateTokenizerState(INITIAL);
...
export async function createTextMateTokensProvider(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `No TextMate grammar registered for scope ${options.scopeName}`,
      );
    }
    throw error;
  }
