...
export default function TerminalPanel() {
  ...
  const [sessions, setSessions] = useState<TermSession[]>([{ id: uid(), name: 'Terminal 1' }]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      handlesRef.current.forEach((h) => { h.container.style.display = h.id === activeId ? '' : 'none'; });
    ...
    <div className="flex h-full flex-col bg-[#0a0c10]">
      <div className="flex items-center gap-1 border-b border-border px-3 py-1.5">
        <div className="mr-2 flex items-center gap-1.5 text-[11px] text-text-dim">
          ...
          <span className={`ml-1 h-1.5 w-1.5 rounded-full ${connected ? 'bg-emerald-400' : 'bg-amber-400'}`} />
        </div>
        {sessions.map((s) => (
          <button key={s.id} onClick={() => setActiveId(s.id)}
            ...
            {s.name}
            {sessions.length > 1 && (
              <span onClick={(e) => { e.stopPropagation(); closeSession(s.id); }} className="opacity-0 group-hover:opacity-100 hover:text-rose-400">
                <X className="h-3 w-3" />
              </span>
            )}
          </button>
        ))}
        <button onClick={createSession} className="ml-1 grid h-6 w-6 place-items-center rounded-md text-text-dim hover:bg-white/[0.06] hover:text-text-primary transition" title="Novo terminal">
          <Plus className="h-3.5 w-3.5" />
        </button>
        <div className="ml-auto truncate font-mono text-[10px] text-text-dim">{cwd}</div>
      </div>
...
async function createXterm(
  ...
  const term = new Terminal({
    ...
    theme: {
      ...
      black: '#1b1e26', red: '#f87171', green: '#34d399', yellow: '#fbbf24',
      blue: '#60a5fa', magenta: '#c084fc', cyan: '#22d3ee', white: '#e6e6e6',
      brightBlack: '#6b7280', brightRed: '#fb7185', brightGreen: '#4ade80',
  ...
  term.loadAddon(fit);
  try { term.loadAddon(new WebLinksAddon()); } catch { /* opcional */ }
  term.open(container);
  setTimeout(() => { try { fit.fit(); } catch { /* ignore */ } }, 40);
  ...
  fetch(`/api/terminal/exec?sessionId=${encodeURIComponent(sessionId)}`)
    .then((r) => r.json())
    .then((d) => { currentCwd = d.cwd ?? ''; onCwd(currentCwd); onStatus(true); })
    .catch(() => onStatus(false));

  say(`\x1b[38;5;80m11 Terminal\x1b[0m — rodando em ${typeof navigator !== 'undefined' ? 'seu PC' : ''}. Ex.: \x1b[38;5;114mls\x1b[0m, \x1b[38;5;114mgit status\x1b[0m, \x1b[38;5;114mopencode\x1b[0m\r\n`);
  drawPrompt();
  ...
  const runCommand = async (cmd: string) => {
    ...
    try {
      ...
      if (!res.ok && ct.includes('application/json')) {
        ...
        busy = false; drawPrompt(); return;
      }
      if (!res.body) { busy = false; drawPrompt(); return; }
      ...
      for (;;) {
        ...
        while ((idx = buffer.indexOf('\n\n')) !== -1) {
          ...
          try { data = JSON.parse(data); } catch { /* texto puro */ }
          if (event === 'stdout' || event === 'stderr') {
            say(String(data).replace(/\r?\n/g, '\r\n'));
          } else if (event === 'exit') {
            fetch(`/api/terminal/exec?sessionId=${encodeURIComponent(sessionId)}`)
              .then((r) => r.json())
              .then((d) => { currentCwd = d.cwd ?? currentCwd; onCwd(currentCwd); })
              .catch(() => {});
          }
        }
      }
  ...
  term.onData((data: string) => {
    ...
    for (const ch of data) {
      ...
      if (ch === '\r') {
        ...
        term.write('^C'); line = ''; term.write('\r\n'); drawPrompt();
      } else if (ch === '\u000c') { // Ctrl+L
        term.write('\x1b[2J\x1b[H'); term.write(prompt() + line);
      } else if (code >= 32) {
        line += ch;
        term.write(ch);
      }
