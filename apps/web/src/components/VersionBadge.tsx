...
/**
 * VersionBadge — badge discreto com versão + tooltip com changelog completo.
 * Estilo: badge sutil no canto inferior da sidebar (desktop) ou rodapé da landing.
 */
      ...
      {open && (
        <div className="absolute bottom-full left-0 mb-2 w-80 glass-card rounded-xl p-4 z-50 shadow-2xl">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] text-emerald-400">v{info.version}</span>
          </div>
          <div className="space-y-1 mb-3">
            {info.changelog.slice(0, 5).map((item, i) => (
              <p key={i} className="text-[11px] text-white/50">• {item}</p>
            ))}
          </div>
          <div className="flex gap-2">
            {info.apkUrl && (
              <a href={info.apkUrl} target="_blank" rel="noopener noreferrer"
            ...
            {info.desktopUrl && (
              <a href={info.desktopUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1 rounded-lg bg-white/[0.06] px-2.5 py-1.5 text-[10px] text-white/70 hover:bg-white/[0.10] transition">
                <Download className="h-3 w-3" /> Windows
              </a>
            )}
            <a href={typeof location !== 'undefined' ? location.origin : '/'} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1 rounded-lg bg-white/[0.06] px-2.5 py-1.5 text-[10px] text-white/70 hover:bg-white/[0.10] transition">
