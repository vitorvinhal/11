...
export default function ConnectorsPanel({ userId }: { userId: string }) {
  ...
  const loadConnectors = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/connectors?userId=${userId}`, {
  ...
  const getConnector = (id: string) =>
    connectors.find((c) => c.provider === id);

  const handleConnect = async (provider: { id: string; authType: string }) => {
    if (provider.authType === "oauth") {
      try {
        const token = await getAccessToken();
        const res = await fetch(`/api/connectors/${provider.id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        if (res.ok && data?.url) {
        ...
        /* falha silenciosa — usuário pode retryar */
      }
    }
  };
  ...
  const handleDisconnect = async (providerId: string) => {
    ...
    if (!conn) return;
    const token = await getAccessToken();
    await fetch(`/api/connectors?id=${conn.id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    loadConnectors();
  ...
  const connectorActions: Record<
    string,
    { label: string; action: string; icon: any }[]
  > = {
    google: [
            ...
            {yourConnectors.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Conecte serviços externos para usar seus dados no chat
                </p>
              ...
              <div className="space-y-3">
                ...
                {testResult && (
                  <div
                    className={`rounded-xl border p-4 ${testResult.ok ? "bg-green-500/5 border-green-500/20" : "bg-red-500/5 border-red-500/20"}`}
                  >
