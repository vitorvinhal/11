...
const embedding_dims_1 = require("./embedding-dims");
/**
...
class SessionWriteQueue {
    ...
    upsertEmbedding(args) {
        return this.enqueue(args.sessionId, async () => {
            if (args.embedding.length !== embedding_dims_1.EMBEDDING_DIMS) {
                ...
                console.warn(`[session-write-queue] embedding com ${args.embedding.length} dims ` +
                    `!= EMBEDDING_DIMS (${embedding_dims_1.EMBEDDING_DIMS}). Vetor NÃO gravado em embeddings.`);
