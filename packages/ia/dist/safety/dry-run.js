 ...
 */
async function simulateSQL(query, ctx) {
 ...
 */
async function simulateCLI(command, ctx) {
    const start = Date.now();
 ...
 */
async function simulateFilesystem(operation, params, ctx) {
    const start = Date.now();
    const { existsSync, statSync } = await Promise.resolve().then(() => __importStar(require('fs')));
...
async function dryRun(action, ctx) {
    ...
    try {
        if (action.startsWith('sql.')) {
            const query = (ctx.params.query ?? ctx.params.sql ?? '');
        ...
        if (action.startsWith('git.')) {
            ...
            const command = `git ${sub} ${(ctx.params.args ?? '')}`.trim();
            return await simulateCLI(command, ctx);
        }
