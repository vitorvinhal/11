...
import type { Checkpoint, PendingAction, CreateCheckpointInput } from './checkpoint';
...
describe('Checkpoint & Rollback — FASE 4C', () => {
  ...
  describe('Type definitions', () => {
    ...
    test('Checkpoint has correct status values', () => {
      const statuses: Array<Checkpoint['status']> = [
    ...
    test('PendingAction has correct status values', () => {
      const statuses: Array<PendingAction['status']> = [
  ...
  describe('Pending actions', () => {
    ...
    test('approval flow: pending → approved → executed', () => {
      const flow: PendingAction['status'][] = [
    ...
    test('rejection flow: pending → rejected', () => {
      const flow: PendingAction['status'][] = ['pending', 'rejected'];
  ...
  describe('Rollback scenarios', () => {
    test('file write rollback: before_state contains original content', () => {
      ...
      const state = checkpoint.beforeState as Record<string, unknown>;
    ...
    test('SQL rollback: before_state contains original row', () => {
      ...
      expect((state.row as Record<string, unknown>).name).toBe('Original Name');
    });
