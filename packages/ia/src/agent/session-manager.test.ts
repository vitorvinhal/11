...
import type { AgentSession, SessionMessage, CreateSessionInput, AddMessageInput, SessionStatus } from './session-manager';
...
describe('Session Manager — FASE 5C', () => {
  ...
  describe('Session status lifecycle', () => {
    test('valid statuses', () => {
      const statuses: SessionStatus[] = ['active', 'paused', 'completed', 'expired'];
      expect(statuses).toHaveLength(4);
    });

    test('active → paused → active flow', () => {
      const flow: SessionStatus[] = ['active', 'paused', 'active'];
      expect(flow).toHaveLength(3);
    });

    test('active → completed flow', () => {
      const flow: SessionStatus[] = ['active', 'completed'];
      expect(flow[flow.length - 1]).toBe('completed');
    });

    test('active → expired flow', () => {
      const flow: SessionStatus[] = ['active', 'expired'];
