...
import type { Memory, SaveMemoryInput, SearchMemoryInput, MemorySearchResult, MemoryKind } from './memory';
...
describe('Memory Module — FASE 6B', () => {
  ...
  describe('Memory kinds', () => {
    test('valid kinds', () => {
      const kinds: MemoryKind[] = ['conversation', 'fact', 'preference', 'task', 'error'];
      expect(kinds).toHaveLength(5);
    });

    test('conversation kind', () => {
      const kind: MemoryKind = 'conversation';
